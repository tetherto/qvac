...
| **Text-to-Speech** | Speech synthesis (TTS) via [`@qvac/tts-ggml`](https://github.com/tetherto/qvac/tree/main/packages/tts-ggml) — Chatterbox, Supertonic, Parler, and CosyVoice3 (CPU, with opt-in GPU offload: Metal on Apple, Vulkan on Linux/Windows, OpenCL/Adreno on Android), plus Audio8 (CPU, with optional desktop Vulkan acceleration). See [Choosing a model](packages/tts-ggml/README.md#choosing-a-model). |
