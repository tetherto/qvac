...
// (with spread), then downloads just the chosen images into media/ + test/mobile/
// testAssets/ and rewrites test/integration/vlm-fixture.data.cjs + a NOTICE.
//
...
//
// Usage: node scripts/build-vlm-fixture.cjs [--per-task 3] [--max-side 1024] [--scan 2000]
...
const PKG = path.join(ROOT, 'packages', 'llm-llamacpp')
const MEDIA_DIR = path.join(PKG, 'media')
const ASSET_DIR = path.join(PKG, 'test', 'mobile', 'testAssets')
const FIXTURE = path.join(PKG, 'test', 'integration', 'vlm-fixture.data.cjs')
const NOTICE = path.join(PKG, 'test', 'integration', 'vlm-fixture.NOTICE.md')
...
async function main () {
  fs.mkdirSync(MEDIA_DIR, { recursive: true }); fs.mkdirSync(ASSET_DIR, { recursive: true })
  const items = []
  const notice = ['# VLM fixture — image sources & licences', '', 'Auto-generated by `scripts/build-vlm-fixture.cjs`. All datasets are on the open-licence allowlist.', '']
  ...
  for (const task of taskNames) {
    ...
    for (let i = 0; i < chosen.length; i++) {
      ...
      await download(c.src, path.join(MEDIA_DIR, image))
      fs.copyFileSync(path.join(MEDIA_DIR, image), path.join(ASSET_DIR, image))
  ...
  const header = `'use strict'\n// QVAC-19178: VLM benchmark fixture — AUTO-GENERATED by scripts/build-vlm-fixture.cjs.\n// Open-licensed samples (see vlm-fixture.NOTICE.md), longest side <= ${MAX_SIDE}px.\nmodule.exports = `
  fs.writeFileSync(FIXTURE, header + JSON.stringify(data, null, 2) + '\n')
  fs.writeFileSync(NOTICE, notice.join('\n') + '\n')

  // Prune any committed vlmx-* images no longer referenced (e.g. dropped tasks).
  const keep = new Set(items.map(it => it.image))
  for (const dir of [MEDIA_DIR, ASSET_DIR]) {
    for (const f of fs.readdirSync(dir)) {
      if (/^vlmx-.*\.(png|jpe?g|webp|gif)$/i.test(f) && !keep.has(f)) { fs.rmSync(path.join(dir, f)); console.log(`pruned ${path.relative(ROOT, path.join(dir, f))}`) }
    }
  }
  console.log(`\nWrote ${items.length} items across ${present.size} tasks -> ${path.relative(ROOT, FIXTURE)}`)
}
