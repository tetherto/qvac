...
Use this skill only for setup/enable/disable. For reading, editing, or adding diary entries after setup, use `qv-dev-diary`.
...
3. Description suggestion: `Local qvac-pr-skills`.
...
6. Store it in your default shell startup file or export it in the current shell. Coding agents may not inherit the app environment, so the helper falls back to asking your default shell for this variable.
...
```bash
node .agents/skills/_lib/developer-workflow/diary-cli.mjs --init --basic
```

Local diary plus Asana config import:

```bash
node .agents/skills/_lib/developer-workflow/diary-cli.mjs --init /path/to/asanaconfig.json
```
...
```bash
node .agents/skills/_lib/developer-workflow/diary-cli.mjs --off
```
