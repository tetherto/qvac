...
1. Run the script with `--mode my`. The script uses `.agents/skills/_lib/pr-skills/pr-activity.mjs` for shared PR state collection and `~/.config/qvac-pr-skills/config.json` for repo/stale settings when available.
