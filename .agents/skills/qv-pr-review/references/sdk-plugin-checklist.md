...
- `packages/sdk/client/**` other than `client/api/**` — internal client helpers, covered by the SDK's scoped instructions
...
For non-plugin SDK PRs, use the nearest scoped `AGENTS.md`, package documentation, and current tests; re-running this plugin-specific checklist would just be noise.

---

## How to apply

The checklist's value is **catching plugin-architecture gaps the general SDK guidance does not cover** — the `definePlugin` contract, 1:1 plugin↔package coupling, cross-cutting registration points (`server/worker.ts` + `plugins/index.ts` + `package.json` exports + bundler `BUILTIN_PLUGINS`), strict unknown-key policy on plugin configs, role-based companion naming, and the streaming protocol with exactly one terminal `done: true`.
...
See `packages/sdk/AGENTS.md`. Native addon-only PRs skip this section.
