...
```bash
node .agents/skills/_lib/developer-workflow/daily-work-update.mjs
```

Specific date:

```bash
node .agents/skills/_lib/developer-workflow/daily-work-update.mjs --date YYYY-MM-DD
```
...
- PR queue state through `.agents/skills/_lib/pr-skills/pr-status.mjs --json` when queue context is needed
- Asana through `.agents/skills/_lib/developer-workflow/asana.mjs` when configured
...
- Prefer diary entries as the source of truth for agent-assisted work.
