...
if [[ "$(basename "$PWD")" == "ggml-rpc-server" ]]; then
  ...
  RPC_BACKEND=$(find prebuilds -type f -name libqvac-ggml-rpc.so -print -quit)
  if [[ -z "$RPC_SERVER" ]]; then
    echo "ggml-rpc-server executable is missing from the prebuild" >&2
    exit 1
  fi
  if [[ -z "$RPC_BACKEND" ]] || ! grep -aFq 'RDMA auto-negotiate enabled' "$RPC_BACKEND"; then
    echo "ggml-rpc-server RPC backend does not contain the RDMA capability marker" >&2
    exit 1
  fi
fi
