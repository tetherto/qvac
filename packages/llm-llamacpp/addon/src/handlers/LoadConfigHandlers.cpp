...
// Selects the idefics3-style no-upscale preprocessing rule. Tri-state on the fabric
// side, but the config map can only say "the caller set something", so this handler
// only ever writes 0 or 1; leaving the key out keeps fabric's -1 model default.
// Needed because the VisionPsy base and Flash mmprojs declare identical vision
// hparams, so without it a Flash model silently runs base preprocessing.
static void
