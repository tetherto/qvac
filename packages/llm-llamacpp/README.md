...
| spec-type         | `"draft-mtp"`                               | — (off)                      | Enable MTP (Multi-Token Prediction) self-speculative decoding. Requires a model that bundles an MTP / next-n head (e.g. `Qwen3.5-*-MTP`); the model's own head drafts ahead and the target verifies in one batched decode. Inert (no speedup, no error) on models without the head, and ignored under continuous batching (`parallel > 1`). Reports `draftAccepted` / `draftTotal` in runtime stats |
