...
With `cacheKey` and `saveCacheToDisk: true`, text, multimodal, and continuous-batch requests pin the existing committed cache artifact as the pre-request transaction checkpoint without rewriting it. Cache loading is resolved first. A missing, corrupt, or dirty baseline fails before tokenization, context sliding, or decode mutates request memory. Empty persistent baselines use an in-memory marker and no file. Successful requests release the pin before normal persistence may atomically replace the cache.
