...
| cache_checkpoints | 0 – 1024                                    | 32                           | Per-sequence cap on process-local full-state checkpoints kept for cached requests on hybrid / recurrent models. Each is a full copy of the sequence state; `0` disables them. Also accepted as `cache-checkpoints` ([details](./docs/cache-api.md), [how it works](./docs/cache-lifecycle.md)) |
...
| cache_checkpoint_storage | `"disk"` or `"memory"`             | `"disk"`                     | Where checkpoints and the per-request rollback snapshot live. `"memory"` keeps them in host RAM so a cached chat never touches the disk. Also accepted as `cache-checkpoint-storage` |
