...
With `cacheKey` and `saveCacheToDisk: true`, text, multimodal, and continuous-batch requests pin a committed cache artifact as the pre-request transaction checkpoint. If a single-prompt same-key session has newer RAM state than disk, the addon first writes that live RAM state to the canonical cache file and pins it before mutating request memory. A non-empty persistent request with no live state to commit and a missing, unusable, or unpinnable rollback artifact fails before tokenization, media loading, context sliding, or decode mutates request memory. Empty persistent baselines use an in-memory marker and no file. Successful requests release the pin before normal persistence may atomically replace the cache with the updated state.

Explicit prefill or generation cancellation restores that committed checkpoint's sequence bytes and artifact-derived position/cache metadata as one coherent unit. Restore first clears the target sequence, then loads the pinned artifact so interrupted prompt or answer tokens cannot survive beside the checkpoint. For dirty same-key single-prompt sessions, the restored checkpoint is the RAM state committed immediately before the request. Partial output may already have streamed to the caller, but it is not retracted and is not retained in KV or recurrent model memory. A prediction-limit cutoff inside an unclosed reasoning block follows the same policy because there is no retained answer to replay. If restoration or metadata validation fails, the affected sequence is cleared, its active cache session is invalidated, that request is not saved, and the last-known-good on-disk cache is preserved.
