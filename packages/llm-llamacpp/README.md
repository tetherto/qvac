...
With `cacheKey` and `saveCacheToDisk: true`, text, multimodal, and continuous-batch requests pin the last valid committed cache artifact as the pre-request transaction checkpoint without rewriting it. Newer unsaved live state is not durable and does not block admission. If no valid persisted checkpoint is available, the request proceeds without one. Empty persistent baselines use an in-memory marker and no file. Successful requests release the pin before normal persistence may atomically replace the cache.

Explicit prefill or generation cancellation restores that committed checkpoint, even when it predates newer unsaved live state. Losing that unsaved delta is expected. Partial output may already have streamed to the caller, but it is not retracted and is not retained in KV or recurrent model memory. A prediction-limit cutoff inside an unclosed reasoning block follows the same policy because there is no retained answer to replay. If no checkpoint exists or restoration fails, the affected sequence is cleared, its active cache session is invalidated, that request is not saved, and the last-known-good on-disk cache is preserved.
