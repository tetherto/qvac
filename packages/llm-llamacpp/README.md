...
| cache_checkpoints | 0 – 1024                                    | 32                           | Per-sequence cap on process-local full-state checkpoints kept for cached requests on hybrid / recurrent models. Each is a full sequence dump in the temp directory; `0` disables them. Also accepted as `cache-checkpoints` ([details](./docs/cache-api.md)) |
