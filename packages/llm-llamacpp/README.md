...
| spec-type         | `"draft-mtp"`                               | off                          | Enable MTP (Multi-Token Prediction) self-speculative decoding. The MTP / next-n head can be bundled in the target model or supplied through `spec-draft-model`. Inert (no speedup, no error) when no usable head is available, and ignored under continuous batching (`parallel > 1`). Reports `draftAccepted` / `draftTotal` in runtime stats |
| spec-draft-model  | file path                                     | none                         | Optional separate MTP head model used with `spec-type=draft-mtp`. When omitted, the addon uses a head bundled in the target model |
| spec-draft-n-max  | positive integer                              | 3                            | Maximum draft tokens proposed per MTP verification round |
| spec-draft-n-min  | non-negative integer                          | 0                            | Minimum draft tokens proposed per MTP verification round |
| spec-draft-p-min  | number                                        | 0                            | Minimum probability for accepting a speculative draft token |
| spec-draft-backend-sampling | boolean                             | true                         | Enable backend sampling for the MTP draft context |
| spec-draft-device | device name                                   | —                            | Device used by the MTP draft context |
| spec-draft-ngl    | integer or `"auto"`                          | `"auto"`                   | Number of MTP draft layers offloaded to the GPU |
| spec-draft-type-k | cache type                                    | f16                          | K-cache type used by the MTP draft context |
| spec-draft-type-v | cache type                                    | f16                          | V-cache type used by the MTP draft context |
