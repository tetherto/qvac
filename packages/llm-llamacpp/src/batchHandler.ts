...
import type { BatchOutputChunk, BatchPrompt, BatchResult, Message, RunOptions } from "./index";
...
function runBusyError(): Error {
  const err: Error & { code?: string } = new Error(RUN_BUSY_ERROR_MESSAGE);
 ...
 */
class BatchHandler {
  static readonly RUN_BUSY_ERROR_MESSAGE = RUN_BUSY_ERROR_MESSAGE;
  static readonly RUN_BUSY_ERROR_CODE = RUN_BUSY_ERROR_CODE;
  static readonly runBusyError = runBusyError;

  private readonly _parsePrompt: BatchHandlerDeps["parsePrompt"];
  private readonly _cancelHandler: BatchHandlerDeps["cancelHandler"];
  private readonly _runJob: BatchHandlerDeps["runJob"];
  /// native jobId -> { ids, response, pendingResult }
  private readonly _groups: Map<number, BatchGroup>;
  /// per-prompt string id -> native jobId, for routing streaming chunks
  private readonly _chunkRoutes: Map<string, number>;
  ...
  static isBatchInput(prompt: unknown): prompt is BatchInput {
    ...
    return (
      ...
      (!!first &&
        typeof first === "object" &&
        !Array.isArray(first) &&
        Array.isArray((first as BatchPrompt).prompt))
    );
  }

  /** Whether a batch item is a `{ id?, prompt, runOptions? }` wrapper. */
  private static _isWrapped(item: unknown): item is BatchPrompt {
    return (
      !!item &&
      typeof item === "object" &&
      !Array.isArray(item) &&
      Array.isArray((item as BatchPrompt).prompt)
    );
  }
   ...
   */
  async run(batchInput: BatchInput): Promise<BatchGroupResponse> {
    ...
    const response = new QvacResponse<BatchOutputChunk>({
      cancelHandler: () => this._cancelHandler(jobId) as Promise<void>,
    }) as BatchGroupResponse;

    let result: AddonBatchRunResult;
    try {
      result = await this._runJob(items);
    } catch (err) {
      response.failed(err as Error);
      throw err;
    }
    if (!result.accepted) {
  ...
  onOutput(data: { id: string; output: string }): void {
  ...
  onJobEnded(jobId: number, stats: unknown): void {
    ...
    const finalResult: BatchResult[] = group.ids.map((id, index) => ({
      id,
  ...
  private _drop(jobId: number, group: BatchGroup): void {
    this._groups.delete(jobId);
    for (const id of group.ids) this._chunkRoutes.delete(id);
  }

  private _unwrapItems(batchInput: BatchInput): AddonBatchRunItem[] {
    return batchInput.map((item) => {
      ...
      const unwrapped: AddonBatchRunItem = { messages: this._parsePrompt(prompt, itemRunOptions) };
