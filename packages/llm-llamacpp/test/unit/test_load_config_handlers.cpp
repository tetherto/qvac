...
// Absent key must leave the -1 sentinel alone, otherwise every existing caller would
// start forcing base preprocessing instead of honouring the GGUF.
...
TEST(LoadConfigHandlers_ImageNoUpscale, RejectsUnknownValue) {
  common_params params;
  std::unordered_map<std::string, std::string> map{{"image-no-upscale", "maybe"}};
  EXPECT_THROW(applyLoadConfigHandlers(params, map), StatusError);
}
