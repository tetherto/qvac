...
/// That callback runs on the scheduler's worker thread while it streams, so
/// this also pins the contract that cancelById is safe to call from inside
/// outputCallback - it must never take a lock the streaming worker already
/// holds.
TEST_F(ConcurrentProcessByIdTest, CancelByIdCancelsOnlyTargetedJob) {
  ...
  auto cancelPrompt = makePrompt(
      ...
      "astronomy.");
  cancelPrompt.outputCallback = [&model, &cancelFired](const std::string&) {
    bool expected = false;
