...
using qvac_lib_inference_addon_llama::kMaxParallelWorkers;

/// `parallel` is validated against kMaxParallelWorkers in createInstance and
...
TEST(ParallelCeiling, MirrorsTheEngineSequenceLimit) {
  EXPECT_EQ(kMaxParallelWorkers, 256U)
      << "kMaxParallelWorkers must equal LLAMA_MAX_SEQ in the pinned "
         "qvac-fabric (src/llama-cparams.h). If fabric changed it, update "
         "this expectation, kMaxParallelWorkers, MAX_PARALLEL in index.js, "
...
TEST(ParallelCeiling, CeilingFitsInTheContextSequenceField) {
  ...
  cparams.n_seq_max = kMaxParallelWorkers;
  EXPECT_EQ(cparams.n_seq_max, kMaxParallelWorkers);
  EXPECT_GE(kMaxParallelWorkers, 1U);
}
...
TEST(ParallelCeiling, DefaultParallelIsWithinTheCeiling) {
  ...
  EXPECT_LE(static_cast<unsigned>(params.n_parallel), kMaxParallelWorkers);
}
