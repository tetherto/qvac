...
//   mmproj-enc (ms)  pure vision-encode time — desktop parses llama.cpp's stderr,
//                    mobile reads the addon's visionEncodeMs runtime stat
//   TTFT (ms)        time-to-first-token (all platforms)
...
for (const f of files) {
  ...
  // The combine report carries the active preset (rendered under the Mode label);
  // reflect it instead of assuming `full` — and make a preset mix impossible to miss,
  // since a checkpoint is only comparable when every run used the same preset.
  const pm = txt.match(/\*\*Preset:\*\* `([^`]+)`/)
  if (pm) presets.add(pm[1])
...
const presetLabel = presets.size
  ? [...presets].sort().join(' + ') + (presets.size > 1 ? ' ⚠ MIXED PRESETS (runs are not comparable)' : '')
  : '?'
const out = []
...
for (const md of MODELS) {
  out.push('')
  out.push(`## ${md.name}`)
  out.push(`**LLM \`${md.llm}\` · mmproj \`${md.mmproj}\` · addon \`@qvac/llm-llamacpp@${addon || '?'}\` · preset \`${presetLabel}\` · n=${maxN} runs · ${date}**`)
  out.push('')
  // A model with no rows gets an explicit note instead of a bare table header —
  // silent absence could hide a parsing failure; loud absence is a prompt to check
  // the collected reports (wrong runs, wrong models, or a report-format change).
  const modelRows = []
  for (const plat of orderedPlats) {
    for (const accel of ['CPU', 'GPU']) {
      const d = data[`${md.key}|${plat}|${accel}`]; if (!d) continue
      modelRows.push(`| ${plat} · ${accel} | ${cell(agg(d.mmproj), 0, maxN)} | ${cell(agg(d.ttft), 0, maxN)} | ${cell(agg(d.wall), 0, maxN)} | ${cell(agg(d.cog), 1, maxN)} | ${cell(agg(d.ocr), 1, maxN)} |`)
    }
  }
  if (!modelRows.length) {
    out.push(`_No \`${md.key}\` rows in the collected reports — did the runs use the checkpoint model set?_`)
    continue
  }
...
out.push('')
out.push('> Value = avg ± deviation% (stdev/mean across runs). `mmproj-enc` = the pure vision-encode time ' +
  '(desktop parses llama.cpp\'s stderr; mobile reads the addon\'s `visionEncodeMs` runtime stat); `TTFT` is reported on all platforms. ' +
