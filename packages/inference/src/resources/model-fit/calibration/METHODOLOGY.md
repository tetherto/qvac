...
- The SDK pinned `n_gpu_layers: 99` when this was measured, so llama.cpp's own
  fit could not reduce the layer count to what the device holds and the load
  hard-failed instead of degrading. The pin is gone (QVAC-25039); the abort
  above will not reproduce.
