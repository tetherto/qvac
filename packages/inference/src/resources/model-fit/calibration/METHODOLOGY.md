...
The `android-arm64` fixture is a bridge. It is frozen as measured and is not
re-run on engine bumps; it is retired for llama.cpp workloads once
`assessModelFit` takes its mobile verdicts from the engine's own fitter, at
which point calibrated estimates remain only as the fallback for hosts where the
fitter cannot run.
