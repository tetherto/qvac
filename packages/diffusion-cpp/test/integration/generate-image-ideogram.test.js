...
const PROMPT = JSON.stringify({
  ...
  compositional_deconstruction: {
    ...
    elements: [
      { type: 'obj', bbox: [230, 260, 800, 740], desc: 'Exactly one matte yellow ceramic coffee mug centered on the desk.' },
      { type: 'text', bbox: [510, 360, 620, 640], text: 'IDEO', desc: 'Small black label printed horizontally on the mug.' }
    ]
  }
})
...
safeTest('Ideogram 4 txt2img smoke - generates a valid PNG image', { timeout: 3600000, skip }, async (t) => {
  let model = null
  try {
    ...
    t.is(progressTicks[progressTicks.length - 1].total, 2, 'Final progress tick reports 2 total steps')
    ...
    const outPath = path.join(path.dirname(diffusionModelPath), 'generate-image--ideogram-smoke-seed42.png')
