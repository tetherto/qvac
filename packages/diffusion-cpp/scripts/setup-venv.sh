#!/usr/bin/env bash
#
# Create a local Python venv at ./venv with the packages the LAM
# Audio2Expression checkpoint -> .gguf converter needs (see
...
#
# After provisioning, the converter is invoked via
# scripts/convert-lam-a2e.sh (or `npm run convert:lam-a2e`), which
# auto-discovers ./venv/{bin,Scripts}/python before falling back to the
# system python3.
...
echo "  ./scripts/convert-lam-a2e.sh --checkpoint lam_audio2exp_streaming.tar --out models/lam-audio2exp-f32.gguf"
echo "(or:   npm run convert:lam-a2e -- --checkpoint lam_audio2exp_streaming.tar --out models/lam-audio2exp-f32.gguf)"
