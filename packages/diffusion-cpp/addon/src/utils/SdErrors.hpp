...
inline constexpr const char* kAddonId = "Diffusion";
inline constexpr const char* kCancelled = "Cancelled";
inline constexpr const char* kJobCancelledMessage = "Job cancelled";

// Convenience factory so the four cancel sites stay perfectly in sync.
inline qvac_errors::StatusError makeCancelledError() {
  return qvac_errors::StatusError(kAddonId, kCancelled, kJobCancelledMessage);
}
