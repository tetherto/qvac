...
std::vector<uint8_t> encodeFramesToAvi(const sd_image_t *frames, int numFrames,
  ...
  if (fps <= 0) {
    throw StatusError(general_error::InvalidArgument,
                      "encodeFramesToAvi: fps must be > 0, got " +
                          std::to_string(fps));
  }
  ...
  if (framesSz > (SIZE_MAX - kHeaderBytes) / perFrameWithOverhead) {
    throw StatusError(general_error::InvalidArgument,
                      "encodeFramesToAvi: " + std::to_string(numFrames) +
                          " frames at " + std::to_string(width) + "x" +
                          std::to_string(height) +
  ...
  if (estimated > static_cast<size_t>(UINT32_MAX)) {
    throw StatusError(general_error::InvalidArgument,
                      "encodeFramesToAvi: estimated output (" +
  ...
  for (int i = 0; i < numFrames; ++i) {
    ...
    const int rc = stbi_write_jpg_to_func(
        &jpegSinkWriteCb, &sink, static_cast<int>(f.width),
        static_cast<int>(f.height), static_cast<int>(f.channel), f.data,
        jpegQuality);
  ...
  // on JPEG compression so re-check before truncating to uint32_t.
  {
    ...
    if (riffPayload > static_cast<size_t>(UINT32_MAX)) {
      throw StatusError(general_error::InternalError,
                        "encodeFramesToAvi: produced AVI of " +
