...
void SdModel::load() {
  ...
  const auto configDevice =
      sd_backend_selection::parseConfigDeviceString(config_.device);
  if (configDevice == sd_backend_selection::ConfigDevice::Gpu) {
    std::optional<sd_backend_selection::MainGpuSpec> mainGpuSpec;
    if (!config_.mainGpu.empty()) {
      mainGpuSpec = sd_backend_selection::parseMainGpu(config_.mainGpu);
    }
    ...
    if (auto resolved = sd_backend_selection::resolveGpuBackendName(
      ...
      params.backend = resolvedBackend.c_str();
      QLOG_IF(
          qvac_lib_inference_addon_cpp::logger::Priority::INFO,
          "stable-diffusion backend resolved to '" + resolvedBackend + "'");
  ...
  } else if (!config_.mainGpu.empty() || !config_.backend.empty()) {
    QLOG_IF(
        qvac_lib_inference_addon_cpp::logger::Priority::INFO,
        "main-gpu and backend ignored because device is 'cpu'");
  }
