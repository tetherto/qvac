...
function loadAddon() {
  ...
  try {
    ...
    cause = new Error(
      '@qvac/audiogen-ggml: require.addon() answered with a module that is not the native binding ' +
...
function loadPlatformPackageAddon(cause) {
  ...
  if (!isNativeBinding(addon)) {
    const err = new Error(
      '@qvac/audiogen-ggml resolved #host-addon to a module that is not the native binding: ' +
