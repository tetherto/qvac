...
export const REGISTRY_SOURCE = 's3'
...
export const REGISTRY_PREFIX = 'qvac_models_compiled/ggml/acestep/2026-07-22'
...
export const DEFAULT_DIT_VARIANT: DitVariant = 'turbo-q4'

export function ditVariants (): DitVariant[] {
  return Object.keys(DIT_VARIANTS) as DitVariant[]
}
...
export function ditFilename (variant: DitVariant = DEFAULT_DIT_VARIANT): string {
...
export function modelFilenames (variant: DitVariant = DEFAULT_DIT_VARIANT): ModelManifest {
  return {
...
export function modelManifest (variant: DitVariant = DEFAULT_DIT_VARIANT): ModelManifest {
...
export function modelSources (variant: DitVariant = DEFAULT_DIT_VARIANT): ModelSources {
...
export function resolveDitModelPath (
  { modelDir, ditModel, ditVariant }: ResolveDitModelPathOptions = {}
): string | undefined {
  if (ditModel) return ditModel
  if (!ditVariant) return undefined
  if (!modelDir) {
    throw new Error(
