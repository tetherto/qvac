...
// Usage (Query Rewriting: the LM formats your caption + lyrics into a
// detailed request before synthesis, preserving the lyric content):
//
//   bare examples/generate-rewrite.js "a short salsa idea"
//
// Env:
...
//   AUDIOGEN_CAPTION     caption to rewrite (overrides argv[2])
//   AUDIOGEN_LYRICS      lyrics to preserve (required by rewriteQuery; a
//                        default verse is used when unset)
...
//
// Faithful rewriting (lyrics back verbatim, caption on-genre) needs the 1.7B
// LM; the 0.6B drifts genre, voice, and language.
...
async function main() {
  const caption = process.env.AUDIOGEN_CAPTION || process.argv[2] || 'a short salsa idea'
  ...
  const outFileRaw = process.env.AUDIOGEN_OUT || 'audiogen-rewrite'
  ...
  console.log('[audiogen] caption to rewrite: ' + caption)
