...
test('hostPlatformPackage maps desktop hosts one to one', (t) => {
  t.is(hostPlatformPackage('linux-x64'), '@qvac/audiogen-ggml-linux-x64')
  t.is(hostPlatformPackage('linux-arm64'), '@qvac/audiogen-ggml-linux-arm64')
  t.is(hostPlatformPackage('darwin-arm64'), '@qvac/audiogen-ggml-darwin-arm64')
  t.is(hostPlatformPackage('darwin-x64'), '@qvac/audiogen-ggml-darwin-x64')
  t.is(hostPlatformPackage('win32-x64'), '@qvac/audiogen-ggml-win32-x64')
  t.is(hostPlatformPackage('android-arm64'), '@qvac/audiogen-ggml-android-arm64')
})

test('hostPlatformPackage groups every ios flavour into the ios package', (t) => {
  t.is(hostPlatformPackage('ios-arm64'), '@qvac/audiogen-ggml-ios')
  t.is(hostPlatformPackage('ios-arm64-simulator'), '@qvac/audiogen-ggml-ios')
  t.is(hostPlatformPackage('ios-x64-simulator'), '@qvac/audiogen-ggml-ios')
})
...
test('falls back to the installed platform package prebuilds', (t) => {
  const manifestPath = path.join('/fake/node_modules/@qvac/audiogen-ggml-linux-x64', 'package.json')
  const requested = []
  const dir = resolveBackendsDirFrom(
    sources({
  ...
  t.alike(requested, ['@qvac/audiogen-ggml-linux-x64/package'])
  t.is(dir, path.join('/fake/node_modules/@qvac/audiogen-ggml-linux-x64', 'addon', 'prebuilds'))
})
