...
const { linkOrCopySync } = require('../integration/_vla-model-download.cjs')
...
function mkTmpDir() {
  ...
  const dir = path.join(
    base,
    `qvac-vla-prestage-${Date.now()}-${Math.random().toString(36).slice(2)}`
  )
...
test('falls back to a byte copy when hardlinking is impossible (Android EXDEV)', function (t) {
  ...
  try {
    ...
    t.is(
      linkOrCopySync({ src, dest, platform: 'android', link: exdev() }),
      'copy',
      'EXDEV falls back to a copy'
    )
