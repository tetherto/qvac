...
if [ "$PRESTAGE_READY" = "1" ]; then
  ...
  if ! node -e "const fs=require('fs');const man=JSON.parse(fs.readFileSync('/tmp/prestage/model-manifest.json','utf8'));const g=(process.env.GREP||'').trim();let re=null;if(g){try{re=new RegExp(g)}catch(e){console.error('[prestage] WARN: invalid tests grep /'+g+'/ ('+e.message+'); staging nothing')}}else{console.error('[prestage] WARN: no shard grep resolved — staging nothing so the device downloads its own models')}const tests=re?Object.keys(man).filter(k=>re.test(k)):[];if(re&&!tests.length)console.error('[prestage] WARN: tests grep /'+g+'/ matched no known runner — staging nothing');const seen=new Set();const out=[];for(const t of tests){for(const m of (man[t]||[])){if(!seen.has(m.name)){seen.add(m.name);out.push(m.name+'\\t'+m.url)}}}fs.writeFileSync('/tmp/prestage/prestage-list.tsv',out.join('\\n')+(out.length?'\\n':''));console.error('[prestage] '+out.length+' model(s) for '+tests.length+' test(s)')"; then
