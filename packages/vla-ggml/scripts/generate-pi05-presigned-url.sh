...
# Generate a presigned URL for the pi05_base q_aggressive GGUF hosted in S3
# so the mobile test-addon-mobile app can download it at runtime on AWS Device
# Farm. Sibling of generate-smolvla-presigned-url.sh — same shape, different
# tensor name + bucket key + env-var prefix.

REGION="${AWS_REGION:-eu-central-1}"
BUCKET="${S3_BUCKET:-${MODEL_S3_BUCKET}}"
BASE_PATH="qvac_models_compiled/vla/pi05-base"
...
echo "🔑 Generating presigned URL for pi05_base q_aggressive model..."
echo "   Region: $REGION"
...
# The model is ~3-4GB, the mobile build+upload+schedule window can easily
