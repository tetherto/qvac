...
export function preprocessImage(
  ...
  if (
    ...
    throw new TypeError(
      "preprocessImage: width/height must be positive integers",
    );
  }
  const expected = width * height * 3;
  if (pixels.length !== expected) {
    throw new RangeError(
      `preprocessImage: expected ${expected} pixel values, got ${pixels.length}`,
    );
  }
...
export function padState(
  ...
  if (state.length > targetDim) {
    throw new RangeError(
      `padState: input length ${state.length} exceeds targetDim ${targetDim}`,
    );
  }
...
function detectScale(pixels: Float32Array | Uint8Array | number[]): number {
