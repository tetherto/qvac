...
function rejectIfUrlWithoutPublicBaseUrl(
...
function assertResponseFormat(raw: unknown): string {
  ...
  if (!SUPPORTED_RESPONSE_FORMATS.has(responseFormat)) {
    throw new HttpError(
      ...
      `Unknown response_format "${responseFormat}". Use "b64_json" or "url".`
    )
  }
...
function mapImageParamError(err: unknown): never {
  if (err instanceof InvalidImagePromptError) {
    throw new HttpError(400, 'missing_prompt', err.message)
  }
...
async function runAndRespond(
  ...
  const drainProgress = async (): Promise<void> => {
    try {
      for await (const tick of result.progressStream) {
        ctx.logger.debug?.(
          `    diffusion step=${tick.step}/${tick.totalSteps} elapsed=${tick.elapsedMs}ms`
        )
      }
    } catch {
  ...
  if (stats !== undefined && stats.seed !== undefined) {
    ctx.logger.info(
      `  ${logLabel} done images=${buffers.length} seed=${stats.seed} ms=${stats.totalGenerationMs ?? stats.totalWallMs ?? 0}`
    )
  } else {
    ctx.logger.info(
      `  ${logLabel} done images=${buffers.length} ms=${stats?.totalGenerationMs ?? stats?.totalWallMs ?? 0}`
    )
  }
...
const plugin: FastifyPluginAsyncZod = async (app) => {
  app.post(
    ...
    async (req, reply) => {
      ...
      try {
        params = extractImageGenerationParams(
          body as Record<string, unknown>,
          req.qvacModel!.sdkModelId
        )
      } catch (err) {
        mapImageParamError(err)
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      const body = req.body
      const imageFiles = (req.multipartFiles ?? []).filter((f) =>
        EDIT_IMAGE_FIELD_NAMES.has(f.fieldname)
      )
