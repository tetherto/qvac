...
| GPU acceleration (Metal on macOS, Vulkan on Linux/Windows) | QVAC inference backends use Metal (always present on macOS) or Vulkan (`vulkaninfo --summary`) on Linux and Windows, and Vulkan must be version 1.4 or newer. On macOS and Linux, inference runs on CPU when a Vulkan GPU backend is not available, just slower. On Windows, the addon currently loads Vulkan even for CPU inference, so a missing or pre-1.4 Vulkan runtime causes a load failure rather than a CPU fallback; install a GPU driver that provides Vulkan 1.4 or newer. |
