...
| CLI in-process HTTP e2e       | `packages/cli/test/e2e/{core,default,openai}`                         | Modelless wire-level validation through `useServer` / `app.inject`, including status codes, error codes, routing, CORS, multipart parsing, and per-surface endpoint validation.                                  |
...
Add CLI TypeScript tests under `packages/cli/test/unit` when the behavior can be exercised deterministically without loading a real model or constructing a Fastify request. This is the right home for adapter/helper contracts, content-parts arrays, chat or Responses API payload translation, JSON response shapes, and SSE chunk parsing. Mirror the source layout: `unit/core` for the shape-agnostic server, `unit/extensions/<name>` for one API surface, `unit/commands` for a CLI command group.

Use `packages/cli/test/e2e/{core,default,openai}` with `useServer` / `app.inject` when the behavior needs wire-level request handling but can stay modelless. Each surface owns the directory named after its extension. This is the right home for endpoint validation, status codes, error codes, routing, CORS, multipart parsing, and HTTP behavior that `unit/core/server.test.ts` delegates to e2e coverage.
