...
| CLI node:test e2e | `packages/cli/test/e2e` | Built CLI startup, real `qvac serve openai` process smoke, model-load checks, network-bound endpoint smoke, logs, diagnostics, and process cleanup. |
...
- Use CLI node:test e2e for process boundaries: the built CLI, real model startup, network reachability, logs, and cleanup.
