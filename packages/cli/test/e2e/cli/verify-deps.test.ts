...
describe('cli: verify deps (real diff)', () => {
  it('reports no native changes (exit 0) when no native package changed', async (t) => {
    ...
    const r = await runCli(
      ['verify', 'deps', '--base', base, '--head', head, '--lockfile', 'package-lock.json'],
      { cwd: dir }
    )
  ...
  it('detects an added native package (exit 1) and names it', async (t) => {
    ...
    await writeFile(
      join(dir, 'node_modules', 'native-foo', 'package.json'),
      JSON.stringify({ name: 'native-foo', version: '1.0.0', addon: true })
    )
    ...
    await writeFile(
      ...
      lock({ 'node_modules/native-foo': { version: '1.0.0' } })
    )
    git(dir, ['add', 'package-lock.json'])
    git(dir, ['commit', '-q', '-m', 'head'])
    const head = headSha(dir)
