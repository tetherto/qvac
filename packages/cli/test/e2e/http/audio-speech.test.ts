...
describe('serve: speech validation', () => {
  ...
  it('missing model returns 400', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { input: 'hello', voice: 'alloy' }
    })
  ...
  it('missing input returns 400', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'test', voice: 'alloy' }
    })
  ...
  it('empty input returns 400', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'test', voice: 'alloy', input: '   ' }
    })
    assertStatusAndError(res, 400, 'missing_input')
  })
  ...
  let ffmpeg = false
  before(async () => {
    ffmpeg = await probeFfmpegAvailable()
  })
  ...
  for (const fmt of ['mp3', 'opus', 'aac', 'flac']) {
    it(`${fmt} format: transcode_unavailable without ffmpeg, model_not_found with ffmpeg`, async () => {
      const res = await server().inject({
        method: 'POST',
        url: '/v1/audio/speech',
        payload: { model: 'test', voice: 'alloy', input: 'hi', response_format: fmt }
      })
  ...
  it('unknown response_format returns 400 invalid_response_format', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'test', voice: 'alloy', input: 'hi', response_format: 'mp4' }
    })
  ...
  it('input over default 4096-char cap returns 400 input_too_long', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'test', voice: 'alloy', input: 'a'.repeat(4097) }
    })
  ...
  it('unknown model returns 404', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'nonexistent', voice: 'alloy', input: 'hi' }
    })
...
describe('serve: speech auth', () => {
  ...
  it('auth required when api-key set', async () => {
    const res = await server().inject({
      method: 'POST',
      url: '/v1/audio/speech',
      payload: { model: 'test', voice: 'alloy', input: 'hi' }
    })
