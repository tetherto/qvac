...
const wavField = { name: 'file', filename: 'silence.wav', contentType: 'audio/wav', data: silenceWav() }
...
describe('models', () => {
  it('GET /v1/models lists all 4 loaded models', async () => {
    ...
    assert.deepEqual([...body.data.map((m) => m.id)].sort(), ['test-embed', 'test-llm', 'test-whisper', 'test-whisper-translate'])
...
describe('chat completions (blocking)', () => {
  it('blocking completion returns valid response', async () => {
    const res = await post('/v1/chat/completions', { model: E2E.llm, messages: [{ role: 'user', content: 'Say hello and nothing else.' }], max_tokens: 512 })
  ...
  it('respects max_completion_tokens', async () => {
    const res = await post('/v1/chat/completions', { model: E2E.llm, messages: [{ role: 'user', content: 'Write a very long story about a cat.' }], max_completion_tokens: 8 })
    const body = res.json() as any
    assert.ok(body.choices[0].message.content.length > 0)
  })

  it('finish_reason=length when max_tokens exceeded (blocking)', async () => {
    const res = await post('/v1/chat/completions', { model: E2E.llm, messages: [{ role: 'user', content: 'Count from 1 to 100.' }], max_tokens: 1 })
    const body = res.json() as any
...
describe('chat completions (streaming)', () => {
  it('finish_reason=length when max_tokens exceeded (streaming)', async () => {
    const res = await post('/v1/chat/completions', { model: E2E.llm, messages: [{ role: 'user', content: 'Count from 1 to 100.' }], stream: true, max_tokens: 1 })
    const chunks = collectSSE(res.payload).map((e) => e.data).filter((d) => d !== '[DONE]') as any[]
  ...
  it('SSE stream returns valid chunks', async () => {
    const res = await post('/v1/chat/completions', { model: E2E.llm, messages: [{ role: 'user', content: 'Say hi.' }], stream: true, max_tokens: 512 })
...
function assertPlainText (payload: string): void {
  let parsed: unknown
  let threw = false
  try { parsed = JSON.parse(payload) } catch { threw = true }
  assert.ok(threw || typeof parsed === 'string', `expected plain text, got: ${payload.slice(0, 80)}`)
}

describe('transcriptions', () => {
  it('returns JSON with text field', async () => {
    const res = await server().inject({ method: 'POST', url: '/v1/audio/transcriptions', ...multipart([{ name: 'model', value: E2E.whisper }, wavField]) })
    assert.equal(typeof (res.json() as { text: unknown }).text, 'string')
  })

  it('response_format=text returns plain text', async () => {
    const res = await server().inject({ method: 'POST', url: '/v1/audio/transcriptions', ...multipart([{ name: 'model', value: E2E.whisper }, { name: 'response_format', value: 'text' }, wavField]) })
...
describe('translations', () => {
  it('returns JSON with text field', async () => {
    const res = await server().inject({ method: 'POST', url: '/v1/audio/translations', ...multipart([{ name: 'model', value: E2E.whisperTranslate }, wavField]) })
    assert.equal(typeof (res.json() as { text: unknown }).text, 'string')
  })

  it('response_format=text returns plain text', async () => {
    const res = await server().inject({ method: 'POST', url: '/v1/audio/translations', ...multipart([{ name: 'model', value: E2E.whisperTranslate }, { name: 'response_format', value: 'text' }, wavField]) })
    assertPlainText(res.payload)
  })

  it('rejects transcription-only alias', async () => {
    const res = await server().inject({ method: 'POST', url: '/v1/audio/translations', ...multipart([{ name: 'model', value: E2E.whisper }, wavField]) })
...
describe('vector stores', () => {
  it('CRUD — create, list, get, update, delete', async () => {
    const create = (await post('/v1/vector_stores', { name: 'crud', metadata: { by: 'e2e' } })).json() as any
    ...
    const del = (await server().inject({ method: 'DELETE', url: `/v1/vector_stores/${id}` })).json() as any
  ...
  it('upload → attach → search end-to-end', async () => {
    const doc = textFile('Local e2e document about planets, moons and the solar system.\nAnother note about OpenAI vector stores and RAG.\n')
    const vs = ((await post('/v1/vector_stores', { name: 'flow' })).json() as any).id

    const upload = (await server().inject({ method: 'POST', url: '/v1/files', ...multipart([{ name: 'file', filename: 'vs_doc.txt', contentType: 'text/plain', data: doc }, { name: 'purpose', value: 'assistants' }]) })).json() as any
    ...
    const search = (await post(`/v1/vector_stores/${vs}/search`, { query: 'planets and solar system', max_num_results: 5 })).json() as any
    assert.equal(search.object, 'vector_store.search_results.page')
    assert.ok(Array.isArray(search.data) && search.data.length > 0)
    const joined = search.data.map((hit: any) => (hit.content ?? []).filter((c: any) => c.type === 'text').map((c: any) => c.text ?? '').join(' ')).join(' ')
  ...
  it('attach with unknown file_id returns 404 file_not_found', async () => {
    const vs = ((await post('/v1/vector_stores', {})).json() as any).id
    assertError(await post(`/v1/vector_stores/${vs}/files`, { file_id: 'file-doesnotexist' }), 'file_not_found')
    await server().inject({ method: 'DELETE', url: `/v1/vector_stores/${vs}` })
  })
  ...
  it('attach binary upload returns 400 unsupported_file_type', async () => {
    ...
    const upload = (await server().inject({ method: 'POST', url: '/v1/files', ...multipart([{ name: 'file', filename: 'bin.png', contentType: 'image/png', data: tinyPng() }, { name: 'purpose', value: 'assistants' }]) })).json() as any
    assertError(await post(`/v1/vector_stores/${vs}/files`, { file_id: upload.id }), 'unsupported_file_type')
    await server().inject({ method: 'DELETE', url: `/v1/vector_stores/${vs}` })
  })
...
describe('responses API', () => {
  ...
  it('streaming returns response.completed and stub header (no [DONE])', async () => {
    const res = await post('/v1/responses', { model: E2E.llm, input: 'Say hi.', stream: true, max_output_tokens: 512 })
    assert.match(String(res.headers['x-qvac-stub']), /responses-volatile/i)
  ...
  it('store retrieve delete and input_items', async () => {
    const rid = ((await post('/v1/responses', { model: E2E.llm, input: 'ping', store: true })).json() as any).id
    ...
    const del = (await server().inject({ method: 'DELETE', url: `/v1/responses/${rid}` })).json() as any
    assert.equal(del.deleted, true)
  ...
  it('previous_response_id chains context', async () => {
    const rid = ((await post('/v1/responses', { model: E2E.llm, input: 'Remember the code word is XYZZY.', store: true, max_output_tokens: 512, temperature: 0, seed: 1 })).json() as any).id
    const body2 = (await post('/v1/responses', { model: E2E.llm, previous_response_id: rid, input: 'What is the code word? Reply with one word only.', max_output_tokens: 512, temperature: 0, seed: 1 })).json() as any
    assert.match(body2.output_text, /XYZZY/i)
  })

  it('previous_response_id walks deeper than one step (chain depth 3)', async () => {
    const rid1 = ((await post('/v1/responses', { model: E2E.llm, input: 'Remember the code word is XYZZY.', store: true, max_output_tokens: 512, temperature: 0, seed: 1 })).json() as any).id
    const rid2 = ((await post('/v1/responses', { model: E2E.llm, previous_response_id: rid1, input: 'Got it.', store: true, max_output_tokens: 256, temperature: 0, seed: 1 })).json() as any).id
    const body3 = (await post('/v1/responses', { model: E2E.llm, previous_response_id: rid2, input: 'What is the code word? Reply with one word only.', max_output_tokens: 512, temperature: 0, seed: 1 })).json() as any
    assert.match(body3.output_text, /XYZZY/i)
  })

  it('bogus previous_response_id returns 404', async () => {
    assertError(await post('/v1/responses', { model: E2E.llm, previous_response_id: 'resp_nonexistent123', input: 'hi' }), 'previous_response_not_found')
  })

  it('rejects conversation id', async () => {
    assertError(await post('/v1/responses', { model: E2E.llm, conversation: 'conv_1', input: 'hi' }), 'conversation_not_supported')
  })

  it('rejects background mode', async () => {
    assertError(await post('/v1/responses', { model: E2E.llm, background: true, input: 'hi' }), 'background_not_supported')
  })

  it('rejects built-in web_search tool', async () => {
    assertError(await post('/v1/responses', { model: E2E.llm, input: 'hi', tools: [{ type: 'web_search' }] }), 'invalid_tool_type')
  })
})

describe('legacy completions', () => {
  it('blocking returns text_completion shape', async () => {
    const body = (await post('/v1/completions', { model: E2E.llm, prompt: 'Say hello and nothing else.', max_tokens: 4096 })).json() as any
  ...
  it('respects max_tokens', async () => {
    const body = (await post('/v1/completions', { model: E2E.llm, prompt: 'Write a very long story about a cat.', max_tokens: 8 })).json() as any
    assert.ok(body.choices[0].text.length > 0)
  })
  ...
  it('multi-prompt with stream:true returns 400 unsupported_streaming', async () => {
    assertError(await post('/v1/completions', { model: E2E.llm, prompt: ['a', 'b'], stream: true, max_tokens: 4 }), 'unsupported_streaming')
  })

  it('rejects token-id prompts', async () => {
    assertError(await post('/v1/completions', { model: E2E.llm, prompt: [15496, 11, 995], max_tokens: 4 }), 'invalid_prompt')
  })
  ...
  it('SSE stream returns valid text_completion chunks', async () => {
    const res = await post('/v1/completions', { model: E2E.llm, prompt: 'Say hi.', stream: true, max_tokens: 512 })
    const datas = collectSSE(res.payload).map((e) => e.data)
...
describe('cross-type model rejection', () => {
  it('chat endpoint rejects embedding model', async () => {
    assertError(await post('/v1/chat/completions', { model: E2E.embed, messages: [{ role: 'user', content: 'hi' }] }), 'invalid_model_type')
  })
  it('legacy completions endpoint rejects embedding model', async () => {
    assertError(await post('/v1/completions', { model: E2E.embed, prompt: 'hi' }), 'invalid_model_type')
  })
  it('embedding endpoint rejects chat model', async () => {
    assertError(await post('/v1/embeddings', { model: E2E.llm, input: 'hello' }), 'invalid_model_type')
  })
  it('transcription endpoint rejects chat model', async () => {
    assertError(await server().inject({ method: 'POST', url: '/v1/audio/transcriptions', ...multipart([{ name: 'model', value: E2E.llm }, wavField]) }), 'invalid_model_type')
  })
  it('translations endpoint rejects chat model', async () => {
    assertError(await server().inject({ method: 'POST', url: '/v1/audio/translations', ...multipart([{ name: 'model', value: E2E.llm }, wavField]) }), 'invalid_model_type')
  })
  it('responses endpoint rejects embedding model', async () => {
    assertError(await post('/v1/responses', { model: E2E.embed, input: 'hello' }), 'invalid_model_type')
  })
})
...
describe('videos (HTTP layer only; test-video preload:false)', () => {
  it('JSON txt2vid reaches model check (503 model_not_ready)', async () => {
    assertError(await post('/v1/videos', { model: E2E.video, prompt: 'a bird flies' }), 'model_not_ready')
  })
  it('JSON img2vid with data URI reaches model check (503 model_not_ready)', async () => {
    assertError(await post('/v1/videos', { model: E2E.video, prompt: 'subject turns', input_reference: { image_url: TINY_PNG_DATA_URI } }), 'model_not_ready')
  })
  it('JSON img2vid with HTTP URL reaches model check (503 model_not_ready)', async () => {
    assertError(await post('/v1/videos', { model: E2E.video, prompt: 'subject turns', input_reference: { image_url: 'http://127.0.0.1:1/v1/models' } }), 'model_not_ready')
  })
  it('input_reference with wrong shape returns 400 invalid_request', async () => {
    assertError(await post('/v1/videos', { model: E2E.video, prompt: 'p', input_reference: { not_image_url: 'x' } }), 'invalid_request')
  })
