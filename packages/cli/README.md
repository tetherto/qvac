...
**The committed state is intentionally transitional.** Until `@qvac/sdk@0.12.0`
ships on npm with the `./commands` subpath, `dependencies["@qvac/sdk"]` is
pinned to `file:../sdk`. The `preinstall` hook builds that sibling so the
symlink resolves a populated `dist/`, and `prepublishOnly`
(`scripts/check-publish-ready.cjs`) blocks `npm publish` while this wiring is
still in place. Don't reintroduce the `file:` URL after the pre-publish flip.

**Iterating on an alternate local `@qvac/sdk` build** (e.g. one checked out in
a separate worktree):

```bash
# Build SDK first (from packages/sdk in the alternate checkout)
cd /path/to/other/qvac/packages/sdk && bun run build

# Point the CLI at the canonical sibling SDK (../sdk)
cd packages/cli
npm run dev:link
...
canonical `file:../sdk` wiring is restored regardless of what you swapped in
locally. `package-lock.json` is gitignored and is regenerated by the trailing
`npm install`.
