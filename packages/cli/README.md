...
Setting `includeAudioDecoder: false` requires the inference runtime used by the
bundled SDK to support lazy audio decoding. The bundle command rejects this option
on older runtimes, which import the decoder during startup even for raw-only apps.
Omit the option to keep those installations working with the decoder included.
