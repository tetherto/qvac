...
export const embedRequestSchema = embedParamsSchema.extend({
  ...
  requestId: z
    ...
    .optional()
    .describe(
      'Stable identifier for this in-flight embed, generated by the caller at call time. Surfaces on the registry so callers can target it with `cancel({ requestId })`. Optional — core falls back to a generated id when the field is missing.'
    )
})
