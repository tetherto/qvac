...
import { SDK_ALL_LOG_ID, SDK_LOG_ID } from '../src/logging'
import {
...
test('global stream preserves the originating source id per log', (t) => {
  ...
  t.alike(sources, [SDK_LOG_ID, 'model-123'], 'global subscriber sees real origin id, not __all__')
...
test('buffered global logs preserve their source id on flush', (t) => {
  ...
  const sources: string[] = []
  registerLoggingStream(SDK_ALL_LOG_ID, (_l, _n, _m, sourceId) => sources.push(sourceId))
