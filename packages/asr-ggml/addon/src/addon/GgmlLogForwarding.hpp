...
// whisper.cpp / parakeet-cpp / ggml native-log forwarding.
//
...
// process via installNativeLogForwarderOnce() (ggml_log_set +
...
// Forward a single whisper.cpp / parakeet-cpp / ggml log line into the addon
// logger (QLOG -> JS logger), preserving its verbosity. Each callback
// invocation is surfaced immediately, with no cross-call buffering --
// mirroring llm-llamacpp's LlamaModel::llamaLogCallback. This is
// deterministic regardless of newline termination: whisper.cpp / ggml have
// error paths that emit a message WITHOUT a trailing '\n', so a
// newline-gated buffer could hold such a message pending indefinitely, or
// prepend it to a later unrelated message at the wrong priority. Per-call
// forwarding avoids that and keeps no shared mutable state (no unbounded
// buffer growth, no partial line lost at shutdown). Trailing newlines/CR are
// trimmed so the JS logger gets a clean line. JsLogger::log() is
// thread-safe, so this is safe on ggml's worker threads; it must never
