...
WhisperLoadSelection
resolveWhisperLoadSelection(bool useGpu, int gpuDevice, bool hasLegacyGpuDevice,
  ...
  WhisperLoadSelection resolved{selected.whisperIndex >= 0, gpuDevice,
                                selected.outOfRange, false, selected.refused};
  if (resolved.useGpu) {
...
WhisperLoadSelection resolveWhisperLoadSelection(bool useGpu, int gpuDevice,
                                                 bool hasLegacyGpuDevice,
                                                 const ConfigMap &config) {
  return resolveWhisperLoadSelection(useGpu, gpuDevice, hasLegacyGpuDevice,
                                     config, GgmlRegistry{});
}
