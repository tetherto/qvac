...
ParakeetStreamingProcessor::~ParakeetStreamingProcessor() { cancel(); }
...
void ParakeetStreamingProcessor::processLoop() {
  while (true) {
    ...
    bool aborted = false;
    {
      ...
      cv_.wait(
          lk, [this] { return cancelled_ || ended_ || !pending_.empty(); });
      if (cancelled_) {
        aborted = true;
      } else {
        if (!pending_.empty())
    ...
    if (!work.empty()) {
      try {
        ...
        audio_seconds_ += static_cast<double>(work.size()) /
    ...
    if (finalising) {
      try {
        ...
        QLOG(
            logger::Priority::WARNING,
            std::string("ParakeetStreamingProcessor: finalize failed: ") +
                e.what());
      }
      emitPending();
      break;
