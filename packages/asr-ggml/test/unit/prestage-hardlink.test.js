...
const { linkOrCopySync: whisperLinkOrCopy } = require('../integration/helpers.js')
const { linkOrCopySync: parakeetLinkOrCopy } = require('../integration/parakeet-helpers.js')

const CONTENT = 'qvac-prestage-fixture-content-0123456789'

const PICKUPS = [
  ['whisper', whisperLinkOrCopy],
  ['parakeet', parakeetLinkOrCopy]
]
...
function mkTmpDir() {
  ...
  const dir = path.join(
    base,
    `qvac-asr-prestage-${Date.now()}-${Math.random().toString(36).slice(2)}`
  )
...
for (const [name, linkOrCopySync] of PICKUPS) {
  test(`${name}: staged model is hardlinked, not copied, when it can be`, function (t) {
  ...
  test(`${name}: an existing destination is replaced rather than failing EEXIST`, function (t) {
    const stagedDir = mkTmpDir()
    const modelDir = mkTmpDir()
    const src = path.join(stagedDir, 'model.bin')
    const dest = path.join(modelDir, 'model.bin')
  ...
  test(`${name}: falls back to a byte copy when hardlinking is impossible (EXDEV)`, function (t) {
    const stagedDir = mkTmpDir()
    const modelDir = mkTmpDir()
    const src = path.join(stagedDir, 'model.bin')
    const dest = path.join(modelDir, 'model.bin')
