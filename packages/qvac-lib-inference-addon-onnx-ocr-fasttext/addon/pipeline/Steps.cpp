...
Ort::SessionOptions getOrtSessionOptions(bool useGPU) {
  const std::string initMsg = "[ORT] getOrtSessionOptions called with useGPU=" + std::to_string(useGPU);
  ...
  QLOG(qvac_lib_inference_addon_cpp::logger::Priority::DEBUG, providersMsg.str());
  ...
  if (!useGPU) {
    ...
    try {
      const bool xnnpackAvailable = std::find(providers.begin(), providers.end(), "XnnpackExecutionProvider") != providers.end();
      if (xnnpackAvailable) {
        // Order must be: spinning -> intra -> inter -> AppendEP.
        // Also see: https://onnxruntime.ai/docs/execution-providers/Xnnpack-ExecutionProvider.html
        int availableThreads = std::max(1, static_cast<int>(std::thread::hardware_concurrency()));

        sessionOptions.SetExecutionMode(ExecutionMode::ORT_PARALLEL);
        sessionOptions.AddConfigEntry(kOrtSessionOptionsConfigAllowIntraOpSpinning, "1");
        sessionOptions.AddConfigEntry(kOrtSessionOptionsConfigAllowInterOpSpinning, "1");
        sessionOptions.SetIntraOpNumThreads(availableThreads);
        sessionOptions.SetInterOpNumThreads(availableThreads);

#ifdef __ANDROID__
        // Cap threads on Android: avoid thermal throttling and big.LITTLE contention.
        availableThreads = std::min(availableThreads, 4);
#endif
        sessionOptions.AppendExecutionProvider("XNNPACK", {{"intra_op_num_threads", std::to_string(availableThreads)}});
  ...
  try {
    ...
    const std::string errMsg = std::string("Error setting up NNAPI provider: ") + e.what();
