...
safeTest(
  'Two embed instances can run inference simultaneously',
  {
    timeout: 900_000
  },
  async (t) => {
    ...
    t.ok(
      embeddings1[0].length === sentences1.length,
      'first instance produced correct number of embeddings'
    )
    t.ok(
      embeddings2[0].length === sentences2.length,
      'second instance produced correct number of embeddings'
    )
...
safeTest(
  'Repeated embed load/unload cycles should remain stable',
  {
    timeout: 900_000
  },
  async (t) => {
...
safeTest(
  'Unloading one embed instance does not affect another running instance',
  {
    timeout: 900_000
  },
  async (t) => {
    ...
    const largeBatch = Array(50)
      .fill(null)
      .map((_, i) => `Test sentence number ${i} for batch processing`)
    ...
    t.ok(
      embeddings1[0].length === largeBatch.length,
      'instance 1 completed processing after instance 2 was unloaded'
    )
  }
)

safeTest(
  'Multiple embed load/unload cycles on one instance while another processes',
  {
    timeout: 900_000
  },
  async (t) => {
    ...
    for (let batch = 0; batch < NUM_BATCHES; batch++) {
      const sentences = Array(10)
        .fill(null)
        .map((_, i) => `Batch ${batch} sentence ${i}`)
      ...
      if (cyclesCompleted < NUM_CYCLES) {
        ...
        cyclesCompleted++
        t.pass(
          `load/unload cycle ${cyclesCompleted} completed while instance 1 processes batch ${batch + 1}`
        )
      }
      ...
      const embeddings1 = await waitForCompletion(response1)
      t.ok(
        embeddings1[0].length === sentences.length,
        `batch ${batch + 1} completed with correct count`
      )
    }

    t.ok(
      cyclesCompleted === NUM_CYCLES,
      `completed ${cyclesCompleted} load/unload cycles during processing`
    )
  }
)
