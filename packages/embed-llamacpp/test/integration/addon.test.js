...
createDeviceModelTest(
  'Model inference works correctly',
  async (t, modelName, modelConfig, device) => {
    ...
    t.ok(
      ...
      'Should generate embeddings with correct dimension'
    )
...
createDeviceModelTest(
  'Model inference works correctly with array input',
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension

    console.log(
      `Creating new GGMLBert instance for array input test [${modelName}] on ${device.toUpperCase()}`
    )
    const { inference } = await createEmbeddingsTestInstance(
      t,
      modelName,
    ...
    t.ok(
      embeddings[0].length === sentences.length,
      `Should generate ${sentences.length} embeddings`
    )
    ...
    for (let i = 0; i < embeddings[0].length; i++) {
      t.ok(
        embeddings[0][i].length === embeddingDimension,
        `Embedding ${i} should have dimension ${embeddingDimension}`
      )
      t.ok(
    ...
    if (embeddings[0].length > 1) {
      ...
      t.ok(
        similarity < 0.999,
        `Different sentences should produce different embeddings (cosine similarity: ${similarity.toFixed(6)})`
      )
    }
...
createDeviceModelTest(
  'Model inference works correctly with long string exceeding context size',
  async (t, modelName, modelConfig, device) => {
    ...
    t.ok(
      errorMessage.includes('exceeds effective context size'),
      'Error should mention exceeding context size'
    )
    // Verify the error contains token count information
    t.ok(
      errorMessage.includes(String(maxContextSize)),
      `Error should mention context size limit (${maxContextSize})`
    )
...
createDeviceModelTest(
  'Model inference works correctly with array input where one sequence exceeds context size',
  async (t, modelName, modelConfig, device) => {
    const maxContextSize = modelConfig.maxContextSize
    ...
    t.ok(
      hasEncodeError || hasTokenizeError,
      'Error should mention encodeHostF32Sequences or tokenizeInput'
    )
    t.ok(
      errorMessage.includes('exceeds effective context size'),
      'Error should mention exceeding context size'
    ...
    t.ok(
      /sequence\s+1|prompt\s+0/.test(errorMessage),
      'Error should mention which sequence is failing'
    )
...
createDeviceModelTest(
  'Model inference works correctly with batching - 5 sequences split into 2 batches',
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
    const maxContextSize = modelConfig.maxContextSize

    console.log(
      `Creating new GGMLBert instance for batching test [${modelName}] on ${device.toUpperCase()}`
    )
    const { inference } = await createEmbeddingsTestInstance(
      t,
      modelName,
    ...
    t.ok(
      embeddings[0].length === sequences.length,
      `Should generate ${sequences.length} embeddings (one per sequence)`
    )
    console.log(`Generated ${embeddings[0].length} embeddings`)
    ...
    if (embeddings[0].length > 1) {
      ...
      t.ok(
        similarity < 0.999,
        `Different sequences should produce different embeddings (cosine similarity: ${similarity.toFixed(6)})`
      )
    }
...
createDeviceModelTest(
  'Embeddings: whitespace-only string input',
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
...
createDeviceModelTest(
  'Embeddings: unicode / multilingual input with emojis',
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
    ...
    t.ok(
      embeddings[0].length === sentences.length,
      'Should generate one embedding per multilingual sentence'
    )
...
createDeviceModelTest(
  'Embeddings: deterministic output for same input',
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
    ...
    t.ok(
      maxDiff <= floatDiffEps,
      'Same input should produce identical embeddings (deterministic behaviour)'
    )

    const similarity = cosineSimilarity(emb1, emb2)
    t.ok(
      similarity > 0.999,
      `Same input should produce identical embeddings (cosine similarity: ${similarity.toFixed(6)})`
    )
...
createDeviceModelTest(
  `Stress: inference with large batch size ${STRESS_BATCH_SIZE}`,
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
...
createDeviceModelTest(
  `Stress: inference with many sequences (~${STRESS_NUM_SEQUENCES})`,
  async (t, modelName, modelConfig, device) => {
    const embeddingDimension = modelConfig.embeddingDimension
    ...
    if (embeddings[0].length > 1) {
      ...
      t.ok(
        similarity < 0.999,
        `Different stress test sequences should produce different embeddings (cosine similarity: ${similarity.toFixed(6)})`
      )
    }
...
createDeviceModelTest(
  'Cancel: immediate cancel returns fewer embeddings than full run',
  async (t, modelName, modelConfig, device) => {
    console.log(
      `Creating GGMLBert instance for cancel comparison test [${modelName}] on ${device.toUpperCase()}`
    )
...
safeTest('idle | run: allowed, returns QvacResponse', { timeout: TEST_TIMEOUT }, async (t) => {
  ...
  t.ok(
    typeof response.await === 'function' || response._finishPromise != null,
    'response has await or _finishPromise'
  )
  const embeddings = await waitForCompletion(response)
...
safeTest(
  'run | response.cancel(): equivalent to model.cancel(), resolves when job stopped',
  { timeout: TEST_TIMEOUT },
  async (t) => {
    const { inference } = await setupModelApiBehavior(t)
    const sequences = Array.from(
      { length: 24 },
      (_, i) => `Sequence ${i} for response.cancel test.`
    )
    const response = await inference.run(sequences)
    const cancelPromise =
      typeof response.cancel === 'function' ? response.cancel() : inference.cancel()
    try {
      await waitForCompletion(response)
    } catch (err) {
      if (!/cancel|aborted|stopp?ed|Failed/i.test(extractErrorMessage(err))) throw err
