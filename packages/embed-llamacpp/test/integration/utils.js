...
// Verifies a cached/downloaded model against pinned sha256/bytes. Byte length
// is checked first (cheap) so a truncated file fails fast before hashing.
...
/**
 * Ensures the model file exists, downloading it if necessary. Model URL +
 * sha256/bytes integrity come from models.manifest.json (single source of
 * truth); MODEL_CONFIGS.downloadUrl is honoured as a fallback.
...
async function ensureModel (modelName) {
  ...
  const urls = entry && Array.isArray(entry.urls) && entry.urls.length ? entry.urls : modelConfig.downloadUrl
  ...
  if (fs.existsSync(modelPath)) {
    if (hasIntegrity) {
      ...
      if (stat.size > 0) {
        if (entry) console.log(`[download] ${modelName}: no sha256/bytes pinned in manifest — integrity check SKIPPED`)
        return [modelName, modelDir]
      }
