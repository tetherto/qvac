...
- The llama/ggml runtime, its compute backends and the `ggml_vec_index_*` vector-index API are now provided by the `@qvac/fabric` npm dependency (`^0.12.0`) rather than the static `qvac-fabric` vcpkg port. The addon no longer bundles them; on desktop it resolves the single `@qvac/fabric` install and loads the backend modules from `node_modules/@qvac/fabric/prebuilds/<host>/qvac__fabric/`, falling back to this addon's own `prebuilds/` on mobile (where the package tree isn't resolvable from the packed worklet bundle). `@qvac/fabric` carries the prebuilt runtime inside its own tarball, so run `npm install` before `bare-make generate`/`build` and do not prune the dependency at runtime.
