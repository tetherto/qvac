 ...
 */
function buildRunnerEnv(processEnv, projectRoot) {
  return {
    ...processEnv,
 ...
 * the tests-only configure lands. A `--build-dir <dir>` (or `--build-dir=<dir>`)
 * flag wins over the CPP_BUILD_DIR env var, which wins over the default.
 */
function resolveBuildDir(argv, processEnv) {
  ...
  if (idx !== -1) {
    ...
    if (next === undefined || next.startsWith('--')) {
      throw new Error('--build-dir requires a directory (e.g. --build-dir build-coverage)')
    }
    return next
  }
...
function main() {
  ...
  const cwd = path.resolve(projectRoot, buildDir, 'test', 'unit')

  const result = spawnSync(binary, ['--gtest_output=xml:cpp-test-results.xml'], {
    cwd,
    stdio: 'inherit',
    shell: false,
    env: buildRunnerEnv(process.env, projectRoot)
  })
