...
const workflowFile = path.resolve(
  ...
  '.github',
  'workflows',
  'benchmark-perf-embed-llamacpp.yml'
)
...
function checkManifest() {
  ...
  for (const model of manifest.models || []) {
    const quants =
      model.gguf && Array.isArray(model.gguf.quantizations) ? model.gguf.quantizations : []
  ...
  const fromMatrix = [
    ...new Set(matrix().map((c) => `${c.model}|${c.quant}|${c.repo}|${c.revision}`))
  ]
  const notInManifest = fromMatrix.filter((c) => !manifestSet.has(c))
  if (notInManifest.length) {
    console.error(
      'MISMATCH: mobile matrix cells are not in models.manifest.json (mobile must be a subset of the manifest):'
    )
