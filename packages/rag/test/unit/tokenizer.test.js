...
test('Tokenizer: should detect Unicode property support correctly', (t) => {
  ...
  t.ok(
    ...
    'Should successfully estimate when detecting Unicode support'
  )
})
...
test('Tokenizer: should produce consistent results across runs', (t) => {
  ...
  for (let i = 1; i < results.length; i++) {
    ...
    for (let j = 0; j < results[i].tokens.length; j++) {
      t.is(
        results[i].tokens[j].text,
        results[0].tokens[j].text,
        `Run ${i} token ${j} text should match`
      )
      t.is(
        results[i].tokens[j].count,
        results[0].tokens[j].count,
        `Run ${i} token ${j} count should match`
      )
    }
  }
})
