...
struct smolvla_hparams {
    ...
    int tokens_per_image() const {
        return patches_per_image() / (connector_scale_factor * connector_scale_factor); // 64
    }
    int connector_in_features() const {
        return vision_hidden_size * connector_scale_factor * connector_scale_factor; // 12288
    }
};
...
struct ggml_tensor * build_smollm2_graph(
    struct ggml_context * ctx,
    smolvla_model & model,
    struct ggml_tensor * prefix_embeddings,
...
struct ggml_tensor * build_denoise_step_graph(
    ...
    struct ggml_tensor * time_embed,
    struct ggml_tensor ** vlm_kv_keys,
    struct ggml_tensor ** vlm_kv_vals,
    struct ggml_tensor * position_ids,
    struct ggml_tensor * cross_pos_ids,
...
bool smolvla_run(
    ...
    const float * noise,  // ODE initial noise (chunk_size*max_action_dim), NULL=random
