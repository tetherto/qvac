...
static struct ggml_tensor* build_gqa_attention(
    ...
    struct ggml_tensor* v, struct ggml_tensor* position_ids, int num_heads,
  ...
  struct ggml_tensor* attn_out =
      ggml_mul_mat(ctx, ggml_cont(ctx, ggml_transpose(ctx, v)), attn_weights);

  // Permute back: (head_dim, num_heads, seq_len) -> reshape to
...
static struct ggml_tensor* build_expert_cross_attn_layer(
    ...
    struct ggml_tensor* vlm_kv_keys,   // (prefix_len, num_kv_heads * head_dim =
                                       // 320) from VLM cache
    ...
    struct ggml_tensor* cross_attn_mask) // (expert_seq, prefix_len)
{
  ...
  struct ggml_tensor* normed =
      smolvla_rms_norm(ctx, expert_hidden, lw.attn_norm_weight, rms_eps);
  ...
  struct ggml_tensor* k =
      smolvla_linear(ctx, vlm_kv_keys, lw.k_proj_weight, nullptr);
  struct ggml_tensor* v =
      smolvla_linear(ctx, vlm_kv_values, lw.v_proj_weight, nullptr);
  ...
  expert_hidden =
      smolvla_rms_norm(ctx, expert_hidden, lw.ffn_norm_weight, rms_eps);

  struct ggml_tensor* gate =
      smolvla_linear(ctx, expert_hidden, lw.gate_proj_weight, nullptr);
  struct ggml_tensor* up =
      smolvla_linear(ctx, expert_hidden, lw.up_proj_weight, nullptr);
  gate = smolvla_silu(ctx, gate);
  struct ggml_tensor* mlp_out = ggml_mul(ctx, gate, up);
  mlp_out = smolvla_linear(ctx, mlp_out, lw.down_proj_weight, nullptr);
...
extern "C" bool smolvla_run(
  ...
  if (!handle)
    return false;
  return smolvla_inference(
      (smolvla_model*)handle,
      images,
      n_images,
      img_width,
      img_height,
