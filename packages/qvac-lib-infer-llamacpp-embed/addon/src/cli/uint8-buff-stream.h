...
/// @brief Custom basic_streambuf<char> for uint8_t input data, that owns the
/// underlying data.
/// @note basic_streambuf<char> has more support on different platforms than
/// basic_streambuf<uint8_t> which is missing on some platforms (e.g. MacOS,
/// newer NDKs). C++ 17 provides additional guarantees for char.
...
protected:
  ...
  /// @brief Efficient bulk reading. The standard implementation specifies that
  /// this function can be overridden to provide a more efficient
  /// implementation: sgetn will call this function if it is overridden.
  ...
  pos_type seekpos(
      pos_type pos, std::ios_base::openmode which = std::ios_base::in) override;
