...
exports.ERR_CODES = exports.QvacErrorAddonParakeet = exports.ParakeetInterface = void 0;
...
Object.defineProperty(exports, "QvacErrorAddonParakeet", { enumerable: true, get: function () { return error_1.QvacErrorAddonParakeet; } });
...
function createParakeetError(code, message, error) {
    ...
    return new error_1.QvacErrorAddonParakeet({ code, adds: message, cause });
}
/**
...
class ParakeetInterface {
    ...
    async loadWeights(weightsData) {
        try {
            ...
            throw createParakeetError(error_1.ERR_CODES.FAILED_TO_LOAD_WEIGHTS, normalized.message, error);
        }
    }
    activate() {
        try {
            ...
            const normalized = normalizeError(error);
            return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_ACTIVATE, normalized.message, error));
        }
    }
    getBackendInfo() {
    ...
    append(data) {
        try {
            if (data?.type === error_1.END_OF_INPUT) {
            ...
            const normalized = normalizeError(error);
            return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_APPEND, normalized.message, error));
        }
    }
    _submitBufferedJob() {
    ...
    _bufferAudioChunk(rawData) {
        ...
        if (this._bufferedBytes + normalized.byteLength > MAX_BUFFERED_BYTES) {
            throw createParakeetError(error_1.ERR_CODES.BUFFER_LIMIT_EXCEEDED, MAX_BUFFERED_BYTES + " bytes");
        }
    ...
    status() {
        try {
            ...
            const normalized = normalizeError(error);
            return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_GET_STATUS, normalized.message, error));
        }
    }
    pause() {
        try {
            ...
            const normalized = normalizeError(error);
            return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_PAUSE, normalized.message, error));
        }
    }
    async stop() {
        try {
            ...
            const normalized = normalizeError(error);
            throw createParakeetError(error_1.ERR_CODES.FAILED_TO_RESET, normalized.message, error);
        }
    }
    async cancel(jobId) {
        try {
            ...
            const normalized = normalizeError(error);
            throw createParakeetError(error_1.ERR_CODES.FAILED_TO_CANCEL, normalized.message, error);
        }
    }
    ...
    unloadWeights() {
        return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_RESET, "unloadWeights is not supported by this package. Use unload() or destroyInstance()."));
    }
    ...
    async destroyInstance() {
        try {
            ...
            const normalized = normalizeError(error);
            throw createParakeetError(error_1.ERR_CODES.FAILED_TO_DESTROY, normalized.message, error);
        }
    }
    runJob(data) {
        const currentJobId = this._nextJobId;
    ...
    endStreaming() {
        try {
            ...
            const normalized = normalizeError(error);
            return Promise.reject(createParakeetError(error_1.ERR_CODES.FAILED_TO_RESET, normalized.message, error));
        }
    }
