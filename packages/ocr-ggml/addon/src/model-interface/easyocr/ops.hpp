...
//   kernel [KW, KH, IC, OC]  — already in ggml order (== PyTorch shape reversed)
//   bias   [OC]              — per-output-channel offset (folded into pre-bn
//                              when the source layer was `Conv -> BN`)
//
// Returns a tensor of shape [W_out, H_out, OC, N].
::ggml_tensor* conv_2d_bias(::ggml_context* ctx,
                            ::ggml_tensor* x,
                            ::ggml_tensor* kernel,
                            ::ggml_tensor* bias,
                            int s0, int s1,
...
::ggml_tensor* conv_2d_bias_relu(::ggml_context* ctx,
                                 ::ggml_tensor* x,
                                 ::ggml_tensor* kernel,
                                 ::ggml_tensor* bias,
                                 int s0, int s1,
...
::ggml_tensor* bilinear_to(::ggml_context* ctx,
                           ::ggml_tensor* x,
                           int64_t W_target,
                           int64_t H_target);
