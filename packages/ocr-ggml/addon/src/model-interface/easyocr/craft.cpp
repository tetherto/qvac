...
::ggml_tensor* conv_bn_relu(::ggml_context* ctx,
                            const CraftWeights& W,
                            ::ggml_tensor* x,
                            const char* path,
                            int s, int p, int d) {
    return ops::conv_2d_bias_relu(ctx, x,
                                  W.w(path), W.b(path),
                                  s, s, p, p, d, d);
}

// Apply a Conv (no activation, no BN) — used in slice5 and conv_cls.
::ggml_tensor* conv_only(::ggml_context* ctx,
                         const CraftWeights& W,
                         ::ggml_tensor* x,
                         const char* path,
                         int s, int p, int d) {
...
::ggml_tensor* double_conv(::ggml_context* ctx,
                           const CraftWeights& W,
                           ::ggml_tensor* x,
...
::ggml_tensor* cat_channels(::ggml_context* ctx,
                            ::ggml_tensor* a,
                            ::ggml_tensor* b) {
...
::ggml_tensor* build_craft(
    ...
    h = ops::conv_2d_bias(ctx, h, W.w("basenet.slice1.10"), W.b("basenet.slice1.10"),
                          1, 1, 1, 1, 1, 1);
    ...
    h = ops::conv_2d_bias(ctx, h, W.w("basenet.slice2.17"), W.b("basenet.slice2.17"),
                          1, 1, 1, 1, 1, 1);
    ...
    h = ops::conv_2d_bias(ctx, h, W.w("basenet.slice3.27"), W.b("basenet.slice3.27"),
                          1, 1, 1, 1, 1, 1);
    ...
    h = ops::conv_2d_bias(ctx, h, W.w("basenet.slice4.37"), W.b("basenet.slice4.37"),
                          1, 1, 1, 1, 1, 1);
    auto* sources_1 = h;          // h_relu5_3
    tap(taps, craft_taps::kBasenetSlice4, sources_1);

    // === basenet.slice5 ====================================================
    // Special: nn.Sequential of (MaxPool k=3 s=1 p=1) -> Conv 512->1024 (k=3 d=6 p=6)
    ...
    h = sources_1;
    h = ggml_pool_2d(ctx, h, GGML_OP_POOL_MAX,
                     /*k0=*/3, /*k1=*/3,
                     /*s0=*/1, /*s1=*/1,
                     /*p0=*/1.0F, /*p1=*/1.0F);
    ...
    y = double_conv(ctx, W, y,
                    "upconv1.conv.0", "upconv1.conv.3");
    ...
    auto* nhwc = ggml_permute(ctx, y, /*axis0=*/1, /*axis1=*/2, /*axis2=*/0, /*axis3=*/3);
