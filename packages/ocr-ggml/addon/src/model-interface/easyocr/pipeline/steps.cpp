...
cv::Mat fourPointTransform(const cv::Mat& image,
    ...
    const int  maxWidth = std::max(static_cast<int>(widthA),
                                   static_cast<int>(widthB));
    ...
    const int  maxHeight = std::max(static_cast<int>(heightA),
                                    static_cast<int>(heightB));
    ...
    cv::Mat warpedImg;
    cv::warpPerspective(image, warpedImg, perspectiveTransform,
