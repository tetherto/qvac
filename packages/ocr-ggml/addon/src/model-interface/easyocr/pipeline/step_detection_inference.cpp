 ...
 */
std::tuple<cv::Mat, float> resizeAspectRatio(const cv::Mat& img,
                                             float          magRatio) {
    int height = img.rows;
    int width  = img.cols;
    ...
    cv::Mat resized;
    cv::copyMakeBorder(proc, resized, 0, targetH32 - targetH, 0,
                       targetW32 - targetW, cv::BORDER_CONSTANT,
                       cv::Scalar::all(0));
...
cv::Mat normalizeAndBuildCHW(const cv::Mat& img) {
    ...
    float* planes[3] = {chwBlob.ptr<float>(0), chwBlob.ptr<float>(1),
                        chwBlob.ptr<float>(2)};
...
cv::Mat StepDetectionInference::preprocess(const cv::Mat& image,
                                           float           magRatio,
                                           float*          outResizeRatio) {
...
StepDetectionInference::StepDetectionInference(const std::string& gguf_path,
                                               float              magRatio,
                                               int                nThreads)
    : magRatio_(magRatio) {
    ...
    if (!weights_->ok()) {
        throw std::runtime_error(
            "StepDetectionInference: CraftWeights load failed: " +
            weights_->err());
    }
}
...
std::vector<BlockTiming> StepDetectionInference::profileBlocks(
    ...
    if (runsPerTap < 1) runsPerTap = 1;
    if (warmupPerTap < 0) warmupPerTap = 0;
...
StepDetectionInference::process(const StepDetectionInference::Input& input) {
    ...
    QLOG(qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
         std::string("[DetectionInference] Starting - origImg size=") +
             ...
             std::to_string(input.origImg.rows) + ", channels=" +
             std::to_string(input.origImg.channels()) + ", magRatio=" +
             std::to_string(magRatio_));
    ...
    QLOG(qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
         std::string("[DetectionInference] After resize - size=") +
             ...
             std::to_string(imgResized.rows) + ", ratio=" +
             std::to_string(imgResizeRatio));
    ...
    QLOG(qvac_lib_inference_addon_cpp::logger::Priority::DEBUG,
         std::string("[DetectionInference] Output extracted - scoreText=") +
             ...
             std::to_string(scoreText.rows) + ", scoreLink=" +
             std::to_string(scoreLink.cols) + "x" +
