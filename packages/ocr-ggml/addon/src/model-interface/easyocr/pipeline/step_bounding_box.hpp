...
private:
    ...
    void loadConnectedComponents(const cv::Mat& textMap,
    ...
    static std::pair<std::vector<std::array<float, ALIGNED_META_SIZE>>,
                     std::vector<std::array<cv::Point2f, 4>>>
    turnPolysIntoBoxes(const std::vector<std::array<cv::Point2f, 4>>& polys,
    ...
    std::vector<AlignedBox>
    getOutputAlignedBoxes(float imgResizeRatio,
                          const std::vector<std::array<float, 4>>& mergedList);

    std::vector<UnalignedBox>
    getOutputUnalignedBoxes(
