...
std::string upload_weights(
    ...
    for (const auto& d : convs) {
        ...
        if (w_src == nullptr) return "missing tensor: " + d.conv_path + ".weight";
        ...
        ggml_backend_tensor_set(w_[d.conv_path], w_folded.data(), 0,
                                ggml_nbytes(w_[d.conv_path]));
        ggml_backend_tensor_set(b_[d.conv_path], b_folded.data(), 0,
                                ggml_nbytes(b_[d.conv_path]));
    }

    for (const auto& full_name : verbatim_paths) {
        auto* src = loader.get_tensor(full_name);
        if (src == nullptr) return "missing tensor: " + full_name;
        if (src->data == nullptr) return "tensor data not loaded for " + full_name;
        ::ggml_tensor* dst = nullptr;
        if      (auto it = t_.find(full_name); it != t_.end()) dst = it->second;
        else if (full_name.size() > 7 &&
                 full_name.compare(full_name.size() - 7, 7, ".weight") == 0) {
            dst = w_[full_name.substr(0, full_name.size() - 7)];
        } else if (full_name.size() > 5 &&
                   full_name.compare(full_name.size() - 5, 5, ".bias") == 0) {
            dst = b_[full_name.substr(0, full_name.size() - 5)];
        }
        if (dst == nullptr) return "internal: no destination for " + full_name;
...
std::string declare_weights(
    ...
    for (const auto& full_name : verbatim_paths) {
        ...
        if      (n_dims == 1) dst = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, src->ne[0]);
        else if (n_dims == 2) dst = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, src->ne[0], src->ne[1]);
        else return "unsupported tensor rank for " + full_name;
        ggml_set_name(dst, full_name.c_str());

        const bool is_w = full_name.size() > 7 &&
                          full_name.compare(full_name.size() - 7, 7, ".weight") == 0;
...
const std::vector<ConvSpec>& gen2_convs() {
    static const auto p = [] {
        ...
        return std::vector<ConvSpec>{
            ...
            {base + "8",  ""}, {base + "11", base + "12"}, {base + "14", base + "15"},
            {base + "18", ""},
        };
    }();
    return p;
...
CrnnGen2Weights::CrnnGen2Weights(const GgufLoader& loader, ggml_backend_t backend) {
...
void build_crnn_weights_impl(
    ...
    std::unordered_map<std::string, ::ggml_tensor*>& b_,
    std::unordered_map<std::string, ::ggml_tensor*>& t_,
    ::ggml_context*& ctx_, ::ggml_backend_buffer_t& buf_,
    std::string& err_) {

    if (!loader.ok())          { err_ = "GgufLoader is not ok"; return; }
    if (backend == nullptr)    { err_ = "backend is null";       return; }
    ...
    if (ctx_ == nullptr) { err_ = "ggml_init failed"; return; }

    if (auto e = declare_weights(loader, ctx_, convs, verbatim, w_, b_, t_); !e.empty()) {
        err_ = e; return;
    }

    buf_ = ggml_backend_alloc_ctx_tensors(ctx_, backend);
    if (buf_ == nullptr) {
        err_ = "ggml_backend_alloc_ctx_tensors failed"; return;
    }

    if (auto e = upload_weights(loader, convs, verbatim, w_, b_, t_); !e.empty()) {
        err_ = e; return;
    }
}
}  // namespace
...
void CrnnGen2Weights::build_(const GgufLoader& loader, ggml_backend_t backend) {
    build_crnn_weights_impl(loader, backend, gen2_convs(),
                            w_, b_, t_, ctx_, buf_, err_);
}
