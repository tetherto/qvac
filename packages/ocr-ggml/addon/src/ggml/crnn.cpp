...
inline ggml_tensor* maxpool(ggml_context* ctx, ggml_tensor* x,
                            int k0, int k1, int s0, int s1) {
    return ggml_pool_2d(ctx, x, GGML_OP_POOL_MAX,
                        k0, k1, s0, s1, /*p0=*/0.0F, /*p1=*/0.0F);
}

void tap(std::unordered_map<std::string, ggml_tensor*>* taps,
         const char* name, ggml_tensor* t) {
    if (taps != nullptr) {
...
template <class W>
ggml_tensor* conv_bias_t(ggml_context* ctx, const W& weights, ggml_tensor* x,
                         const char* path, int p, int s = 1) {
    return ops::conv_2d_bias(ctx, x,
                             weights.w(path), weights.b(path),
                             /*s0=*/s, /*s1=*/s,
                             /*p0=*/p, /*p1=*/p,
                             /*d0=*/1, /*d1=*/1);
}
...
template <class W>
ggml_tensor* conv_bias_relu_t(ggml_context* ctx, const W& weights, ggml_tensor* x,
                              const char* path, int p, int s = 1) {
    return relu(ctx, conv_bias_t(ctx, weights, x, path, p, s));
}

// Backwards-compatible wrappers used by build_crnn_gen2 below.
ggml_tensor* conv_bias(ggml_context* ctx, const CrnnGen2Weights& W,
                       ggml_tensor* x, const char* path, int p) {
    return conv_bias_t(ctx, W, x, path, p);
}
ggml_tensor* conv_bias_relu(ggml_context* ctx, const CrnnGen2Weights& W,
                            ggml_tensor* x, const char* path, int p) {
...
struct LstmStep { ggml_tensor* h; ggml_tensor* c; };

LstmStep lstm_cell_step(ggml_context* ctx,
    ...
    auto slice = [&](int64_t k) {
        return ggml_view_1d(ctx, gates, hidden,
...
ggml_tensor* lstm_one_direction(
    ...
    if (reverse) {
        for (int64_t t = T - 1; t >= 0; --t) step_iter(t);
    } else {
        for (int64_t t = 0; t < T; ++t)     step_iter(t);
    }
...
ggml_tensor* bilstm_block(ggml_context* ctx, const CrnnGen2Weights& W,
                          ggml_tensor* seq, const char* prefix) {
...
ggml_tensor* build_crnn_gen2(
    ...
    // is exactly what ggml_pool_2d AVG with k=H' does on the (W', H') axes.
    {
        ...
        // (k1=H', s1=H').
        h = ggml_pool_2d(ctx, h, GGML_OP_POOL_AVG,
                         /*k0=*/1, /*k1=*/static_cast<int>(Hp),
                         /*s0=*/1, /*s1=*/static_cast<int>(Hp),
                         /*p0=*/0.0F, /*p1=*/0.0F);
