...
public:
    ...
    static constexpr int   DBNET_INPUT_SIZE   = qvac_lib_infer_ggml_classification::graph::kInputHw;
    ...
    explicit StepDoctrDetectionGGML(const std::string& pathDetector,
                                    int nThreads = 0);
...
private:
    ...
    extractPolygons(const cv::Mat& probMap, float scale,
