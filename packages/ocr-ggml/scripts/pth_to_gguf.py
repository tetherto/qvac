...
into a GGUF file, keeping the original dtype (F32 / I64). The model graph
itself is NOT serialized - it must be rebuilt by the GGML runtime that
loads this file (see easyocr/craft.py and easyocr/model/{vgg_model,model}.py
...
Usage:
    python scripts/pth_to_gguf.py <input.pth> <output.gguf> [--arch NAME]
...
def convert(
    ...
    writer.add_description(
        ...
        "Graph must be reconstructed by the GGML runtime; see easyocr/craft.py "
        "and easyocr/model/{vgg_model,model}.py for the reference forward passes."
    )
    ...
    writer.add_string("conversion.tool", "easyocr/scripts/pth_to_gguf.py")
