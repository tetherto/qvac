...
async function loadCosyvoiceTTS(params = {}) {
  ...
  // Pinning the LM trajectory is what makes two runs comparable: sampling is
  // chaotic in the logits, so without greedy + a fixed seed a before/after
  // diff measures the sampler, not the conditioning.
  if (params.greedy !== undefined) options.greedy = params.greedy
