...
// True when an ARM Mali / Immortalis (Valhall) GPU device is registered with ggml.
// Read-only: enumerates devices and inspects their name/description; it never inits a
// device or commits a backend selection, so it is safe to call after the engine has
// already chosen CPU. Mirrors tts-cpp's desc_or_name_is_arm_mali (matches "mali" or
// "immortalis"). Scoped to the vendor tts-cpp declines by policy (allow_arm_mali=false)
// so Adreno / Xclipse — which DO run GPU — are never matched, and a genuine GPU->CPU
// regression on those vendors stays unflagged.
inline bool androidOffAllowlistGpuPresent() {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (containsCaseInsensitive(name, "mali") || containsCaseInsensitive(desc, "mali") ||
        containsCaseInsensitive(name, "immortalis") || containsCaseInsensitive(desc, "immortalis")) {
      return true;
    }
  }
  return false;
