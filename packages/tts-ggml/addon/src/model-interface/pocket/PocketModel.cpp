...
PocketModel::PocketModel(PocketConfig config)
  ...
  if (o.flow_lm_path.empty() || o.mimi_path.empty() ||
      o.frontend_path.empty() ||
      o.voice_path.empty() == o.reference_audio_path.empty())
    throw StatusError(
        general_error::InvalidArgument,
        "Pocket requires FlowLM, Mimi, frontend, and exactly one prepared "
        "voice or reference WAV");
  if (sampleRate_ < 8000 || sampleRate_ > 192000)
    throw StatusError(
        general_error::InvalidArgument,
        "outputSampleRate must be 8000..192000");
}
...
void PocketModel::reload(PocketConfig config) {
  ...
  if (config.options.output_sample_rate != sampleRate_)
    throw StatusError(
        general_error::InvalidArgument,
        "recreate PocketModel to change outputSampleRate");
  auto engine =
      makeEngine(config); // retain the previous usable model on failure
  std::lock_guard lock(mutex_);
...
std::any PocketModel::process(const std::any& input) {
  ...
  if (!request)
    throw StatusError(
        general_error::InvalidArgument, "PocketModel requires AnyInput");
  BusyGuard guard(busy_);
  ...
  if (!engine)
    throw createTTSError(
        TTSErrorCode::InitializationFailed, "PocketModel is not loaded");
  Output output;
  int index = 0;
  int64_t samples = 0;
  ...
  try {
    const auto result = engine->synthesize_stream(
        request->text, [&](const float* pcm, size_t n, int rate) {
          if (cancelEpoch_.load() != epoch)
            return false;
          if (rate != sampleRate_)
            throw std::runtime_error("unexpected native output sample rate");
          Output chunk(n);
          for (size_t i = 0; i < n; ++i) {
            if (!std::isfinite(pcm[i]))
              throw std::runtime_error("non-finite audio sample");
            chunk[i] = static_cast<int16_t>(
                std::lround(std::clamp(pcm[i], -1.0f, 1.0f) * 32767.0f));
          }
          if (!samples)
            firstAudioMs = elapsed() * 1000;
          samples += n;
          if (request->chunkCallback)
            request->chunkCallback(std::move(chunk), index++, false);
          else
            output.insert(output.end(), chunk.begin(), chunk.end());
          return cancelEpoch_.load() == epoch;
        });
    if (result.cancelled || cancelEpoch_.load() != epoch)
      throw std::runtime_error("synthesis cancelled");
    if (request->chunkCallback)
      request->chunkCallback({}, index, true);
  } catch (const std::exception& e) {
    throw createTTSError(
        TTSErrorCode::SynthesisFailed,
        std::string("pocket.synthesize: ") + e.what());
  }
  const double seconds = elapsed(), duration = double(samples) / sampleRate_;
  {
    ...
    stats_.emplace_back(
        "realTimeFactor", duration > 0 ? seconds / duration : 0.0);
    stats_.emplace_back(
        "tokensPerSecond", seconds > 0 ? request->text.size() / seconds : 0.0);
