...
on **CPU** by default; GPU offload is opt-in via `useGPU` / `nGpuLayers` on
Metal (Apple) and Android's OpenCL/Adreno path (`openclCacheDir` persists its
compiled-kernel cache), with other hosts falling back to CPU.
...
| `cosyvoice-tts.js` | CosyVoice3 instruct-conditioned batch synth (24 kHz; CPU by default, `--gpu` opts into Metal / Android GPU). `bare examples/cosyvoice-tts.js --gpu "Hello"` |
