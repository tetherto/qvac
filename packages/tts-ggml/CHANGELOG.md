...
- **CosyVoice3 engine.** Adds the Fun-CosyVoice3-0.5B native C++/ggml TTS engine
  ...
  vocoder (24 kHz), on CPU with opt-in GPU offload (Metal on Apple,
  OpenCL/Adreno on Android). Dialect / volume / style control via the
  `instruct` option; emotion and speaking rate use the cross-engine `emotion` /
  `pace` options below.
