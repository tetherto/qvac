...
try {
  ...
  const schema = new a.Schema([
    ...
    new a.Field('vector', new a.FixedSizeList(1024, new a.Field('item', new a.Float32())))
  ])
  ...
  const results = (await documentsTable.vectorSearch(queryEmbedding).limit(1).toArray()) as {
    id: string
    text: string
    vector: number[]
    _distance: number
