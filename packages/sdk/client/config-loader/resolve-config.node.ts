...
import { validateConfig, parseJsonConfig, type QvacConfig } from './config-utils'
...
async function loadTsConfig(filePath: string): Promise<QvacConfig> {
  ...
  try {
    const tsxModule = (await import(tsxApiPath)) as {
      tsImport: (configFilePath: string, baseUrl: string) => Promise<{ default?: unknown }>
    }
...
async function findConfigFileInDir(searchDir: string): Promise<string | undefined> {
