...
publishes a description for every source that load names. One fit measures one
model against the whole machine, and two such answers carry nothing that can be
summed under one budget, so a set of candidates keeps the calibrated estimate
that can be aggregated. A model with no published description, and an offline
caller, fall back the same way.
...
or to the file itself where it is already on disk. The estimator reads its
workload from the same config: `ctx_size` for a token context, `duration_ms` for
an audio window.
...
| `the streaming-session coefficient for this platform has not been measured`                                               | A speech load whose config declares `streaming: true`, which only the parakeet config carries.                                                                                                                                                                                                        | Same.                                                                                                |
...
| `workload kind '<kind>' is not supported by <estimator>`                                                                  | The `modelType` names an engine whose estimator does not size this kind of load.                                                                                                                                                                            | Pass the `modelType` the load would use.                                                       |
