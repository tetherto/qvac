...
This development workflow requires the RPC-enabled LLM client and managed server
builds recorded in [inference configuration](../inference/docs/configuration.md).
Use matching Fabric builds on the initiator and servers. Published packages must
include those native changes before releasing inference, then SDK.

On each serving machine, start a server using its private IPv4 address:

```ts
import { startRpcServer, stopRpcServer } from '@qvac/sdk'
...
// Later, after the initiator has unloaded its model:
await stopRpcServer({ serverId: server.serverId })
```

Use a different host on the second serving machine. Keep both servers running
while the initiating machine discovers and loads:

```ts
import { discoverRpcServers, loadModel, completion, unloadModel } from '@qvac/sdk'

const candidates = await discoverRpcServers({ topic: 'my-private-rpc-group', timeoutMs: 5000 })
const selected = candidates.sort((a, b) => a.url.localeCompare(b.url)).slice(0, 2)
if (selected.length !== 2) throw new Error('Two reachable RPC servers are required')
...
try {
  console.log(await completion({ modelId, history: [{ role: 'user', content: 'Hello' }] }).text)
} finally {
  await unloadModel({ modelId })
}
```

The model file stays on the initiator. Discovery supplies candidates; your
application chooses their order. Alternatively, supply known private endpoints
directly through `rpc-servers`. `RPC0` and `RPC1` are per-load native device names
assigned in endpoint order, not peer identities. Explicit `devices` prevents
unintended selection of other GPUs. Preserve the matching weight order in
`tensor-split` and verify native placement logs before claiming distributed
execution. Unloading a model leaves the serving workers available. A serving
worker owns each returned `serverId` until stop or worker close; IDs do not
survive worker restart. Do not start local inference on a serving device until
its serving work has stopped.

A discovery topic does not authenticate peers. Native GGML RPC has neither
authentication nor encryption. Use a trusted private network with external access
controls for discovery participants, native endpoints, and eligible tensor
all-reduce connections between servers. Discovery cannot make endpoints reachable
through NAT and does not check Fabric build compatibility. Tensor all-reduce
normally needs the RPC port plus 1000 between servers; use `GGML_RPC_NO_COMM=1`
when checking its fallback. Start with layer splitting on slower links.

SDK workers always use the Bare in-process server and TCP. Optional Linux RDMA
builds in the direct Node native packages are outside this worker path. Native
stop has no deadline and can wait for native work to finish. Mobile serving and
initiation require validation on physical devices; the presence of a shared
executor does not establish platform support. Public-internet operation is not
supported.
