  ...
  const rpc = new RPC(conn, async (req) => {
    try {
      ...
      if (message.type === 'loadModel') {
        try {
          console.log(`🔧 Processing loadModel request for: ${message.modelIdOrPath}`)

          if (message.withProgress) {
            console.log('📊 Progress streaming enabled - using RPC response stream')
            ...
            const sendProgress = () => {
              if (downloaded < totalSize) {
                ...
                console.log(`📤 Streaming progress: ${progressUpdate.percentage.toFixed(1)}%`)
                responseStream.write(JSON.stringify(progressUpdate) + '\n', 'utf-8')
                ...
                console.log('📤 Sending final response through stream:', response)
        ...
        console.log(`🔧 Processing completion request for model: ${message.modelId}`)

        // Mock completion response - use response stream like client expects
        const responseStream = req.createResponseStream()

        const tokens = ['This is', ' a test', ' response', ' from', ' the', ' mock', ' provider']
        ...
        console.log('📤 Sending completion final response via stream:', finalResponse)
        responseStream.end()
      } else {
