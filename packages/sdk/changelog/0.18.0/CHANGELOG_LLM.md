...
Parakeet Indic Conformer CTC models are in the registry (`PARAKEET_INDIC_CONFORMER_CTC_*` plus matching `PARAKEET_CTC_*` constants). They use the existing unified ASR transcription path.
...
This release adds VisionPsy Nano (base and Flash) multimodal constants, Indic Conformer / Parakeet CTC transcription weights, Audio8 codec + LM constants, CosyVoice3 companions, and Qwen3-8 27B multimodal shards. `TTS_COSYVOICE3_LLM_COSYVOICE_Q8_0` is updated.
