...
//
// `analyze` is exported so `.github/scripts/test/sdk-e2e-impacted.test.mjs` can
// exercise relations 2 and 3 against the real repository; only the catalog it
// takes as input needs esbuild, which `policy-tests` does not install.
//
// Usage:
...
/**
 * Resolves the changed files against the three relations.
 *
 ...
 */
export function analyze({ repoRoot, catalog, idsByDefinitionFile, changed, hunksByFile }) {
...
async function main() {
  ...
  if (args.githubOutput) {
    writeFileSync(args.githubOutput, `also-tests=${report.alsoTests.join(',')}\n`, { flag: 'a' })
  }
  ...
  console.log(`adding: ${report.alsoTests.length} (~${report.addedMinutes} min)`)
