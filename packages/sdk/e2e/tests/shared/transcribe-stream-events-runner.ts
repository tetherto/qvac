...
export async function runTranscribeStreamEventsTest(
  ...
  let session: TranscribeStreamConversationSession | TranscribeStreamSession | null = null
  try {
    ...
    const chunkSize = Math.floor((chunkMs / 1000) * EXPECTED_SAMPLE_RATE) * BYTES_PER_SAMPLE
