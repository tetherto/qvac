...
function isDelegationError(msg: string): boolean {
  return msg.includes('DELEGATE_CONNECTION_FAILED') || msg.includes('RPC connection failed')
}
...
export class DelegatedInferenceExecutor extends BaseExecutor<typeof allTests> {
  ...
  private async withProvider<T>(fn: (ctx: { publicKey: string }) => Promise<T>): Promise<T> {
  ...
  async providerStart(): Promise<TestResult> {
    ...
    try {
      if (!response.publicKey || typeof response.publicKey !== 'string') {
        return {
          passed: false,
          output: `Missing or invalid publicKey: ${JSON.stringify(response)}`
        }
      }
      return {
        passed: true,
  ...
  async providerFirewall(params: typeof delegatedProviderFirewall.params): Promise<TestResult> {
    ...
    try {
      if (!response.publicKey) {
        return {
          passed: false,
          output: `Provider with firewall failed: ${JSON.stringify(response)}`
        }
      }
      return {
        passed: true,
    ...
    return this.withProvider(async ({ publicKey }) => {
      try {
        ...
        if (response.type !== 'heartbeat') {
          return {
            passed: false,
            output: `Invalid heartbeat response: ${JSON.stringify(response)}`
          }
        }
        ...
        if (isDelegationError(msg)) {
          return {
            passed: true,
            output: `Delegated heartbeat routed correctly (same-process): ${msg.substring(0, 120)}`
          }
        }
    ...
    return this.withProvider(async ({ publicKey }) => {
      ...
      try {
        ...
        if (isDelegationError(msg)) {
          return {
            passed: true,
            output: `Delegated cancel routed correctly: ${msg.substring(0, 100)}`
          }
        }
  ...
  async connectionFailure(params: typeof delegatedConnectionFailure.params): Promise<TestResult> {
    ...
    try {
      ...
      return {
        passed: false,
        output: `Unexpected error (expected delegation error): ${msg.substring(0, 120)}`
      }
    }
  }
  ...
  async invalidProviderKey(): Promise<TestResult> {
    try {
      ...
      return {
        passed: false,
        output: `Unexpected error (expected delegation/validation error): ${msg.substring(0, 120)}`
      }
    }
  }
  ...
  async providerNotFound(params: typeof delegatedProviderNotFound.params): Promise<TestResult> {
    try {
      ...
      if (msg.includes('not found on the DHT')) {
        return {
          passed: true,
          output: `Unreachable provider categorized as not-found: ${msg.substring(0, 160)}`
        }
      }
      if (isDelegationError(msg)) {
        return {
          passed: true,
          output: `Unreachable provider detected (uncategorized, likely timeout): ${msg.substring(0, 160)}`
        }
      }
      return {
        passed: false,
        output: `Unexpected error (expected delegation error): ${msg.substring(0, 160)}`
      }
    }
  }
}
