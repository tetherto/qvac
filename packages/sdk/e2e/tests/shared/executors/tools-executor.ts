...
export class ToolsExecutor extends AbstractModelExecutor<typeof toolsTests> {
  ...
  async generic(params: unknown, expectation: unknown): Promise<TestResult> {
    ...
    try {
      ...
      const resultData =
        text || (toolCalls && toolCalls.length > 0 ? 'tool call made' : 'no response')
  ...
  private validateToolCallShape(
    ...
    const valid = toolCalls.filter((c) => typeof c.name === 'string' && declaredNames.has(c.name))
    if (valid.length === 0) {
      const got = toolCalls.map((c) => c.name ?? '<unnamed>').join(', ')
      return {
        passed: false,
        output: `No tool call matched a declared tool. Got: [${got}], declared: [${[...declaredNames].join(', ')}]`
      }
    }
    ...
    if (match) {
      ...
      for (const key of expected.argKeys ?? []) {
        if (!(key in args)) {
          return {
            passed: false,
            output: `Tool call '${expected.name}' missing argument key '${key}'. Got: ${JSON.stringify(args)}`
          }
        }
      }
    }
