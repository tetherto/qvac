...
export class LifecycleExecutor extends AbstractModelExecutor<typeof lifecycleTests> {
  ...
  protected defaultHandler = (async (
    testId: string,
    _params: {},
    ...
    try {
      ...
      const recoveryError = await this.ensureActiveStrict().then(
        () => null,
        (e: unknown) => formatError(e)
      )
      const detail = recoveryError ? ` [recovery also failed: ${recoveryError}]` : ''
      return {
        passed: false,
        output: `lifecycle [${testId}] failed: ${formatError(error)}${detail}`
      }
    }
  }) as never
  ...
  private async ensureActiveStrict(maxAttempts = ENSURE_ACTIVE_MAX_ATTEMPTS): Promise<void> {
    ...
    throw new Error(
      `Failed to restore runtime to "active" after ${maxAttempts} attempts: ${reason}`
    )
  }
