...
export class DiffusionExecutor extends AbstractModelExecutor<typeof diffusionTests> {
  ...
  protected handlers: Required<{
    ...
    [diffusionStandaloneUpscalerBackendDevice.testId]:
      this.standaloneUpscalerBackendDevice.bind(this),
  ...
  async streamingProgress(params: DiffusionParams): Promise<TestResult> {
    ...
    try {
      const { progressStream, outputs, stats } = diffusion(this.buildParams(modelId, params))
  ...
  async standaloneUpscalerCpu(params: DiffusionParams, _expectation: unknown): Promise<TestResult> {
    const p = await this.resolveParams(params)
  ...
  protected buildParams(modelId: string, p: DiffusionParams): DiffusionClientParams {
  ...
  private readPngDims(buf: Uint8Array): { width: number; height: number } | null {
  ...
  private assertEqualPngDimensions(left: Uint8Array, right: Uint8Array): TestResult | null {
