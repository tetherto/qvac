...
export class ParakeetStreamExecutor extends AbstractModelExecutor<typeof parakeetStreamTests> {
  ...
  private async loadAudioBytes(audioFileName: string): Promise<Uint8Array> {
    const audioPath = path.resolve(process.cwd(), 'assets/audio', audioFileName)
