...
export class DelegatedInferenceExecutor extends SharedDelegatedInferenceExecutor {
  ...
  private async withRemoteProvider<T>(fn: (ctx: { publicKey: string }) => Promise<T>): Promise<T> {
    ...
    return this.withRemoteProvider(async ({ publicKey }) => {
      const modelId = await loadModel({
        ...
        delegate: {
          ...
          timeout: E2E_DELEGATION_TIMEOUT,
      ...
      try {
        ...
        return {
          passed: true,
          output: `E2E delegated streaming (${tokenCount} tokens): "${fullText.substring(0, 80)}"`
        }
      } finally {
        try {
