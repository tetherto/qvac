...
export class LoggingExecutor extends AbstractModelExecutor<typeof loggingTests> {
  ...
  private runHandler(handler: HandlerKey, testId: string, params: AnyParams): Promise<TestResult> {
    switch (handler) {
      case 'addon-logging':
        return this.runAddonLogging(testId, params)
      case 'invalid-model-id':
        return this.runInvalidModelId(requireInvalidModelIdParams(params))
      case 'during-inference':
        return this.runDuringInference(testId, params as DuringInferenceParams)
      case 'concurrent':
        return this.runConcurrent(testId, requireOperationsParams(params))
      case 'reload':
        return this.runReload(testId)
    }
  }
  ...
  private async runAddonLogging(testId: string, params: AnyParams): Promise<TestResult> {
    ...
    if (!isTriggerKey(triggerKey)) {
      throw new Error(
        `addon-logging test "${testId}" has invalid params.trigger: ${String(triggerKey)}`
      )
    }
  ...
  private async runDuringInference(
    testId: string,
    params: DuringInferenceParams
  ): Promise<TestResult> {
    const dep = getRequiredMeta(testId, 'dependency')
    const targetId = await this.resources.ensureLoaded(dep)
    ...
    return collectLogs({
      ...
      trigger: async () => {
        ...
        if (params.operations.includes('completion')) {
          ops.push(
            callWhenAddonIdle(() => runCompletion(targetId, 'Test concurrent logging', false))
          )
        }
        if (params.operations.includes('embedding')) {
          const embeddingModelId = await this.resources.ensureLoaded('embeddings')
          ops.push(
            embed({ modelId: embeddingModelId, text: 'test concurrent' }).then(() => undefined)
          )
        }
  ...
  private async runReload(testId: string): Promise<TestResult> {
    ...
    return collectLogs({
      ...
      trigger: () =>
        callWhenAddonIdle(() => runCompletion(reloadedModelId, 'Post-reload test', false))
    })
  }
...
async function collectLogs(opts: CollectLogsOptions): Promise<CollectLogsResult> {
  ...
  await Promise.race([collectPromise, triggerPromise.then(() => sleep(postTriggerWaitMs))])
  ...
  await Promise.race([
    ...
    sleep(TRIGGER_JOIN_GRACE_MS).then(() => {
      graceFired = true
    })
  ])
...
const requireInvalidModelIdParams = (p: AnyParams) =>
  requireStringField(p, 'invalidModelId', 'invalid-model-id handler')
const requireOperationsParams = (p: AnyParams) =>
  requireStringArrayField(p, 'operations', 'concurrent handler')
