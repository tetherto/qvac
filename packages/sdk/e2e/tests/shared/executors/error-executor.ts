...
export class ErrorExecutor extends AbstractModelExecutor<typeof errorTests> {
  ...
  async chainingCause(): Promise<TestResult> {
    try {
      ...
      return {
        passed: hasCause || isStructured,
        output: `hasCause=${hasCause}, structured=${isStructured}`
      }
    }
  }
  ...
  async ragOperationFailed(params: RagOperationFailedParams): Promise<TestResult> {
    try {
      await ragIngest({
        ...
        documents: 'test content' as never,
        workspace: 'test'
      })
  ...
  async completionError(
    ...
    try {
      ...
      const text = params.stream
        ? await (async () => {
            let t = ''
            for await (const tok of result.tokenStream) t += tok
            return t
          })()
        : await result.text
