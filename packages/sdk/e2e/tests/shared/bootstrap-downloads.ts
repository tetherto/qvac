...
export function resolveBootstrapDownloadConcurrency(
  ...
  return (
    positiveIntegerOrNull(env.QVAC_E2E_DOWNLOAD_CONCURRENCY) ?? DEFAULT_DOWNLOAD_CONCURRENCY[target]
  )
}
...
async function mapSettledWithConcurrency<T>(
  ...
  await Promise.all(
    Array.from({ length: workerCount }, () => runQueueWorker(items, results, state, worker))
  )
  return results
}
...
export async function runBootstrapDownloads(
  ...
  const firstPassResults = await mapSettledWithConcurrency(items, concurrency, (item) =>
    runItem(item, false)
  )
  ...
  for (const failure of firstPassFailed) {
    log?.(`❌ download failed: ${failure.item.name}: ${formatReason(failure.reason)}`)
  }
  ...
  const retryResults = await mapSettledWithConcurrency(retryItems, retryConcurrency, (item) =>
    runItem(item, true)
  )
  const finalFailed = collectFailures(retryItems, retryResults)
  if (finalFailed.length > 0) {
    for (const failure of finalFailed) {
      log?.(`❌ retry failed: ${failure.item.name}: ${formatReason(failure.reason)}`)
    }
    throw new Error(`${finalFailed.length}/${items.length} downloads failed after retry pass`)
  }
