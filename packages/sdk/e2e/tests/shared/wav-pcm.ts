...
export function decodeWavToMonoF32(bytes: Uint8Array): DecodedPcm {
  ...
  if (audioFormat !== 1) {
    throw new Error(`decodeWavToMonoF32: unsupported audio format ${audioFormat} (only PCM=1)`)
  }
  if (bitsPerSample !== 16) {
    throw new Error(`decodeWavToMonoF32: unsupported bit depth ${bitsPerSample} (only 16-bit)`)
  }
  ...
  const pcm = new DataView(bytes.buffer, bytes.byteOffset + dataOffset, dataLength)
