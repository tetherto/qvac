...
import { BaseExecutor, type TestResult } from '@qvac/qvac-test-suite/mobile'
import {
    ...
    try {
      ...
      if (!firstMidProgress) {
        return {
          passed: false,
          output:
            'could not exercise mid-download — WHISPER_TINY produced no in-flight progress after a forced cold cache'
        }
      }
      return {
        passed: true,
  ...
  async httpNetdrop(): Promise<TestResult> {
    ...
    if (!base) {
      return {
        ...
        skipped: true,
        output:
          'skipped: flaky-lan-server not reachable (start it locally with `node tests/shared/flaky-lan-server.mjs`; unreachable on CI Device Farm)'
      }
    }
