...
export class MobileClassificationExecutor extends ModelAssetExecutor<typeof classificationTests> {
  ...
  private async readSampleImage(): Promise<Uint8Array> {
    ...
    if (!assetModule) {
      throw new Error(`Image file not found in bundled assets: ${SAMPLE_IMAGE_FILENAME}`)
    }
    return await this.resolveAssetBytes(assetModule)
  }

  async runClassify(params: ClassificationParams, expectation: Expectation): Promise<TestResult> {
    try {
