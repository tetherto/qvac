...
export class MobileOcrExecutor extends ModelAssetExecutor<typeof ocrTests> {
  ...
  private checkStats(
    stats: { detectionTime?: number; recognitionTime?: number; totalTime?: number } | undefined
  ): TestResult | null {
    ...
    if (typeof stats.totalTime !== 'number' || stats.totalTime <= 0) {
      return {
        passed: false,
        output: `Expected stats.totalTime > 0, got: ${JSON.stringify(stats)}`
      }
    }
    return null
  }
  ...
  private checkBlockStructure(blocks: OCRTextBlock[]): TestResult | null {
    for (const [i, block] of blocks.entries()) {
      if (typeof block.text !== 'string') {
        return {
          passed: false,
          output: `Block[${i}].text is not a string: ${JSON.stringify(block)}`
        }
      }
      if (!block.bbox || !Array.isArray(block.bbox) || block.bbox.length !== 4) {
        return {
          passed: false,
          output: `Block[${i}].bbox is not a 4-element array: ${JSON.stringify(block)}`
        }
      }
      ...
      if (badCoord !== -1) {
        return {
          passed: false,
          output: `Block[${i}].bbox[${badCoord}] is not a number: ${block.bbox[badCoord]}`
        }
      }
      if (typeof block.confidence !== 'number') {
        return {
          passed: false,
          output: `Block[${i}].confidence is not a number: ${JSON.stringify(block)}`
        }
      }
      if (block.confidence < 0 || block.confidence > 1) {
        return {
          passed: false,
          output: `Block[${i}].confidence out of range [0,1]: ${block.confidence}`
        }
      }
    }
    return null
  ...
  async generic(params: OcrParams, expectation: Expectation): Promise<TestResult> {
    try {
      ...
      return {
        passed: false,
        output: `OCR failed: ${error instanceof Error ? error.message : String(error)}`
      }
    }
  }
  ...
  async withStats(params: OcrParams, expectation: Expectation): Promise<TestResult> {
    try {
      ...
      return {
        passed: false,
        output: `OCR stats failed: ${error instanceof Error ? error.message : String(error)}`
      }
    }
  }
  ...
  async blockStructure(params: OcrParams, expectation: Expectation): Promise<TestResult> {
    try {
      ...
      return {
        passed: false,
        output: `OCR block-structure failed: ${error instanceof Error ? error.message : String(error)}`
      }
    }
  }
}
