    ...
    batchCompletionStream: defineHandler({
      ...
      handler: async function* (request) {
        ...
        if (Boolean(ctx.signal.aborted)) {
          const abortedIds = request.prompts.map(
          ...
          yield {
            ...
            events: abortedIds.flatMap((id) =>
              wrapBatchEvents(
                id,
                getNormalizer(id).finish({ stopReason: "cancelled" as const }),
              ),
            ),
          };
          return;
        ...
        try {
          ...
          for (const batchResult of results) {
            ...
            if (
              normalizer.getAccumulated().rawText.length === 0 &&
              batchResult.output.length > 0
            ) {
              terminalEvents.push(
                ...wrapBatchEvents(
                  batchResult.id,
                  normalizer.push(batchResult.output),
                ),
              );
            }
