...
function installPackage(parentDir: string, name: string) {
  ...
  fs.writeFileSync(path.join(pkgDir, 'package.json'), JSON.stringify({ name, main: 'index.js' }))
  return pkgDir
}

function withTempProject(fn: (projectRoot: string, workspaceRoot: string) => void) {
  const workspaceRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qvac-resolve-sdk-')))
  const projectRoot = path.join(workspaceRoot, 'mobile')
  fs.mkdirSync(projectRoot)
  fs.writeFileSync(path.join(projectRoot, 'package.json'), JSON.stringify({ name: 'mobile' }))
  try {
...
test('resolveSDKPackageDir: prefers projectRoot copy over a hoisted ancestor copy and warns', (t) => {
  withTempProject((projectRoot, workspaceRoot) => {
    ...
    t.ok(warnings[0]!.includes(shadowed), 'warning names the shadowed directory path')
    t.ok(warnings[0]!.includes(expected), 'warning names the winning directory path')
  })
})
...
test('resolveSDKPackageDir: prefers the closest install when two different SDK packages live at different depths', (t) => {
  withTempProject((projectRoot, workspaceRoot) => {
    ...
    t.ok(warnings[0]!.includes('@tetherto/sdk-dev'), 'warning lists the ignored package name')
  })
})
...
test('resolveSDKPackageDir: throws MultipleSDKInstallationsError when two different SDK packages share the closest depth', (t) => {
  withTempProject((projectRoot) => {
    ...
    t.ok(threw instanceof MultipleSDKInstallationsError, 'throws MultipleSDKInstallationsError')
    const err = threw as MultipleSDKInstallationsError
    t.ok(
      err.message.includes('@qvac/sdk') && err.message.includes('@tetherto/sdk-dev'),
...
test('resolveSDKPackageDir: throws SDKNotFoundInNodeModulesError when no SDK is installed anywhere', (t) => {
  withTempProject((projectRoot) => {
    ...
    t.ok(threw instanceof SDKNotFoundInNodeModulesError, 'throws SDKNotFoundInNodeModulesError')
  })
})
