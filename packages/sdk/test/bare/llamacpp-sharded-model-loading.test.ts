...
import { unregisterAddonLogger } from '@/logging'
...
test('llamacpp completion plugin passes only the first shard path to the addon', async (t) => {
  const { llmPlugin } = await import('@/server/bare/plugins/llamacpp-completion/plugin')
...
test('llamacpp embedding plugin passes only the first shard path to the addon', async (t) => {
  const { embeddingsPlugin } = await import('@/server/bare/plugins/llamacpp-embedding/plugin')
