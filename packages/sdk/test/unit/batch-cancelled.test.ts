...
test("isAddonCancelledError: codeString anchored — sibling names don't match", (t) => {
  t.is(
    isAddonCancelledError(
      Object.assign(new Error("x"), { code: "[ LLM :: CancelledByPolicy ]" }),
    ),
    false,
  );
  t.is(
    isAddonCancelledError(
      Object.assign(new Error("x"), { code: "[ LLM :: PreCancelled ]" }),
    ),
    false,
  );
  t.is(
    isAddonCancelledError(
      Object.assign(new Error("x"), { code: "[ LLM :: Cancelled ]" }),
    ),
    true,
  );
});
