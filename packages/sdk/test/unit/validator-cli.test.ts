...
for (const { title, desc, body: bodyFile } of validTitles) {
  test(`PR valid: ${desc}`, (t) => {
    ...
    const result = runValidator(`--type=pr --title="${title}" --body=${escapeShellArg(body)}`)
    t.is(result.exitCode, 0)
...
test('PR [mod]: rejects with empty body', (t) => {
  const result = runValidator(`--type=pr --title="QVAC-123 feat[mod]: add models" --body=""`)
  t.is(result.exitCode, 1)
  t.ok(result.output.includes('Models section'))
})
...
test('edge: PR without body passes for non-special tags', (t) => {
  const result = runValidator(`--type=pr --title="QVAC-123 feat: add feature" --body=""`)
  t.is(result.exitCode, 0)
  t.ok(result.output.includes('✅ Valid PR'))
})
...
test('parsed: commit returns correct structure', (t) => {
  const result = runValidator(`--type=commit --msg="feat[api]: add streaming support"`)
  t.is(result.exitCode, 0)
...
test('parsed: commit with multiple tags returns all tags', (t) => {
  const result = runValidator(`--type=commit --msg="feat[mod|notask]: update models"`)
  t.is(result.exitCode, 0)
