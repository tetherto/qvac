...
const progressAlwaysEntry: HandlerEntry = { type: "reply", handler: noop, supportsProgress: true };
const progressFnEntry: HandlerEntry = {
  type: "reply",
  handler: noop,
  supportsProgress: (r) => r.type === "rag" && ["ingest", "saveEmbeddings"].includes((r as Record<string, unknown>).operation as string),
};
...
test("reply handler with supportsProgress=true and withProgress uses stream transport", (t: { ok: Function }) => {
  t.ok(shouldUseStreamErrorTransport(progressAlwaysEntry, raw({ type: "loadModel", withProgress: true })));
});

test("reply handler with supportsProgress function and matching operation uses stream transport", (t: { ok: Function }) => {
  t.ok(shouldUseStreamErrorTransport(progressFnEntry, raw({ type: "rag", operation: "ingest", withProgress: true })));
});

// ========== Must NOT use stream transport ==========

test("reply handler without progress uses reply transport", (t: { ok: Function }) => {
  t.ok(!shouldUseStreamErrorTransport(replyEntry, raw({ type: "getModelInfo" })));
});

test("reply handler with supportsProgress=true but no withProgress uses reply transport", (t: { ok: Function }) => {
  t.ok(!shouldUseStreamErrorTransport(progressAlwaysEntry, raw({ type: "loadModel" })));
});

test("reply handler with supportsProgress function and non-matching operation uses reply transport", (t: { ok: Function }) => {
  t.ok(!shouldUseStreamErrorTransport(progressFnEntry, raw({ type: "rag", operation: "search", withProgress: true })));
});
