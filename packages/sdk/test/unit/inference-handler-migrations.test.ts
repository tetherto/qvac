...
test('lifecycle logs: registry emits [request-lifecycle] lines on begin/cancel/end for the four registry-routed inference kinds', async (t) => {
  ...
  const { createRequestRegistry } = await import('@/server/bare/runtime/request-registry')

  const kinds = ['embeddings', 'transcribe', 'translate', 'finetune'] as const
  for (const kind of kinds) {
    ...
    const cancel = lines.find((l) => l.includes('[request-lifecycle] cancel'))
    ...
    t.ok(begin?.includes(`modelId=test-${kind}`), `begin carries modelId for ${kind}`)
  }
})
