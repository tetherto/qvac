...
import type { CompanionSetMetadata, ProcessedModel } from '@/models/update-models/types'
...
test('deduplicateModels: preserves companion-referenced duplicates (QVAC-18420)', (t) => {
  ...
  t.ok(paths.includes('bergamot/bergamot-enfr/2025/vocab.enfr.spm'), 'enfr vocab preserved')
  t.ok(
...
test('deduplicateModels: still drops non-companion duplicates even when others are companions', (t) => {
  ...
  t.absent(paths.includes('other/unrelated.bin'), 'unreferenced duplicate still dropped')
})
...
test('deduplicateModels: preserves companion references across different companion sets', (t) => {
  ...
  const result = deduplicateModels([primaryA, vocabA, primaryB, vocabB], false)
  const paths = result.map((m) => m.registryPath)
