...
const completionClientParamsBaseSchema = completionParamsSchema.extend({
  ...
  requestId: z
    ...
    .optional()
    .describe(
      "Stable identifier for this in-flight request, generated by the client at call time (UUIDv4). Surfaced on the `CompletionRun` result so callers can target it with `cancel({ requestId })`. Optional on the wire so legacy clients keep working — the server falls back to a server-generated id when the field is missing.",
    ),
});
