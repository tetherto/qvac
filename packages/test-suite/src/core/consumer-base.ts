...
export class ConsumerBase {
    ...
    if (this.callbacks.onBootstrap && !this.bootstrapped) {
      ...
      if (message.filteredTestIds && this.testDefinitions.size > 0) {
        ...
        for (const testId of message.filteredTestIds) {
          ...
          if (this.getTestSkipReason(def) !== null) {
  ...
  protected getTestSkipReason(definition: TestDefinition): string | null {
    ...
    // One matcher, used everywhere `skip.platforms` is read. The bootstrap
    // pass used to compare with `includes`, so a coarse entry like `mobile`
    // decided the run on `mobile-ios` but not what it downloaded: the leg
    // fetched gigabytes of diffusion and audiogen weights for tests it then
    // skipped. Two readings of one field is a bug waiting for whoever writes
    // the next rule.
