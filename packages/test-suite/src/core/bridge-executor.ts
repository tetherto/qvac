...
export class BridgeExecutor implements TestExecutor {
  ...
  async executeTest(
    ...
    // A client can only run a definition that carries a declarative body. That
    // is not a failure and it is not a platform skip: the test applies to this
    // client, the client simply has nothing to run yet.
    if (!definition?.steps?.length) {
      const reason = `${testId} has no steps; not migrated to the shared catalog yet`
