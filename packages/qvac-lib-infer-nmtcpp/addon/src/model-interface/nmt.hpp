...
// Large-scale “modernization” (RAII, refactors, casts, array replacements, etc.)
// risks altering runtime behavior or performance characteristics.
// To avoid regressions, we suppress clang-tidy in this translation unit/header.
// If future changes are made, please validate with thorough parity tests
// and benchmarking before removing these suppressions.
// NOLINTBEGIN
...
struct nmt_model {
    ...
    struct ggml_tensor *m_encoder_embeddings = nullptr; // encoder embeddings (shared)
    struct ggml_tensor *m_decoder_embeddings = nullptr; // decoder embeddings
    struct ggml_tensor *m_encoder_pos_emb = nullptr;    // encoder positional embeddings
    struct ggml_tensor *m_decoder_pos_emb = nullptr;    // decoder positional embeddings
    struct ggml_tensor *m_encoder_norm_w = nullptr;     // encoder final layer norm weight
    struct ggml_tensor *m_encoder_norm_b = nullptr;     // encoder final layer norm bias
    struct ggml_tensor *m_decoder_norm_w = nullptr;     // decoder final layer norm weight
    struct ggml_tensor *m_decoder_norm_b = nullptr;     // decoder final layer norm bias
    ...
    struct ggml_tensor *m_dec_layer_norm_b = nullptr; // decoder layernorm_embedding bias
...
template <typename A, typename B> struct nmt_pair {
    ...
    nmt_pair(const A &firstValue, const B &secondValue) : first(firstValue), second(secondValue) {
    }
...
struct nmt_decoder {
    ...
    int seek_delta = 0; // the window shift found so far based on the decoded timestamp tokens
    ...
    // new token probs, logits and logprobs after the last nmt_decode (1-dimensional array: [n_vocab])
...
struct nmt_state {
    ...
    nmt_decoder decoders[NMT_MAX_DECODERS]; // NOLINT(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays)
...
int nmt_get_runtime_stats(struct nmt_context *ctx, double *encodeTime, double *decodeTime, int *totalTokens);

void nmt_reset_runtime_stats(struct nmt_context *ctx);

void nmt_reset_state(struct nmt_context *ctx);

struct nmt_context *nmt_init_from_file_with_params(const char *pathModel, struct nmt_context_params params);
