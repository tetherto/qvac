 ...
 *
 * Evaluates translation quality using BLEU score and measures performance.
 * Reports both Native TPS (C++ model-internal) and End-to-End TPS (JS wall-clock)
 * in both batch and sequential modes. Uses FLORES devtest dataset format.
 *
 * Supports CPU and GPU execution:
 *   - Opus/IndicTrans (GGML backend): --use-gpu enables Metal (macOS) or CUDA (Linux) acceleration
 *   - Bergamot (intgemm backend): CPU-only, --use-gpu flag is ignored
 *
 * Usage:
 *   bare benchmarks/backend_evaluate_bare.js --model-path ./model/bergamot/enit/2025-11-26 --src-lang en --tgt-lang it
 *   bare benchmarks/backend_evaluate_bare.js --model-path ./qvac_models/en-it --model-type Opus --sequential
 *   bare benchmarks/backend_evaluate_bare.js --model-path ./qvac_models/en-it --model-type Opus --use-gpu
 */
...
async function main () {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--help':
        console.log(`
Usage: bare benchmarks/backend_evaluate_bare.js --model-path <path> [options]

Required:
  --model-path <path>   Path to model directory

Options:
  --model-type <type>   Model type: Bergamot, Opus, or IndicTrans (default: Bergamot)
  ...
  --use-gpu             Use GPU acceleration (Opus/IndicTrans only, requires Metal/CUDA)
  --help                Show this help

Device:
  GPU acceleration is supported for Opus and IndicTrans models (GGML backend).
  Bergamot uses intgemm (CPU-optimized integer quantized math) — --use-gpu is ignored.

Examples:
  bare benchmarks/backend_evaluate_bare.js --model-path ./model/bergamot/enit/2025-11-26
  bare benchmarks/backend_evaluate_bare.js --model-path ./qvac_models/en-it --model-type Opus
  bare benchmarks/backend_evaluate_bare.js --model-path ./qvac_models/en-it --model-type Opus --use-gpu
  bare benchmarks/backend_evaluate_bare.js --model-path ./model/bergamot/enit/2025-11-26 --max 50
        `)
        return
    }
  }
  ...
  if (!modelPath) {
    ...
    console.log('')
    console.log('Usage: bare benchmarks/backend_evaluate_bare.js --model-path <path> [options]')
    console.log('')
    console.log('Examples:')
    console.log('  bare benchmarks/backend_evaluate_bare.js --model-path ./model/bergamot/enit/2025-11-26')
    console.log('  bare benchmarks/backend_evaluate_bare.js --model-path ./qvac_models/en-it --model-type Opus')
    console.log('')
    console.log('Use --help for more options')
    process.exit(1)
  }

  // Warn if --use-gpu is used with Bergamot (it will be ignored by the engine)
  const isGgmlBackend = modelType === 'Opus' || modelType === 'IndicTrans'
  if (useGpu && !isGgmlBackend) {
    console.log('⚠️  --use-gpu is only effective for Opus/IndicTrans (GGML backend).')
    console.log('   Bergamot uses intgemm (CPU-optimized) — GPU flag will be ignored.')
    console.log('')
  }

  const deviceLabel = useGpu && isGgmlBackend ? 'GPU (Metal/CUDA)' : 'CPU'
  ...
  let translations = []
  let totalTokens = 0
  let nativeStats = null // C++ runtime stats (available in both batch and sequential mode)
  ...
  if (useBatch) {
    ...
    // Capture native C++ runtime stats from batch mode
    nativeStats = model.batchStats
  } else {
    ...
    for (let i = 0; i < sources.length; i++) {
      ...
      // Capture cumulative C++ native stats from each response
      // Stats are cumulative, so the last response has the totals
      if (response.stats) {
        nativeStats = response.stats
      }
  ...
  // End-to-End TPS (JS wall-clock): uses JS whitespace tokenization + Date.now() timing
  const e2eTokensPerSec = (totalTokens / translateTime) * 1000

  // Native TPS (C++): uses model-internal tokenization + chrono timing
  // Only available in sequential mode (run() API)
  const hasNativeStats = nativeStats && nativeStats.TPS !== undefined
  const nativeTps = hasNativeStats ? nativeStats.TPS : null
  const nativeTotalTokens = hasNativeStats ? nativeStats.totalTokens : null
  const nativeTotalTimeMs = hasNativeStats ? (nativeStats.totalTime * 1000) : null
  const nativeDecodeTimeMs = hasNativeStats ? (nativeStats.decodeTime * 1000) : null
  ...
  console.log(`Translate time: ${translateTime}ms (JS wall-clock)`)
  console.log('')
  console.log('--- Performance: End-to-End (JS wall-clock) ---')
  console.log(`Total tokens:   ${totalTokens} (whitespace tokenization)`)
  console.log(`Tokens/sec:     ${e2eTokensPerSec.toFixed(2)}`)
  console.log(`Ms/sentence:    ${(translateTime / translations.length).toFixed(2)}`)
  console.log('')

  if (hasNativeStats) {
    console.log('--- Performance: Native C++ (model-internal) ---')
    console.log(`Total tokens:   ${nativeTotalTokens} (model tokenizer)`)
    console.log(`Total time:     ${nativeTotalTimeMs.toFixed(2)}ms`)
    console.log(`Decode time:    ${nativeDecodeTimeMs.toFixed(2)}ms`)
    if (nativeStats.encodeTime !== undefined) {
      console.log(`Encode time:    ${(nativeStats.encodeTime * 1000).toFixed(2)}ms`)
    }
    if (nativeStats.TTFT !== undefined) {
      console.log(`TTFT:           ${nativeStats.TTFT.toFixed(2)}ms`)
    }
    console.log(`Tokens/sec:     ${nativeTps.toFixed(2)}`)
    console.log('')
    console.log('--- TPS Comparison ---')
    console.log(`Native TPS (C++):       ${nativeTps.toFixed(2)} tokens/sec`)
    console.log(`End-to-End TPS (JS):    ${e2eTokensPerSec.toFixed(2)} tokens/sec`)
    const overhead = nativeTps > 0 ? (((nativeTps - e2eTokensPerSec) / nativeTps) * 100) : 0
    console.log(`JS overhead:            ${overhead.toFixed(1)}%`)
    console.log('')
  } else {
    console.log('ℹ️  Native C++ stats not received from model.')
    console.log('')
  }
  ...
  const summary = {
    ...
    endToEnd: {
      ...
      tokensPerSec: parseFloat(e2eTokensPerSec.toFixed(2)),
      msPerSentence: parseFloat((translateTime / translations.length).toFixed(2))
    },
    native: hasNativeStats
      ? {
          totalTokens: nativeTotalTokens,
          totalTimeMs: parseFloat(nativeTotalTimeMs.toFixed(2)),
          decodeTimeMs: parseFloat(nativeDecodeTimeMs.toFixed(2)),
          encodeTimeMs: nativeStats.encodeTime !== undefined ? parseFloat((nativeStats.encodeTime * 1000).toFixed(2)) : undefined,
          ttftMs: nativeStats.TTFT !== undefined ? parseFloat(nativeStats.TTFT.toFixed(2)) : undefined,
          tokensPerSec: parseFloat(nativeTps.toFixed(2))
        }
      : null,
    loadTimeMs: loadTime,
