...
protected:
  ...
  ggml_backend_dev_t select(
    ...
    return nmtSelectGpuDevice(backend, true, requested, ordinal, "test",
                              allowDefaultOpenCl, {}, true);
  }
};
...
TEST_F(NmtGpuSelectionTest, AutomaticDedicatedGpuOutranksOpenClIntegratedGpu) {
  inventory = {{"OpenCL0", "OpenCL", GGML_BACKEND_DEVICE_TYPE_IGPU},
               {"Vulkan0", "Vulkan", GGML_BACKEND_DEVICE_TYPE_GPU}};
  EXPECT_EQ(nmtSelectGpuDevice(backend, true, {}, 0, "test", true),
            deviceGet(1));
  EXPECT_EQ(select({}, 0, true), deviceGet(0));
}
...
TEST_F(NmtGpuSelectionTest, CpuFallbackWarnsWithEveryRefusedGpuIdentity) {
  ...
  const auto selected = nmtSelectGpuDevice(backend, true, {}, 0, "refused-test",
                                           false, {}, false);
