...
import { videoError } from "./config";

export type VideoRotation = 0 | 90 | 180 | 270;

/** QuickTime/FFmpeg display matrices use fixed-point coefficients. Mirroring/shear are not rotation. */
export function displayRotation(data?: Uint8Array): VideoRotation {
  if (!data) return 0;
  if (data.byteLength !== 36) throw videoError(ERR_CODES.INVALID_VIDEO_INPUT, "Invalid display matrix");
  ...
  const close = (x: number, y: number) => Math.abs(x - y) < 0.0001;
  if (view.getInt32(8, true) !== 0 || view.getInt32(20, true) !== 0 || view.getInt32(32, true) !== 1073741824) {
    throw videoError(ERR_CODES.UNSUPPORTED_VIDEO, "Perspective display transforms are unsupported");
  }
  ...
  throw videoError(ERR_CODES.UNSUPPORTED_VIDEO, "Only unmirrored right-angle video rotation is supported");
}

export function rotateRgb(rgb: Uint8Array, width: number, height: number, rotation: VideoRotation) {
