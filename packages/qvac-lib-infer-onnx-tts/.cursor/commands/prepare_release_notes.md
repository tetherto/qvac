...
1. **Title**: `# QVAC TTS ONNX Addon v{VERSION} Release Notes`
...
```markdown
### TTS language configuration moved to constructor

The `language` option is now set during ONNXTTS initialization rather than at runtime. This ensures the model loads with the correct language configuration upfront and prevents runtime mismatches.
