...
template <>
void qvac_lib_inference_addon_tts::Addon::loadWeights(
    js_env_t* env, js_value_t* weightsData) {}

template <>
std::string qvac_lib_inference_addon_tts::Addon::getNextPiece(
    std::string& input, size_t lastPieceEnd) {
...
qvac_lib_inference_addon_tts::Addon::Addon(
    js_env_t* env, std::unordered_map<std::string, std::string> configMap,
    std::vector<float> referenceAudio, js_value_t* jsHandle,
...
template <>
js_value_t* createOutputData<std::vector<int16_t>>(
    js_env_t* env, const std::vector<int16_t>& data) {
  void* buffer = nullptr;
  js_value_t* arrayBuffer = nullptr;
  JS(js_create_arraybuffer(
      env, data.size() * sizeof(int16_t), &buffer, &arrayBuffer));

  js_value_t* typedArray = nullptr;
  JS(js_create_typedarray(
      env, js_int16array, data.size(), arrayBuffer, 0, &typedArray));
