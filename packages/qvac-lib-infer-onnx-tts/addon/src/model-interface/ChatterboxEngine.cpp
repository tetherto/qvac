...
template <typename T>
void copyFromTensor(
    const qvac::ttslib::chatterbox::OrtTensor& tensor, T* dest) {
  std::memcpy(
      dest, static_cast<T*>(tensor.data), getNumElements(tensor) * sizeof(T));
}
...
AudioResult ChatterboxEngine::synthesize(const std::string& text) {
  ...
  for (size_t i = 0; i < MAX_NEW_TOKENS; i++) {
    ...
    insertFromOrtTensorToVector(
        inputsEmbsTensor, inputsEmbs.data, inputsEmbs.data.begin());
    ...
    for (size_t i = 3; i < languageModelSession_->getInputNames().size(); i++) {
      ...
      std::memcpy(
          pastKeyValues[inputName].data.data(),
          outputTensor.data,
...
void ChatterboxEngine::runLanguageModelInfer(
  ...
  std::memcpy(
      inputsEmbsTensor.data,
      inputsEmbs.data.data(),
