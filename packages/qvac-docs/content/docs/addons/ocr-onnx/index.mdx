...
[Bare module](https://bare.pears.com) that adds support for OCR in QVAC using [ONNX runtime](https://onnxruntime.ai) as the inference engine.
...
```bash
npm install @qvac/ocr-onnx bare-process bare-path
```
</Step>
