...
2. [`textToSpeech()`](/sdk/api/latest/textToSpeech)
...
### Required inputs to `loadModel()`

When loading TTS models, `loadModel()` also requires:
- `configSrc`: [Piper voice configuration JSON that pairs with `.onnx` model](https://deepwiki.com/rhasspy/piper/3.2-voice-configuration) (`config.json` from voice bundle).
- `eSpeakDataPath`: [path to eSpeak NG data directory, used to convert text to phonemes before synthesis](https://man.archlinux.org/man/extra/espeak-ng/espeak-ng.1.en).

## Example

The following script shows an example of text-to-speech, saving the output to a WAV file and playing it back:

<Tabs>
<Tab value="js" label="JavaScript" default>
<WrapCode>

```js file=../dist/examples/text-to-speech.js title="text-to-speech.js" lineNumbers
import {
  loadModel,
  textToSpeech,
  unloadModel,
...
function createWav(audioBuffer, sampleRate = 22050, filename = "output.wav") {
  const audioData = int16ArrayToBuffer(audioBuffer);
...
```ts file=../examples/text-to-speech.ts title="text-to-speech.ts" lineNumbers
import {
  loadModel,
  textToSpeech,
  unloadModel,
