...
- Recommended model family: Piper
- Model format: ONNX (`*.onnx`)

### Required artifacts

When loading TTS models, `loadModel()` also requires:
- `configSrc`: [Piper voice configuration JSON that pairs with `.onnx` model](https://deepwiki.com/rhasspy/piper/3.2-voice-configuration).
