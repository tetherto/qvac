...
Blind relays help establish peer connections through NAT/firewalls by routing traffic through relay nodes. Use it for model downloads from [Hyperdrive](https://docs.pears.com/building-blocks/hyperdrive) and for delegated inference scenarios where peers may be behind restrictive network configurations.
...
<WrapCode>
```js file=../dist/examples/blind-relays.js title="blind-relays.js" lineNumbers
...
<WrapCode>
```ts file=../examples/blind-relays.ts title="blind-relays.ts" lineNumbers
// Set config path before importing SDK - the SDK loads config during initialization
// This example uses a config file that defines blind relay public keys for improved P2P connectivity
// Blind relays help establish connections through NAT/firewalls by acting as intermediaries
import { getConfigPath } from "./config/config-path.js";
...
The same `swarmRelays` config also applies to delegated inference (provider/consumer). Configure it via config file (or `QVAC_CONFIG_PATH`) and then start your provider/consumer as usual.
