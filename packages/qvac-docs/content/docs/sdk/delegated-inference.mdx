...
Delegation is configured at model-load time by passing a `delegate` object to [`loadModel()`](/sdk/api/latest/loadModel). The provider is started separately using [`startQVACProvider()`](/sdk/api/latest/startQVACProvider).
...
if (!topicHex) {
  console.error(
    "❌ Topic hex is required. Usage: node consumer.js <topic-hex> <provider-public-key> [consumer-seed]",
  );
  process.exit(1);
}
...
if (!providerPublicKey) {
  console.error(
    "❌ Provider public key is required. Usage: node consumer.js <topic-hex> <provider-public-key> [consumer-seed]",
  );
  process.exit(1);
}
...
try {
  ...
  console.log(`   node consumer.js ${topic} ${response.publicKey}`);
  console.log("");
  console.log("💡 To reproduce this provider identity:");
  console.log(`   node provider.js ${topic} ${seed || "<random-seed>"}`);
  if (!seed) {
    console.log(
  ...
  console.log(
    `   4. Run consumer with: node consumer.js ${topic} ${response.publicKey} <consumer-seed>`,
  );
...
- To stop a running provider and leave the topic, call [`stopQVACProvider()`](/sdk/api/latest/stopQVACProvider).
