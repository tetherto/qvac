...
sd_image_t resizeSdImage(const sd_image_t& src, int dstW, int dstH) {
  ...
  auto* buf = static_cast<uint8_t*>(
      malloc(static_cast<size_t>(dstW) * static_cast<size_t>(dstH) *
             static_cast<size_t>(ch)));
  if (!buf)
    return sd_image_t{};

  unsigned char* ok = stbir_resize_uint8_linear(
      src.data, srcW, srcH, srcW * ch, buf, dstW, dstH, dstW * ch,
  ...
  return {static_cast<uint32_t>(dstW), static_cast<uint32_t>(dstH),
          static_cast<uint32_t>(ch), buf};
}
