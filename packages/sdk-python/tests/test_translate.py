"""Unit tests for qvac.api.translate: source-language auto-detection for
LLM models, the NMT no-from/to path, and the stream/non-stream handle
...
from qvac.errors import TranslationFailedError
from qvac.language_detect import detect_one
...
def test_detect_one_names_and_undetermined():
    assert detect_one("Bonjour le monde, comment allez-vous?").language == "French"
    assert detect_one("").code == "und"
    assert detect_one("   ").language == "Undetermined"


# ---- translate: LLM auto-detection ----------------------------------------


async def test_llm_translate_autodetects_source_language():
    ...
    assert transport.sent["from"] == "French"


async def test_llm_translate_undetermined_raises():
    transport = FakeTransport()
    with pytest.raises(TranslationFailedError, match="from_"):
        api.translate(
            transport, model_id="m-1", text="   ", to="German", model_type="llm"
        )


async def test_llm_translate_explicit_from_skips_detection():
    ...
    assert transport.sent["from"] == "Klingon"
