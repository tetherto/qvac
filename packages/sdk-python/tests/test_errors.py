"""Unit tests for qvac.errors: envelope reconstruction and the class
...
def test_flat_reexports_the_same_error_classes():
    # The flat `qvac` surface re-exports the same class objects as
    # `qvac.errors`; identity must hold so except-clauses catch regardless of
    # import path.
    import qvac
    from qvac import errors

    assert qvac.CancelFailedError is errors.CancelFailedError
    assert qvac.ModelUnloadFailedError is errors.ModelUnloadFailedError
...
def test_reconstructor_names_are_known_sdk_error_codes():
    ...
    from qvac._generated.error_codes import (
        CLIENT_ERROR_CODES,
        REGISTRY_ERROR_CODES,
        SERVER_ERROR_CODES,
    )
    from qvac.errors import _RECONSTRUCTORS
...
def test_transport_json_or_raise_uses_reconstruction():
    from qvac.bare_rpc_transport import _json_or_raise
