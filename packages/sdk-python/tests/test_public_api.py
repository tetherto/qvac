...
Fails if the flat `qvac` surface stops covering a generated contract method,
...
import qvac
from qvac import methods as _methods


def test_every_generated_method_is_reachable_from_flat_qvac():
    missing = [name for name in _methods.__all__ if not hasattr(qvac, name)]
...
def test_ergonomic_wrappers_shadow_generated_stubs():
    ...
    assert qvac.load_model.__module__ == "qvac._api"
    assert qvac.unload_model.__module__ == "qvac._api"
    assert qvac.cancel.__module__ == "qvac._api"
    assert qvac.delete_cache.__module__ == "qvac._api"
    assert qvac.translate.__module__ == "qvac._api"
    assert qvac.model_registry_list.__module__ == "qvac._api"
    assert qvac.model_registry_search.__module__ == "qvac._api"
    assert qvac.model_registry_get_model.__module__ == "qvac._api"
    assert qvac.completion_orchestrate.__module__ == "qvac._completion"
    # completion is a wrapper with no generated namesake (completionStream is).
    assert qvac.completion.__module__ == "qvac._completion"
