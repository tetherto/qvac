...
def render_model_type_maps_module(maps: dict[str, dict[str, str]]) -> str:
    ...
    plain dict literals. `qvac.model_types`'s inference reads these instead of
...
def render_error_codes_module(codes: dict[str, dict[str, int]]) -> str:
    ...
    dict literals. `qvac.errors` reads these instead of hand-copying the
...
def render_sdk_version_module(version: str) -> str:
    ...
    shapes match it), so `Client` and `python -m qvac install-worker` pin to it.
...
def sort_imports_with_ruff(path: Path) -> None:
    ...
    # defaults, and classifies the first-party `qvac` import as third-party --
...
def render_plugin_module(contract: dict) -> str:
    ...
    lines = [
        ...
        "over qvac.invoke_plugin / invoke_plugin_stream.",
        '"""',
        "",
        models_source,
        "",
        ...
        "",
        "from qvac import invoke_plugin, invoke_plugin_stream  # noqa: E402",
        "from qvac._transport import Transport  # noqa: E402",
        "",
