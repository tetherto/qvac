...
`bare_path` still derives the Bare binary from a later tier):
  ...
  5. A thin wheel's fetched worker: `python -m qvac install-worker` puts the
...
def managed_worker_prefix() -> Path:
    """npm --prefix dir `python -m qvac install-worker` installs into, scoped by
...
def _warn_on_version_mismatch(sdk_root: Path) -> None:
    ...
    if version and version != SDK_VERSION:
        warnings.warn(
            ...
            f"`python -m qvac install-worker` for the matching version.",
            RuntimeWarning,
            stacklevel=4,
        )
...
def _resolve_command(
    ...
    # Thin wheel: resolve a worker fetched by `python -m qvac install-worker`
    ...
    raise WorkerNotFoundError(
        f"no worker found -- run `python -m qvac install-worker` to fetch "
