...
class SyncClient:
    ...
    (`SyncClient(transport=...)`) or let it own a `qvac.Client`
    (`SyncClient(sdk_dir=...)` etc. -- any `qvac.Client` kwargs), then call
    ...
    def load_model(self, **kwargs: Any) -> str:
        ...
        return self._run(api.load_model(self._transport, **kwargs))

    def unload_model(self, model_id: str, clear_storage: bool = False) -> None:
        from . import api

        self._run(api.unload_model(self._transport, model_id, clear_storage))
    ...
    def text_to_speech(self, model_id: str, text: str) -> Any:
        ...
        return numpy.asarray(self._run(run()), dtype=numpy.float32)
