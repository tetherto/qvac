...
- `JsAsyncTask` now releases the work closure (and its captures) in the completion callback, *before* settling the promise, instead of in the libuv close callback afterwards. The empty-snapshot skip above only covered idle cancels; a real cancellation still pinned the addon — and its model — past the point where `cancel()`'s promise resolved, so an `unload()` awaiting it still handed a live model to the next `load()` and the iOS jetsam kill just moved one test later. Awaiting any `JsAsyncTask` promise now means its captures are already gone. The release still happens on the loop thread and still ahead of `js_finish_deferred_teardown_callback`, so the 1.3.2 guarantees (never destroyed on the detached worker, env pinned throughout) are unchanged; the close callback keeps releasing the work for the paths that never reach completion (a worker thread that failed to start). Note the reordering is observable: destructor side effects of captures now run before the promise settles rather than after.
