...
// Generates test/mobile/integration.auto.cjs — one `run<Name>` wrapper per
// test/integration/*.test.js — so the qvac-test-addon-mobile harness can invoke
// each desktop integration test as an independent on-device test. Mirrors
// packages/llm-llamacpp/scripts/generate-mobile-integration-tests.js, minus the
// benchmark-shard bootstrap (inference-addon-cpp has no benchmark shards).
...
// test-groups.json is optional for this addon (single group per platform at the
// current test count). When present, every generated runner must be assigned to
// a group so no test is silently dropped from the Device Farm run.
