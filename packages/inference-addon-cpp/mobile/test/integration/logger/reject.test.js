...
//
// Source: tests/integration_js/logger/reject.test.js
...
test('setLogger from a second live env is rejected', async (t) => {
  ...
  const worker = new Thread(require.resolve('./worker-set-hold.js'), { data: flags.buffer })
...
test('sequential handoff across envs keeps working', async (t) => {
  ...
  async function runOwner() {
    const flags = new Int32Array(new SharedArrayBuffer(2 * Int32Array.BYTES_PER_ELEMENT))
    const worker = new Thread(require.resolve('./worker-set-release.js'), { data: flags.buffer })
    worker.join()
...
test('releaseLogger from a non-owner env is a no-op', async (t) => {
  ...
  const worker = new Thread(require.resolve('./worker-owner-guard.js'), { data: flags.buffer })
...
test('new env installs the logger after a prior env torn down without releaseLogger', async (t) => {
  ...
  const first = new Thread(require.resolve('./worker-set-norelease.js'), { data: firstFlags.buffer })
  ...
  const second = new Thread(require.resolve('./worker-set-release.js'), { data: secondFlags.buffer })
  second.join()
