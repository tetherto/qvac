...
public:
  ...
  initializeProcessingThread(std::shared_ptr<OutputQueue> outputQueue) final {
    ...
    JS(js_add_teardown_callback(state_->env, onEnvTeardown, state_));
  }
  ...
  void stop() final {
    State* s = std::exchange(state_, nullptr);
    if (s == nullptr) {
      return;
    }
    s->stopped = true;
    if (s->asyncHandle == nullptr) {
      ...
      // path on the JS thread, nothing queued and no handle to close.
    ...
    if (std::this_thread::get_id() == s->loopThreadId) {
      ...
      drainAndTeardown(s, envUsable, envUsable);
      return;
    }
    ...
    if (s->drained) {
      return; // env teardown hook already flushed; the close callback owns s
    }
    if (s->drainPending) {
    ...
    if (!s->envAlive) {
      ...
      // impossible and no close callback will ever run, so free the state
      // here and leak the uv handle (its loop died with the env).
      delete s;
      return;
    }
...
private:
  ...
  /// close callback frees the State. @p removeHook is false when called from
  /// the env teardown hook itself: the hook is already being consumed and the
  /// env teardown machinery must not be mutated mid-iteration.
  static void drainAndTeardown(State* s, bool envUsable, bool removeHook) {
    ...
    uv_close(
        ...
        [](uv_handle_t* handle) {
          ...
          {
            // An off-thread stop() may still be leaving its wait; taking the
            // mutex orders that release before the delete.
            std::scoped_lock lk{state->mtx};
          }
          delete reinterpret_cast<uv_async_t*>(handle);
          delete state;
