...
- **Submission:** `addon.runJob(input)` mints the job's `JobId` (monotonic, never reused), pushes the `{input, id}` pair onto a FIFO queue (`queued_`), notifies one worker, and returns the id; admission is rejected (`std::nullopt`) once `in-flight + queued` would exceed `maxConcurrency + queueCapacity` (back-pressure), or while an exclusive job (e.g. a finetune, via `runExclusiveJob`) holds the model.
...
- **Cancellation:** `cancel(id)` unlinks a still-queued job in O(1) (via `queuedIndex_`) and queues its terminal "Job cancelled" error directly; an in-flight job instead routes to the model's `IModelCancelById::cancelById(id)`. `cancelAll()` drops every queued job the same way, then cancels the in-flight set — per id through `cancelById` when the model offers it, falling back to one indiscriminate whole-model `cancel()` only for models that implement nothing finer. The JS cancel-all is snapshot-based instead: the binding captures `liveJobIds()` on the JS thread and later cancels exactly that set via `cancelJobs(ids)`, so jobs admitted after the request are never touched.
- **When a cancel returns:** every entry point (`cancel(id)`, `cancelAll()`, `cancelJobs(ids)`) returns only once each job it delivered a model-side cancel for has fully left the scheduler — queue and in-flight set both, meaning the admission slot is released. A returned cancel therefore means "the job is gone": `activeJobs()` no longer counts it and an immediate follow-up admission cannot be refused as busy. Cancels that reach no model at all (the model implements neither `cancelById` nor `cancel`) return immediately, since there is nothing whose completion could be awaited. The corollary is that a cancel entry point must never be called from a scheduler worker thread — it would wait on itself.
- **Teardown:** the destructor fires the model's cancel hook (only while a job is actually in flight) so a worker inside a long `process()` returns promptly, joins every worker, then drains and fails any job still queued so no admitted job is ever left without a terminal event. Unlike the cancel entry points above, teardown never waits for a slot to be released.
