  ...
  async findBy(params = {}) {
    ...
    if (engine) {
      ...
      models = await this.db
        .find(`@${QVAC_MAIN_REGISTRY}/models-by-engine-quantization`, query)
        .toArray()
    } else if (quantization) {
      models = await this.db
        .find(`@${QVAC_MAIN_REGISTRY}/models-by-quantization`, {
          gte: { quantization },
      ...
      models = await this.db
        .find(`@${QVAC_MAIN_REGISTRY}/models-by-name`, {
          gte: name,
          lte: name + '\uffff'
        })
        .toArray()
    ...
    if (name && engine) {
      models = models.filter((m) =>
        m.path?.split('/').pop()?.toLowerCase().includes(name.toLowerCase())
      )
    }
