...
The description lives in the same Hyperblobs core as the weights, under the same
Merkle verification, and the record points at it through the optional
`fitBlobBinding` field. Clients that do not know the field ignore it; records
...
Records without a pointer are filled in with `npm run fill:fit-blobs`, which
reads each artifact back out of the blob core and never re-uploads weights.
