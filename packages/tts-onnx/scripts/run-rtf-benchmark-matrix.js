 ...
 * setting QVAC_ONNX_TTS_BENCHMARK_* env vars for each entry.
 *
 * The matrix is read from QVAC_ONNX_TTS_BENCHMARK_MATRIX_JSON (stringified array).
 ...
 *   }
 *
 * If QVAC_ONNX_TTS_BENCHMARK_MATRIX_JSON is empty, a small default matrix
 * (all 3 engines, CPU-only) is run.
 */
...
// finish in 1-6 minutes; QVAC_ONNX_TTS_BENCHMARK_ENTRY_TIMEOUT_MS lets CI
// override per-runner if needed.
const DEFAULT_ENTRY_TIMEOUT_MS = 10 * 60 * 1000 // 10 minutes
function getEntryTimeoutMs () {
  const raw = process.env.QVAC_ONNX_TTS_BENCHMARK_ENTRY_TIMEOUT_MS
...
function parseMatrixConfig () {
  const raw = process.env.QVAC_ONNX_TTS_BENCHMARK_MATRIX_JSON
  if (!raw) {
    return [
  ...
  if (!Array.isArray(parsed) || parsed.length === 0) {
    throw new Error('QVAC_ONNX_TTS_BENCHMARK_MATRIX_JSON must be a non-empty JSON array')
  }
  return parsed
}
...
function buildEnv (entry, index) {
  ...
  const env = {
    ...process.env,
    QVAC_ONNX_TTS_BENCHMARK_ENGINE: String(entry.engine || 'chatterbox-en'),
    QVAC_ONNX_TTS_BENCHMARK_VARIANT: String(entry.variant || process.env.QVAC_ONNX_TTS_BENCHMARK_VARIANT || 'q4'),
    QVAC_ONNX_TTS_BENCHMARK_USE_GPU: normalizeBoolean(entry.useGPU) ? 'true' : 'false',
    QVAC_ONNX_TTS_BENCHMARK_LABEL: label,
    QVAC_ONNX_TTS_BENCHMARK_BACKEND:
      entry.backendHint !== undefined
        ? String(entry.backendHint)
        : (process.env.QVAC_ONNX_TTS_BENCHMARK_BACKEND || ''),
    QVAC_ONNX_TTS_BENCHMARK_DEVICE:
      entry.deviceLabel !== undefined
        ? String(entry.deviceLabel)
        : (process.env.QVAC_ONNX_TTS_BENCHMARK_DEVICE || ''),
    QVAC_ONNX_TTS_BENCHMARK_RUNNER:
      entry.runnerLabel !== undefined
        ? String(entry.runnerLabel)
        : (process.env.QVAC_ONNX_TTS_BENCHMARK_RUNNER || '')
  }

  if (entry.numWarmup !== undefined) {
    env.QVAC_ONNX_TTS_BENCHMARK_WARMUP_RUNS = String(entry.numWarmup)
  }
  if (entry.numRuns !== undefined) {
    env.QVAC_ONNX_TTS_BENCHMARK_RUNS = String(entry.numRuns)
  }
  if (entry.numThreads !== undefined) {
    env.QVAC_ONNX_TTS_BENCHMARK_NUM_THREADS = String(entry.numThreads)
  }
  if (entry.rtfUpperBound !== undefined) {
    env.QVAC_ONNX_TTS_BENCHMARK_RTF_UPPER_BOUND = String(entry.rtfUpperBound)
  }
...
function runEntry (pkgDir, entry, index, matrixLen) {
  ...
  console.log(`  engine:     ${env.QVAC_ONNX_TTS_BENCHMARK_ENGINE}`)
  console.log(`  variant:    ${env.QVAC_ONNX_TTS_BENCHMARK_VARIANT}`)
  console.log(`  useGPU:     ${env.QVAC_ONNX_TTS_BENCHMARK_USE_GPU}`)
  console.log(`  backend:    ${env.QVAC_ONNX_TTS_BENCHMARK_BACKEND || 'default'}`)
  console.log(`  label:      ${env.QVAC_ONNX_TTS_BENCHMARK_LABEL}`)
  console.log('='.repeat(70))
  ...
  if (result.error && result.error.code === 'ETIMEDOUT') {
    ...
    throw new Error(
      `Benchmark entry ${index + 1} (${env.QVAC_ONNX_TTS_BENCHMARK_LABEL}) ` +
      `exceeded watchdog timeout of ${timeoutMs}ms (elapsed ${elapsedMs}ms) ` +
      'and was terminated. Set QVAC_ONNX_TTS_BENCHMARK_ENTRY_TIMEOUT_MS to override.'
    )
  }
  ...
  if (result.status !== 0) {
    throw new Error(`Benchmark entry ${index + 1} (${env.QVAC_ONNX_TTS_BENCHMARK_LABEL}) exited with status ${result.status}`)
  }
}
