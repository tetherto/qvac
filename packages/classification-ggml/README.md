...
| `threads`      | `number`            | libggml default (`std::thread::hardware_concurrency`) | Upper bound on CPU worker threads the GGML compute graph may use. Set lower (e.g. `2`) on battery-constrained mobile devices; set higher on servers. Must be a positive integer. **Note:** validated on every platform, but on Android the value is currently a no-op at runtime — see the `threads` bullet under [Performance / What affects classify() latency](#what-affects-classify-latency).                                                                                                                                                       |
...
- **threads** — capped at `hardware_concurrency` by default on desktop and iOS. Lowering it trades latency for battery or contention with other addons (LLM, whisper) running on the same device. **Android is a special case**: with `qvac-fabric`'s `GGML_CPU_ALL_VARIANTS=ON` the CPU backend ships as per-microarch MODULE `.so` files loaded via `dlopen`, and the `set_n_threads` symbol lives inside those variant `.so`s rather than `libggml-base`. The `.bare` therefore can't statically link it, so the constructor's `threads` value is validated fail-fast like everywhere else but is silently ignored at runtime — Android falls back to libggml's default thread pool.
