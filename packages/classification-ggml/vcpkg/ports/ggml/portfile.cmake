# ggml vcpkg overlay port — classification-ggml addon
#
# Forks tetherto/qvac-registry-vcpkg/ports/ggml @ port-version 6 to add an
# addon-specific backend filename prefix (libqvac-classif-ggml-*) and a
# defensive #ifdef GGML_BACKEND_DL guard around ggml_backend_load_all_from_path.
# The two changes together prevent cross-addon backend collisions on Android
# (where multiple qvac native addons may coexist in one process) and stop
# accidental dlopen of system-installed libggml-* libraries from llama.cpp
# installations or other ggml consumers.
#
# Mirrors the per-addon namespace pattern qvac-fabric-llm.cpp#95 established
# for @qvac/llm-llamacpp (qvac-ggml-) and that the upstream fork already
# applied for diffusion-cpp (qvac-diffusion-ggml-). Once the registry adopts a
# per-consumer-configurable prefix, this overlay can be removed and the pin
# bumped back to whatever the registry ships.
...
vcpkg_from_github(
    ...
    HEAD_REF 2026-01-30
    PATCHES
        ggml-qvac-classif-backend-prefix.patch
)
...
if(VCPKG_TARGET_IS_ANDROID AND "vulkan" IN_LIST FEATURES)
    ...
    # headers at src/vulkan/*.hpp makes #include resolve.
...
# MODULE target but does NOT install them via cmake install(). The three glob
# patterns cover (a) our addon's libqvac-classif-ggml-* prefix, (b) the LLM
# addon's libqvac-ggml-* prefix it ships via qvac-fabric-llm.cpp#95, and (c)
# the diffusion-cpp prefix the upstream fork hardcoded; only the first applies
# in this addon's build but the others are kept for forward-compat with any
# future shared-build scenarios.
if(VCPKG_TARGET_IS_ANDROID)
    file(GLOB _backend_sos
        "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/bin/libqvac-classif-ggml-*.so"
