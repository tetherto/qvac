...
TEST_F(CacheManagementTest, CacheClearedWhenNoSessionMessage) {
  ...
  EXPECT_NO_THROW({
    std::string saveOutput =
        std::any_cast<std::string>(model->process(saveInput));
    EXPECT_EQ(saveOutput.length(), 0);
    auto statsSave = model->runtimeStats();
    EXPECT_GE(statsSave.size(), 0);
  });
...
TEST_F(CacheManagementTest, CacheClearedWhenSwitchingToDifferentCache) {
  ...
  std::string saveInput1 =
      R"([{"role": "session", "content": "test_session1.bin"}, {"role": "session", "content": "save"}])";
  EXPECT_NO_THROW({
    std::string saveOutput1 =
        std::any_cast<std::string>(model->process(saveInput1));
  ...
  std::string saveInput2 =
      R"([{"role": "session", "content": "test_session2.bin"}, {"role": "session", "content": "save"}])";
  EXPECT_NO_THROW({
    std::string saveOutput2 =
        std::any_cast<std::string>(model->process(saveInput2));
    EXPECT_EQ(saveOutput2.length(), 0);
    auto statsSave2 = model->runtimeStats();
    EXPECT_GE(statsSave2.size(), 0);
  });
...
TEST_F(CacheManagementTest, GetTokensCommand) {
  ...
  EXPECT_NO_THROW({
    std::string output =
        std::any_cast<std::string>(model->process(getTokensInput));
...
TEST_F(CacheManagementTest, GetTokensCommandAfterDisable) {
  ...
  EXPECT_NO_THROW({
    std::string disableOutput =
        std::any_cast<std::string>(model->process(disableInput));
...
TEST_F(CacheManagementTest, GetTokensCommandAfterReset) {
  ...
  EXPECT_NO_THROW({
    std::string resetOutput =
        std::any_cast<std::string>(model->process(resetInput));
...
TEST_F(CacheManagementTest, CacheTokensExceedContextSize) {
  ...
  EXPECT_NO_THROW({
    std::string output1 =
        std::any_cast<std::string>(model_large->process(input1));
    EXPECT_GE(output1.length(), 0);
  ...
  EXPECT_NO_THROW({
    std::string output2 =
        std::any_cast<std::string>(model_large->process(input2));
    EXPECT_GE(output2.length(), 0);
  ...
  EXPECT_NO_THROW({
    std::string output3 =
        std::any_cast<std::string>(model_large->process(input3));
    EXPECT_GE(output3.length(), 0);
  ...
  EXPECT_NO_THROW({
    std::string output4 =
        std::any_cast<std::string>(model_large->process(input4));
  ...
  EXPECT_NO_THROW({
    std::string output5 =
        std::any_cast<std::string>(model_large->process(input5));
  ...
  EXPECT_NO_THROW({
    std::string saveOutput =
        std::any_cast<std::string>(model_large->process(saveInput));
    EXPECT_EQ(saveOutput.length(), 0);
  ...
  EXPECT_THROW(
      {
        std::string output =
            std::any_cast<std::string>(model_small->process(loadInput));
      },
      qvac_errors::StatusError);
}
