// Regression tests for the stale cancel flag bug in JobRunner
// (addon-cpp v1.1.2).
// cancel() calls modelCancel_->cancel() unconditionally when
...
TEST_F(CancelStaleFlagTest, CancelQueuedJob_DoesNotPoisonNextJob) {
  auto [addon, output, modelPtr] = createTestAddon(std::chrono::milliseconds{200});
  (void)output;
...
TEST_F(CancelStaleFlagTest, CancelActiveJob_StopsModelWithoutPoisoning) {
  auto [addon, output, modelPtr] = createTestAddon(std::chrono::milliseconds{5000});
  (void)output;
...
TEST_F(CancelStaleFlagTest, RepeatedCancelThenRun_NeverPoisons) {
  auto [addon, output, modelPtr] = createTestAddon(std::chrono::milliseconds{100});
  (void)output;
  ...
  for (int i = 0; i < kIterations; ++i) {
    ...
    EXPECT_FALSE(modelPtr->wasLastRunAborted())
        << "Iteration " << i << ": follow-up was poisoned by stale cancel flag.";
  }
}
