...
bool isIntegratedGpu(const DeviceDescription& devDescr) {
  const std::string& desc = devDescr.gpuDescription;
  return desc.find("intel") != std::string::npos ||
         desc.find("uhd") != std::string::npos ||
         desc.find("iris") != std::string::npos ||
         desc.find("apu") != std::string::npos ||
         desc.find("radeon vega") != std::string::npos;
}
...
bool shouldProcessDevice(
  ...
  const bool INTEGRATED_GPU =
      ...
      isIntegratedGpu(DEV_DESCR);
  const bool DEDICATED_GPU =
      MAIN_GPU_TYPE.has_value() &&
