...
title: "@qvac/sdk"
titleStyle: code
description: Release notes — v0.5.0 (latest)
---

## Highlights

`@qvac/sdk` v0.5.0 introduces a streamlined configuration system, powerful new APIs for batch embeddings and MCP tool integration, and a unified logging experience across all SDK components. We've also improved Android compatibility and fixed several critical audio processing issues.
...
- **Unified addon logging**: all native addons (Whisper, TTS, NMT) now use the same logging infrastructure, providing consistent log output across the entire SDK.
- **Android 16KB page size compatibility:** dependencies have been upgraded to comply with Android's 16KB page size requirement, ensuring compatibility with newer Android devices.
- **`bare-ffmpeg` cecoder:** the SDK now uses `bare-ffmpeg` for audio decoding, improving compatibility and performance.
...
- Remove all usages of `setConfig()`
- Add a `qvac.config.{json,js,ts}` file to your project root
- Optionally set `QVAC_CONFIG_PATH` for custom config locations