...
const { isMobile, platform, getImagePath, ensureDoctrModels, safeUnload } = require('./utils')
...
if (isWindows) {
  test('DocTR lifecycle - skipped on Windows (memory constraints)', function (t) {
    t.comment('ONNX Runtime on Windows does not release memory between sessions')
    t.comment('Lifecycle tests (7 load/unload cycles) run on macOS and Linux CI')
    t.pass('skipped')
  })
} else {
  let DOCTR_DETECTOR
