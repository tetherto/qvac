...
The package is identified by the `<package-name>` argument, which is the directory name under `packages/` (e.g., `ocr-onnx`, `qvac-sdk`, `onnx-tts`). The skill reads `packages/<package-name>/package.json` to determine the package type and npm scope.
...
Where:
- `<package-name>` is the directory name under `packages/` (e.g., `ocr-onnx`, `onnx-tts`)
