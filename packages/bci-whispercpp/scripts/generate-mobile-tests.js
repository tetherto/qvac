...
//   1. Runs scripts/download-models.js to fetch model binaries from the
//      QVAC registry and fixture binaries from the release asset.
...
function runDownloadScript () {
  ...
  const result = spawnSync('node', [DOWNLOAD_SCRIPT], {
    cwd: PACKAGE_DIR,
    stdio: 'inherit'
  })
  if (result.status !== 0) {
