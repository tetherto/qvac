...
export interface BciFitRequest {
  /** Absolute path to the BCI model, or to the registry's weightless copy. */
  modelPath: string
...
export interface BciFitResult {
   ...
   * fitter, so this is a file size rather than a measured allocation and is
   * not counted in `deviceBytes` or `hostBytes`.
   */
