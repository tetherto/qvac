...
import { requiredMobileHosts, type HarnessContribution } from './types.ts'
...
export async function finalizeHarnessStandalone(
  projectRoot: string,
  ...
  await writeFile(
    path.join(qvacDirectory, 'harness-stack.validation.json'),
    `${JSON.stringify(validation, null, 2)}\n`
  )
  if (!validation.ok) {
    throw new Error(
      `Harness standalone validation failed:\n${validation.errors.map((error) => `- ${error}`).join('\n')}`
    )
  }
  await writeFile(
...
export function validateStandaloneContribution(contribution: HarnessContribution): ValidationReport {
  const errors: string[] = []
...
export async function installBareKitLinkerAdaptation(projectRoot: string) {
  ...
  if (!packageRoot) {
    throw new Error(
      'Standalone Harness Expo plugin requires react-native-bare-kit to prepare native linking'
    )
  }
