...
var syncReactNativeHosts = [...readTargetHosts()];
function createSyncReactNativeDescriptor() {
  const require2 = createRequire(import.meta.url);
  const packageRoot = path.dirname(require2.resolve("@qvac/sync/package"));
  const generatedDirectory = path.join(packageRoot, "generated", "react-native");
  return {
    entryPath: path.join(packageRoot, "mobile-entry.ts"),
    harnessPath: path.join(generatedDirectory, "sync.js"),
    metadataPath: path.join(generatedDirectory, "sync.metadata.json"),
    contract: "qvac.sync",
    protocolVersion: 1,
...
async function buildSyncReactNativeBundle({
  ...
  const harnessPath = path.join(buildDirectory, "sync.js");
  const metadataPath = path.join(buildDirectory, "sync.metadata.json");
...
// lib/expo/contribution.ts
async function composeSyncContribution(projectRoot, buildResult, options = {}) {
  ...
  const destination = path3.join(projectRoot, "qvac", "contributions", "sync.json");
  await mkdir2(path3.dirname(destination), { recursive: true });
  await writeFile2(destination, `${JSON.stringify(contribution, null, 2)}
`);
  return contribution;
}
async function readAndValidateSyncMetadata(metadataPath) {
  let parsed;
  try {
...
function validateSyncMetadataShape(metadata) {
  ...
  if (typeof bundleId !== "string" || bundleId.length === 0 || contract !== "qvac.sync" || protocolVersion !== 1 || !Array.isArray(hosts) || !hosts.every((host) => typeof host === "string") || !requiredMobileHosts.every((host) => hosts.includes(host)) || !Array.isArray(nativeAddons) || !nativeAddons.every((addon) => typeof addon === "string") || !isValidPackages(packages)) {
    throw new Error("Malformed Sync metadata.");
  }
  return {
    bundleId,
...
async function finalizeSyncStandalone(projectRoot, contribution) {
  ...
  await writeFile3(path4.join(qvacDirectory, "sync-stack.validation.json"), `${JSON.stringify(validation, null, 2)}
`);
  if (!validation.ok) {
...
function validateStandaloneContribution(contribution) {
  ...
  if (contribution.packageName !== "@qvac/sync")
    errors.push("Contribution package name must be @qvac/sync");
  if (contribution.contract !== "qvac.sync")
    errors.push("Contribution contract must be qvac.sync");
...
async function installBareKitLinkerAdaptation(projectRoot) {
  ...
  if (!packageRoot) {
    throw new Error("Standalone Sync Expo plugin requires react-native-bare-kit to prepare native linking");
  }
...
// lib/expo/plugin.ts
var pluginId = "@qvac/sync/expo-plugin";
var buildRunOnceId = "@qvac/sync/expo-plugin/build";
function createSyncExpoPlugin(options = {}) {
  ...
  function withSyncExpoPlugin(config) {
  ...
  async function composeOnce(context) {
    ...
    const work = (async () => {
      ...
      if (mode === "standalone")
        await finalizeSyncStandalone(projectRoot, contribution);
    })().catch((error) => {
...
// expo-plugin.ts
var expo_plugin_default = createSyncExpoPlugin();
export {
