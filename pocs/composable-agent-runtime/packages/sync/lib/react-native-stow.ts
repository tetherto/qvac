...
export const syncReactNativeHosts = [...readTargetHosts()] as const

export interface SyncReactNativeDescriptor {
...
interface BuildSyncReactNativeBundleOptions {
  readonly outputDirectory?: string
}

export function createSyncReactNativeDescriptor(): SyncReactNativeDescriptor {
  const require = createRequire(import.meta.url)
  const packageRoot = path.dirname(require.resolve('@qvac/sync/package'))
...
export async function buildSyncReactNativeBundle({
  outputDirectory
}: BuildSyncReactNativeBundleOptions = {}) {
