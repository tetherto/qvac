...
Assistant exposes three independent readiness gates:

1. **State ready.** Local Sync storage and stable device identity are available. State operations and local watches depend only on this gate.
2. **Execution ready.** Harness is started, compatible, and connected to its state endpoint. `assistant.ready()` retains this meaning and does not load SDK or models.
3. **Model ready.** The SDK runtime is compatible and the requested model is provisioned and loaded. This readiness is keyed by model rather than represented by one global flag.

`assistant.run()` waits for execution readiness and loads its requested model. An optional model-preparation API may warm that path without being required for normal use.

State and execution readiness do not depend on DHT reachability or the presence of peers. Sync reports connectivity separately; no peers and no network are healthy offline states, not startup failures. Concurrent calls share the same readiness attempt. Readiness follows runtime generations, so calls during replacement wait for the new generation or fail with its terminal cause. Sync restarts preserve device identity, model failures do not make local state unavailable, and interrupted inference is never silently replayed.
...
Package-owned worker lifecycle and parent supervision are separate
responsibilities. Sync, Harness, and SDK each own their worker artifacts,
`bare-stow` or platform-specific assembly, public client startup adapter,
readiness checks, and normalization of platform-specific death signals. A
composing parent uses that package-owned lifecycle unit rather than building a
worker or receiving a worker entry path.

The parent still owns lifecycle policy for its subtree. Assistant applies
dependency ordering, restart budgets, suspension, and escalation to the Sync
and Harness lifecycle units. Harness applies the same policy to the SDK
lifecycle unit. A direct package consumer becomes the root lifecycle owner for
that package. This keeps packaging and launch mechanics with the worker-owning
package without bypassing the hierarchical supervision model.
...
Sync starts independently and makes local state and identity available through the state readiness gate before Harness or inference is required. `assistant.ready()` resolves when Harness is compatible and connected to the current Sync state endpoint. SDK and model readiness remain lazy and per-model. None of these local gates waits for DHT discovery or peers.
...
| Model preparation or load fails | The requested model remains unavailable, while local state and other model readiness remain independent. |
...
Every sidecar package owns one concise declarative source of truth for its versioned RPC contract. Method definitions and their request and response runtime schemas live together. Hyperschema codecs, HRPC definitions, typed clients, typed handler interfaces, client/server bindings, and capability metadata are generated from that source. Handlers remain ordinary functions, and persistence schemas remain separate from RPC contracts.

The exact authoring API and generator implementation are separate design choices. Generated client exports must run in Node/Bun, Electron, Expo/React Native/Hermes, and Bare-compatible hosts. Server entries remain Bare-only. Generic structural types may be repeated between packages rather than introducing a shared protocol dependency.
...
- cold state-readiness and execution-readiness time offline;
...
| --- | --- | --- |
| **S1 - Mobile three-runtime topology** | Minimal mobile app with Sync, Harness, and SDK in three Bare runtimes on supported mobile hosts | All start independently; RPC requests cross each boundary; each child restarts, reconnects, and renegotiates where the platform provides an isolation boundary; suspend/resume works; addons load after SDK restart. |
| **S2 - P9 footprint baseline** | S1 on a named constrained mobile reference device | Record cold offline state-readiness and execution-readiness time, incremental/total RSS, peak model-load memory, application/bundle/native binary size, background retained memory, and resume latency. Produce proposed numeric budgets for approval and CI. |
...
| **S7 - Fully offline local path** | Provisioned model, no DHT/peers/network | State becomes ready before Harness or inference; `assistant.ready()` resolves without loading a model; a local run loads its requested model and completes; networking reports offline/degraded separately. |
...
| **S9 - iOS inference containment** | Physical iOS device with a candidate out-of-process SDK host, beginning with an Enhanced Security helper extension | Determine whether the host, Sync, and Harness survive fatal SDK failure; verify restart signaling, model-loaded Metal inference, memory limits, signing, and distribution viability; return the containment choice to approvers. |
...
| **G6 - Offline and host lifecycle** | Declaring Assistant's normal API ready | Passing staged state, execution, and model readiness in S7 plus a per-host lifecycle policy backed by S8. |

### What is not clear yet

1. Which iOS mechanism can provide an acceptable inference crash boundary; the Enhanced Security helper extension requires a dedicated PoC before selection.
...
7. iOS inference-containment mechanism and accepted mobile P9 budgets.
...
The current Harness dependency on Assistant for RPC types must be removed. Harness owns its declarative wire contract and generated client types; Sync and SDK own theirs. Callers map and negotiate these package contracts without introducing a shared protocol dependency.
