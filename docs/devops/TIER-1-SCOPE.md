...
- **Secrets:** `PAT_TOKEN` (cross-repo read for the private
  `qvac-registry-vcpkg` + GitHub Packages), `NPM_TOKEN`, `HF_TOKEN`. Fork PRs
  never receive these, so the gate skips fork PRs and relies on the
  push-to-`main` and weekly scheduled runs as the backstop.
