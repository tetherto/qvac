...
**Phase 2 config-parser spike implemented for `model-fit`** (bounded
FuzzTest over `parseInteger` / `canonicalKey`).
...
- **Phase 1 — image + model-header families (as addons migrate).** As each
  ...
  is genuinely shared.
  **`model-fit` llama config integer/key parsers: done.** Header-only, no
