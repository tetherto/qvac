...
**Phase 2 text-parser spike implemented for `llm-llamacpp`** (bounded
FuzzTest over `forLogMessage` / `toLowerAscii` / `matchesAnyStopString`).
...
- **Phase 1 — image + model-header families (as addons migrate).** As each
  ...
  is genuinely shared.
  **`llm-llamacpp` stop-string / log sanitiser: done.** Header-only, no
