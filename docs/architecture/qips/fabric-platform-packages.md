...
Each platform package keeps the current on-disk shape for one host, nested one level down: `addon/prebuilds/<platform>-<arch>/qvac__fabric.bare` plus DL backends under `qvac__fabric/`. The `addon/` level carries a second, inner `package.json` still named `@qvac/fabric` — both `require.addon()` and cmake-bare's `include_bare_module()` derive the artifact basename from the nearest manifest name, so this is what keeps the file `qvac__fabric.bare` (and the `DT_NEEDED qvac__fabric@0.bare` every shipped consumer already links) inside a package published as `@qvac/fabric-<platform>`. Renaming the artifact per platform package instead would break every consumer's link line.
