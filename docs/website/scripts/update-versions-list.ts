...
export const VERSIONS: Version[] = [
  { label: 'Latest (${latestVersion})', path: 'latest', isLatest: true },
 ...
 * @example getVersionFromPath('/sdk/api/v0.6.1/loadModel') → 'v0.6.1'
 */
export function getVersionFromPath(pathname: string): string | null {
  const match = pathname.match(/\\/sdk\\/api\\/(v[\\d.]+|latest)\\//);
  return match?.[1] ?? null;
}
`;
