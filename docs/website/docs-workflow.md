...
3. Only the generated surfaces are committed — `content/docs/reference/api/**`, `content/docs/reference/release-notes/**`, and `src/lib/versions.ts`. The skill stages these with explicit pathspecs; all build/generation byproducts (`api-data.json`, `.typedoc-temp/`, `.next/`, `.source/`, `out/`, `dist/`) are gitignored.
