...
**Reason**: The reorganization created Resources as a placeholder holding only its index page, and the requirement said so. This change gives the collection its purpose — the home of everything that supports the products without documenting a release of one — and publishes Solutions and the corpus protocol there, so a requirement asserting a single page cannot hold.

**Migration**: Replaced by "Resources holds the unversioned supporting material", which fixes the collection's role rather than its page count: unversioned, sitting outside every documentation line, and never copied into one when a line is cut.

### Requirement: SDK collection composition

**Reason**: The requirement described an unversioned collection holding pages directly, and asserted that the patch-series version archives move into it unchanged. Both stop being true here: every SDK page moves inside a documentation line, and the archives are superseded by the line model and leave the published set.

**Migration**: Replaced by "SDK collection composition inside its lines", which keeps what the collection holds and adds where it holds it. The archives are covered by that requirement's scenario on leaving the published set, and their former URLs redirect to the current series.
