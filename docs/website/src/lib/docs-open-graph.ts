...
/**
 * Map of archived page URL → section it belongs to. Two families of URL
 * feed into this map:
 *
 * 1. **All versioned pages of a section marked hidden from indexing**
 *    (`SECTIONS_HIDDEN_FROM_INDEXING`, currently API only) — every
 *    archived snapshot is a near-duplicate of the section's canonical
 *    latest, so we consolidate authority on the bare basePath.
 *
 * 2. **The current-latest versioned page of _every_ versioned section**
 *    (added regardless of whether the section is otherwise hidden).
 *    Under the shim-based layout the canonical bare path
 *    (`/reference/api`, `/reference/release-notes`) is served by
 *    `index.mdx`, which `<include>`s `v<latestSeries>.x.mdx`. The
 *    versioned URL for the current latest is a pure 301 alias of the
 *    shim (see the managed block in `public/_redirects`), so its
 *    static HTML is a byte-duplicate of the shim's — no different from
 *    the API-summary "archived is a duplicate" story, but strictly
 *    scoped to the one series that is currently latest.
 *
 * `buildPageCanonicalUrl` looks up in this map in O(1) so it can return
 * the URL of the section's latest as the canonical target.
 *
 * Built once at module load from `versions.ts` — auto-updated whenever
 * the version list is regenerated by `scripts/update-versions-list.ts`,
 * so we never have to hand-maintain a parallel list.
 *
 * URLs are normalized (no trailing slash, no `index` suffix) to match
 * what `source.getPages()` exposes via `page.url`.
 */
...
function buildHiddenArchivedPageMap(
  hiddenSections: VersionedSection[],
  ...
  for (const section of allSections) {
    for (const version of section.versions) {
      ...
      // hidden (301 alias of the shim canonical).
      if (!hidden.has(section) && !version.isLatest) continue;
