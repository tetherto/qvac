...
| `includeAudioDecoder` | Include the FFmpeg audio decoder in generated worker bundles. Set to `false` only when audio input is raw PCM, supplied as files or base64. Requires an inference runtime with optional audio decoder support; unsupported opt-outs are rejected before bundling. Rebuild the bundle after changing this option. | `boolean` | No | `true` |
...
| `ragTurbovec` | Store built-in [RAG](/ai-capabilities/rag) workspace vectors in a TurboVec index instead of the default HyperDB. Applies only to workspaces created after the flag is set, and the choice is one-way per workspace (existing workspaces keep their adapter). Requires embedding dimension divisible by 8 and ≤ 1024 (fits the default `GTE_LARGE_FP16` at 1024). | `boolean` | No | `false` |
