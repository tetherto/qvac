...
Configuring one or more origins enables CORS. The legacy `--cors` flag no longer grants wildcard access; when retained in scripts, it must be paired with `--cors-origin` or a non-empty `serve.cors.origins` list.
