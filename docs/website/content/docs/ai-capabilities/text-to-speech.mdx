...
[`cancel()`](/reference/api#cancel) to stop synthesis:
...
`cancel({ modelId, kind: "tts" })` cancels whatever run currently owns the
model. Synthesis is serialised per model, so a run still waiting its turn is
cancelled before it starts rather than interrupting the one in flight.
