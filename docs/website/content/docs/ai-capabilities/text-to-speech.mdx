...
default voice — the engine's reference-audio path is reserved and voice
cloning is not yet exposed — and emits native 24 kHz audio.
...
`outputSampleRate` work as on the other engines; `useGPU` offloads to Vulkan
on desktop Linux/Windows only.
...
With Chatterbox, Supertonic, or CosyVoice3, you can choose to perform post-processing using **LavaSR** models. They are applied to the synthesized audio before it is returned, and each stage is enabled purely by supplying its model source — i.e., there is no separate on/off flag.
