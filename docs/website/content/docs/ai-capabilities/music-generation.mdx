...
- `audio` — a promise resolving to interleaved Int16 PCM, its sample rate, and channel count.
...
const [{ pcm, sampleRate, channels }, stats] = await Promise.all([
  run.audio,
  run.stats,
]);
```
...
The SDK returns raw interleaved Int16 PCM rather than a WAV file. Add a WAV header or use an audio encoder before saving or playing it. The complete example below includes a dependency-free WAV writer.
...
<Callout type="success">
  **Tip:** examples in this documentation are self-contained and runnable. See
  the [SDK quickstart](/quickstart) for instructions.
</Callout>
