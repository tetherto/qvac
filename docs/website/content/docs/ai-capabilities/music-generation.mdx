...
`sourceAudio` is required for cover tasks and is validated on the client before the request is sent. `audioCoverStrength` (0..1) is the fraction of DiT steps that keep the source context and must currently be `1` for `cover-nofsq`; `coverNoiseStrength` (0..1) blends the initial noise toward the source latent (`0` = pure noise, `1` ≈ source). The FSQ-roundtrip `cover` task is reserved by the engine but not implemented, so the SDK offers only `text2music` (default) and `cover-nofsq` (see `AUDIOGEN_TASK_TYPES`).
