...
Pick ACE-Step when you need lyrics, per-song musical controls (BPM, key, time signature, vocal language), reference-audio timbre conditioning, cover generation, or [editing of an existing recording](#edit-an-existing-recording) with `audioEdit()`. Pick MiniMax-Music3 when you want a lighter API surface — just a caption and a handful of sampling knobs — on desktop hardware with a capable GPU.
...
2. [`audioGen()`](/reference/api#audiogen) — or [`audioEdit()`](/reference/api#audioedit) to edit an existing recording (ACE-Step only)
...
const run = audioEdit({
  ...
  operations: [
    ...
    { type: "repaint", caption: "analog synth solo", lyrics: "[Instrumental]", start: 10, end: 20 },
    { type: "flow-edit", from: { caption: "guitar pop-rock" }, to: { caption: "dark synthwave" } },
  ],
  seed: 22883,
});
