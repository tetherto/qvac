...
- A **`Buffer` / `Uint8Array`** holding raw interleaved stereo 48 kHz Float32 LE PCM. The `AUDIOGEN_INPUT_SAMPLE_RATE` (48000) and `AUDIOGEN_INPUT_CHANNELS` (2) constants describe that layout. Malformed PCM fails the run with `INVALID_AUDIO_INPUT` before the engine is invoked.
