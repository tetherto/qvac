...
Recommended pairs:
- VisionPsy Nano (base or Flash) + mmproj-*
...
VisionPsy Nano ships in two variants — base and Flash — that share the same projection model (`mmproj-*.gguf`). To pick between them at [`loadModel()`](/reference/api#loadmodel) time, set `modelConfig.image_no_upscale`:
- `'off'` (default): base variant.
- `'on'`: Flash variant.
