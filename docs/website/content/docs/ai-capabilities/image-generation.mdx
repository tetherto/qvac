...
- **ESRGAN** (via `modelConfig.upscaler.model_src`): post-generation upscaler. Model file format: `*.pth` _(optional)_.

For models available as constants, see [SDK — Models](/introduction#models).

<Callout type="info">
**On upscaling:** ESRGAN is an ancillary model that can be paired with any supported diffusion family to upscale generated images in the same call. Available constants: `REALESRGAN_X4PLUS_ANIME_6B`, `REALESRGAN_X4PLUS`.
</Callout>
