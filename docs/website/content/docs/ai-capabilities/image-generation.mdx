...
For image-to-image, also pass `init_image` (a `Uint8Array` of PNG bytes) — the model transforms the input guided by the prompt instead of starting from noise. `diffusion()` returns one or more PNG images as `Uint8Array` buffers. Use `progressStream` to track generation progress step-by-step.
...
- **ESRGAN** (via `modelConfig.upscaler.model_src`): post-generation upscaler. Model file format: `*.pth`; required when `modelConfig.upscaler` is configured.
