...
- **[π₀.₅ (pi05)](https://www.physicalintelligence.company/blog/pi05)**: single all-in-one `*.gguf` file. Expects **3 camera views** and a **discrete** robot state. The state is tokenized into the language prompt instead of passed as a buffer, and the `noise` prior is required. Available constant: `PI05_BASE_Q_AGGRESSIVE`.
