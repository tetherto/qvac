...
`imageInputMode` is either `"pixels"` (each `images[]` entry is a `3·w·h` pixel plane, as prepared by `vlaPreprocessImage()`) or `"patches"` (each entry is a pre-patchified buffer of `vlaHparams().imagePatchElems` floats that the consumer produces by running the model's own vision patch-embed over each camera frame). `vla()` returns the produced action chunk together with per-stage timing stats.
