...
- **[NVIDIA GR00T N1.7-3B](https://huggingface.co/nvidia/GR00T-N1.7-LIBERO)**: single all-in-one `*.gguf` file carrying **multiple embodiments** (see [GR00T embodiments](#gr00t-embodiments) below). Reports `imageInputMode: "patches"` — each camera view is a pre-patchified buffer of `vlaHparams().imagePatchElems` floats, so `vlaPreprocessImage()` does not apply. Uses a **continuous** robot state (pad with `vlaPadState()`); the `noise` prior is required (flow-matching). Available constants: `GROOT_Q8_VF16` (desktop) and `GROOT_Q5_VF16` (mobile).

For models available as constants, see [SDK — Models](/introduction#models).

### GR00T embodiments

Unlike SmolVLA and π₀.₅, a single GR00T GGUF carries up to 32 embodiments (the shipped `GROOT_Q8_VF16` / `GROOT_Q5_VF16` include 17). Pick one at load via `modelConfig.embodiment` — a tag string, a numeric `cat_id` in `0..31`, or an object carrying either plus an optional `numCameras` override for a rig whose view count differs from the stored one. Switch to another at runtime with `setEmbodiment()` (~20 MB read vs. reloading the ~4 GB GGUF). `vlaHparams()` reports the resolved embodiment as `selectedEmbodimentTag` / `selectedEmbodimentCatId`, along with its `numCameras`.
