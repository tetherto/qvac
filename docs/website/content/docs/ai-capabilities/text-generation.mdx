...
* MTP speculative decoding: load a GGUF containing a bundled MTP head with `modelConfig["spec-type"]: "draft-mtp"` and `parallel: 1`. Completion statistics report `draftAccepted` and `draftTotal` when speculation runs — see [MTP speculative decoding](#mtp-speculative-decoding).
...
Multi-token prediction (MTP) can accelerate generation by drafting and verifying several tokens at once. The GGUF must contain a bundled MTP head; a standalone MTP-head file cannot be supplied separately through the SDK.

Enable it while loading the model:

```js
const modelId = await loadModel({
  modelSrc: "https://example.com/model-with-bundled-mtp-head.gguf",
  modelType: "llm",
  modelConfig: {
    parallel: 1,
    "spec-type": "draft-mtp",
...
MTP currently requires `parallel: 1`. If the model has no bundled MTP head or `parallel` is greater than `1`, the addon logs a warning and uses normal decoding. When MTP is active, `completionStats` events and the final result include `draftAccepted` and `draftTotal` counters.
