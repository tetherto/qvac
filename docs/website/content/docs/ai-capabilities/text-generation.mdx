...
* Multi-job continuous batching: load with `modelConfig.parallel >= 2` to run several `completion()` calls in parallel on one loaded model. A burst of prompts then finishes in roughly the time of the slowest one, and each call has its own `requestId` so you can cancel one without touching the others — see [Concurrent completions](#concurrent-completions).
