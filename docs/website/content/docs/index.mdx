...
import { MessagesSquare, Hash, Languages, Mic, Music, Speech, Volume2, ScanText, Image as ImageIcon, Video, Rotate3d, GalleryHorizontal, FlaskConical, ScanSearch, Shapes, Eye, Brain, DoorOpen, Server, MonitorPlay } from 'lucide-react'
import { SiKotlin, SiPython, SiTypescript } from '@icons-pack/react-simple-icons'
...
<Cards className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
  ...
  <Card href="/ai-capabilities/world-simulation" title={<span className="inline-flex items-center gap-2"><Rotate3d className="size-4 text-[var(--color-fd-primary)]" />World simulation</span>}>
    Turn a first-frame image and navigation controls into a streamed, navigable video via an interactive world model.
  </Card>
  <Card href="/ai-capabilities/music-generation" title={<span className="inline-flex items-center gap-2"><Music className="size-4 text-[var(--color-fd-primary)]" />Music generation</span>}>
    Generate music from text prompts and lyrics via ACE-Step or MiniMax-Music3.
  </Card>
...
_The SDK is the main entry point for using QVAC_. It is type-safe and exposes all QVAC capabilities through a unified interface. Three clients share that interface — JS/TS (`@qvac/sdk`, running on Node.js, [Bare](https://bare.pears.com), and [Expo](https://expo.dev)), Python (`tetherto-qvac-sdk`, asyncio-native), and Kotlin (`io.tether:qvac-sdk-kotlin`, plus an Android runtime artifact) — and all call into the same underlying runtime, so capabilities and behavior are identical.

Additionally, QVAC provides a CLI with development tools and an HTTP server that wraps QVAC and exposes an [**OpenAI-compatible API**](https://platform.openai.com/docs/api-reference). _You can use the HTTP server as a local model provider to connect any AI tool, harness, or system._

Finally, QVAC also encompasses [**QV.AC**](https://qv.ac), a desktop and mobile assistant app, to empower users and showcase QVAC capabilities, as well as [**research initiatives**](https://huggingface.co/qvac) to advance the state of the art in local AI.
...
<Cards>
  <Card href="/introduction" icon={<DoorOpen className="text-[var(--color-fd-primary)]" />} title="Get started">
    Learn the essentials for using the SDK: system requirements, compatibility matrix, setup basics, and core usage flows.
  </Card>
  <Card href="/js-ts-sdk" icon={<SiTypescript className="text-[var(--color-fd-primary)]" size={20} />} title="JS/TS SDK">
    Install `@qvac/sdk`, run the quickstart, and target Node.js, Bare, or Expo.
  </Card>
  <Card href="/python-sdk" icon={<SiPython className="text-[var(--color-fd-primary)]" size={20} />} title="Python SDK">
    Install `tetherto-qvac-sdk`, run the quickstart, and drive the same worker from Python.
  </Card>
  <Card href="/kotlin-sdk" icon={<SiKotlin className="text-[var(--color-fd-primary)]" size={20} />} title="Kotlin SDK">
    Add the KMP client or an embedded Android profile and use coroutine-native APIs.
  </Card>
  ...
  <Card href="/cli/http-server" icon={<Server className="text-[var(--color-fd-primary)]" />} title="Use QVAC as a local model provider">
