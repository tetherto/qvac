...
Speech addons (`tts-ggml`, `asr-ggml`, `audiogen-ggml`) ship Android and iOS binaries in a separate package. If you enable those plugins, add that package as a direct dependency, pinned to the same exact version as the JavaScript package — for example `"@qvac/tts-ggml": "x.y.z"` and `"@qvac/tts-ggml-android-arm64": "x.y.z"` (iOS uses `@qvac/tts-ggml-ios`). Llama does not need an extra package. `npx expo prebuild` names the missing pin if you forget. To have `npx expo prebuild` add the missing packages with your package manager instead, pass `installMissingPrebuilds` to the plugin: `["@qvac/sdk/expo-plugin", { "installMissingPrebuilds": true }]`.
