...
if (resources.capabilities.gpus.status === "supported") {
  for (const gpu of resources.capabilities.gpus.value) {
    console.log(gpu.name, gpu.driverName, gpu.driverVersion, gpu.drivers);
  }
}
```
...
fallback reason. Machine-readable compatibility probes remain addon-owned
future work; the SDK does not infer them from inventory or logs.
