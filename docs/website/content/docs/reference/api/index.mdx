...
>
> **Scope**: 50 functions in `packages/sdk/client/api/` plus the `profiler` object.
...
- `ModelNotFoundError` — When the model ID is not loaded.
- `InvalidResponseError` — When the worker returns an unexpected response type.
