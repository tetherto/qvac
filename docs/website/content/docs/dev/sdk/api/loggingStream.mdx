---
title: "loggingStream( )"
titleStyle: code
description: "Opens a logging stream to receive real-time logs."
---

<WrapCode>

```typescript
function loggingStream(params: { id: string }): AsyncGenerator<{ id: string; level: LogLevel; message: string; namespace: string; timestamp: number; type: "loggingStream" }>
```

</WrapCode>

## Parameters

| Name | Type | Required? | Description |
| --- | --- | :---: | --- |
| `params` | `\{ id: string \}` | ✓ | The arguments for the logging stream |

## Returns

```typescript
AsyncGenerator<{ id: string; level: LogLevel; message: string; namespace: string; timestamp: number; type: "loggingStream" }>
```

No description available

## Throws

| Error | When |
| --- | --- |
| `When the response type is invalid or when the stream fails` | — |

## Example

```typescript
// Open a logging stream for a model
const logStream = loggingStream({ id: 'my-model-id' });

// Or stream SDK server logs
const sdkLogs = loggingStream({ id: SDK_LOG_ID });

for await (const logMessage of logStream) {
  console.log(`[${logMessage.level}] ${logMessage.namespace}: ${logMessage.message}`);
}
```
