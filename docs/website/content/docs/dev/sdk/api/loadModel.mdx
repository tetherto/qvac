...
| --- | --- | :---: | --- |
| `options` | [`LoadModelOptions`](#loadmodeloptions) | ✓ | An object that defines all configuration parameters required for loading the model, including:
  ...
  - logger: Logger instance for model operation logs || `rpcOptions` | `\{ forceNewConnection?: boolean; healthCheckTimeout?: number; profiling?: \{ enabled?: boolean; includeServerBreakdown?: boolean; mode?: "summary" \| "verbose" \}; timeout?: number \}` | ✗ | Optional RPC options including per-call profiling configuration |
### `LoadModelOptions`
