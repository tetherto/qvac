...
| `INVALID_AUDIO_CHUNK_TYPE` | 50004 | Invalid audio chunk type. | `transcribe()`, `transcribeStream()` |
...
| `DELEGATE_NO_FINAL_RESPONSE` | 50402 | No final response received from delegated provider. | `completion()`, `loadModel()` |
| `DELEGATE_PROVIDER_ERROR` | 50403 | Delegated provider returned an error. | `completion()`, `loadModel()` |
| `DELEGATE_CONNECTION_FAILED` | 50404 | Failed to connect to delegated provider. | `completion()`, `loadModel()` |
