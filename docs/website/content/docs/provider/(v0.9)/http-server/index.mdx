...
title: HTTP server
icon: Server
description: Run a local HTTP server exposing the QVAC API, with an optional OpenAI-compatible extension.
...
To run the server, install the `@qvac/cli` npm package — it depends on `@qvac/sdk` directly, so the SDK is installed automatically. The server is provided by `@qvac/cli` and internally translates HTTP requests into SDK calls.

## Surfaces

`qvac serve` runs the QVAC API. Additional surfaces are extensions, mounted by
their own flag:

| Surface | Mounted with | Paths |
| --- | --- | --- |
| QVAC | `qvac serve` | `/qvac/v1/*` |
...
Any system compatible with the [OpenAI REST API](https://developers.openai.com/api/reference/overview) can then point to `http://localhost:11434/v1/` and work without changes.

Pass `--no-default` to leave the QVAC surface out. Introspection
(`/openapi.json`, and `/docs` with `--docs`) is always served and describes
exactly what you mounted.

<Callout type="warn">
`qvac serve openai` is deprecated; it is equivalent to `qvac serve --openai --no-default`.
</Callout>

## AI capabilities

At the moment, the OpenAI-compatible extension supports the following QVAC AI capabilities:
...
The QVAC surface supports:
* [Text translation](#post-qvacv1translate) — via `/qvac/v1/translate`, backed by an NMT model.
...
See [System requirements](/sdk/system-requirements#environment-setup) for environment-specific instructions (e.g., Linux Vulkan runtime, Windows GPU drivers).
</Step>

<Step>
Create the `qvac.config.json` file at the root of your project declaring which models the server can load. The recommended way is to let the CLI assemble it for you:

```bash
qvac configure           # interactive: pick models by capability or search
qvac configure --yes     # non-interactive: chat + transcription starter
```

`qvac configure` searches the SDK's model catalog, previews the exact `serve.models` entry it will write, and merges into an existing `qvac.config.json` — see [`qvac configure`](/sdk/cli#qvac-configure) for the full reference.

<Callout type="info">
**Prefer to write it by hand?** Any of `qvac.config.{json,js,mjs,ts}` works. `qvac configure` only writes JSON; skip it and author the file yourself if you want a `.js` or `.ts` config, or if you need to set fields beyond `serve.models` (such as `plugins`, `cacheDirectory`, or `serve.cors`) at the same time. See [Configuration — `ServeConfig`](/sdk/configuration#serveconfig) for the full schema. Minimal example:
...
- **`preload`**: when `true`, the model is loaded into memory on server startup. When `false`, it is loaded lazily on the first request that names it (cold start — that request blocks while the model loads; concurrent first requests share a single load). Defaults to `true` for constant model entries and `false` for explicit `{ src, type }` entries. Use `--model <alias>` to force a lazy model to warm at startup.
...
Lazy-load behavior (whether requests may load models on demand, load concurrency, per-load timeout, and cancel-on-disconnect) is tuned under [`serve.load`](/sdk/configuration#loadoptions-serveload), with matching `qvac serve --openai` flags. Set `serve.load.lazy: false` (or `--no-lazy-load`) to require preloaded models and return `503 model_not_loaded` otherwise.

<Callout type="info">
The default LLM context window is 1,024 tokens and includes both the input and generated output. Set `config.ctx_size` explicitly for longer prompts. A request's `max_tokens` limits only generated tokens, so budget the prompt and requested output together within `ctx_size`.
</Callout>
...
Relevant options are summarized below; run `qvac serve --help` for the exact generated help output.

| Option | Summary |
| --- | --- |
| `--openai` | Mount the OpenAI-compatible surface |
...
| `--cors` | Validate that an explicit trusted origin is configured |
| `--cors-origin <origin>` | Add a trusted CORS origin (repeatable; no wildcard) |
| `--public-base-url <url>` | Externally reachable origin required for image `response_format=url` |
| `--docs` | Mount Swagger UI at `/docs` and add same-port loopback CORS origins (requires a fixed `--port`) |
| `-v, --verbose` | Detailed output |

### CORS

CORS is disabled unless trusted origins are supplied through repeatable `--cors-origin` flags or `serve.cors.origins` in `qvac.config.*`, or `--docs` adds its same-port loopback defaults. Origins must be exact HTTP(S) origins without credentials, paths, queries, or fragments. Wildcard (`*`) access is not supported.
...
The legacy `--cors` flag is retained as a compatibility validation switch, but it does not enable CORS by itself. It requires at least one explicit origin from `--cors-origin` or `serve.cors.origins`; otherwise startup fails. `--cors --docs` also fails without an explicit origin because the docs defaults are added only after this validation. Existing scripts can migrate as follows:
...
Served by the QVAC surface, mounted by default. Base path: `/qvac/v1`.

### KV cache

Chat requests cache each conversation's prefix under `~/.qvac/kv-cache`, so a follow-up turn only prefills the new tail. Chat is the only endpoint that caches — Responses and Legacy completions run uncached and leave nothing here to reclaim. The directory grows on the order of hundreds of megabytes per conversation, but it is **bounded** — you do not need to write a sweeper of your own.

| Bound | Value |
| --- | --- |
| Size quota | 4 GiB least-recently-used (512 MiB on mobile) |
| Idle TTL | 24 hours |
| Sweep frequency | After a cached turn finishes, at most every 5 minutes |

Caches held by an in-flight request are never evicted.

<Callout type="warn">
**Named caches are not covered.** The policy applies only to the *automatic* caches these endpoints create. A caller-owned named cache — `completion({ kvCache: "my-session" })` through the SDK — is exempt from both the quota and the TTL, and the endpoint below never touches it. Clean those up with [`deleteCache({ kvCacheKey })`](/sdk/reference/api#deletecache). The server itself only ever creates automatic caches; it does not use named keys.
</Callout>

### `DELETE /qvac/v1/kv_cache`

Frees every automatic KV cache no in-flight request is using, immediately, rather than waiting for the quota or the TTL. Use it when reclaiming disk matters more than keeping conversations warm: deleting a cache costs the next turn a full prefill instead of a partial one, and loses no conversation content, since a KV cache is derived data.
...
A failed delete returns `500 kv_cache_reclaim_failed`.

<Callout type="info">
**Scope is the host, not this server.** `~/.qvac/kv-cache` is shared by every QVAC process running under the same home directory, so this reclaims another local process's automatic caches too. That is the same scope the automatic sweep already acts on. Nothing in a cache path identifies the process that wrote it, because cache identity is content-derived, so per-server scoping is not available.
</Callout>

This route needs the QVAC surface mounted. It is absent under `--no-default`, and therefore under the deprecated `qvac serve openai`, which implies it.
...
That alias configures the engine and the language direction, as shown at the end
of this section.
...
`stats` is returned for a single input; a batch does not report stats.

Streaming a single input emits `translation.chunk` events carrying `delta` as
the text is decoded. Streaming an array emits one `translation.item` per input,
carrying `index` and the whole `text`, and these arrive together once the batch
finishes. Both end with one `translation.done` event, then `data: [DONE]`.
...
Configure a translation model with its engine and direction:
...
Served by the `--openai` extension. All endpoints follow the [OpenAI API](https://platform.openai.com/docs/api-reference) request and response format. Base path: `/v1`.
...
Inspect and unload models registered in `serve.models`. Every configured model is
usable: one that is not preloaded loads on the first request that names it.

#### `GET /v1/models`

List all configured models (loaded or not — each loads on first request).
...
Get details of a configured model (whether or not it is currently loaded).
...
Unload a model, releasing its resources. The alias stays configured and
reloadable — the next request that names it loads it again. There is no separate
load endpoint; send a normal request (or set `preload: true`) to load a model.
...
`GET /v1/models/catalog` is a discovery endpoint (a QVAC extension, not part of OpenAI). It
lists your configured models **and** every model the SDK ships with, so you can find one by
capability. Browsing loads nothing — it's fully in-process.

Rows are catalog entries (`object: "model_catalog_entry"`), not usable `model` objects. A
model that isn't configured shows `state: "not_configured"`, `usable: false` and a `hint` —
it **cannot be called** until you add it to `serve.models`. Configured models show
`configured: true` with their live load `state`. (Only `GET /v1/models` lists callable
models.)

Filter with query params (combined with AND): `search` (id substring), `role`
(chat/embedding/transcription/speech/image/…), `addon`/`type` (llm/tts/whisper/diffusion/…),
`quantization`, `engine`, `configured`, `limit` (omit for no limit), `offset`.

```bash
curl 'http://localhost:11434/v1/models/catalog?role=chat&limit=20'
```

Response: `{ "object": "list", "data": [ … ], "has_more": <bool> }`, each entry:
...
```json
{
  ...
  "params": "600M",
  "size": 382156480,
  "hint": "Not in serve.models — add it there to make it usable."
}
```

`GET /v1/models/catalog/:id` returns a single catalog entry. The live remote registry and
on-disk download state are planned follow-ups.
...
tool name in place of the object form is rejected the same way — use the object
...
log records what happened (`toolerrors=1 (PARSE_ERROR)`) — the OpenAI response
...
| `seconds` | Target duration as a **string** (e.g. `"2"`; OpenAI uses `"4"` / `"8"` / `"12"`). For MiniMax-H3, mapped at 24 FPS to the nearest `17k+5`; other models retain the existing nearest `4k+1` mapping using `fps`. Model-specific validation still applies. | No |
| `fps` | QVAC extension. `0 < fps ≤ 120`, default `16`. MiniMax-H3 requires exactly `24` and rejects any other value; its `seconds` mapping always uses 24 regardless of what is sent here. | No |
...
Bearer authentication does not apply to the introspection routes: `/openapi.json`, `/docs`, and every path under `/docs/` remain public even when `--api-key` is set. `/docs` and `/docs/*` exist only when `--docs` is enabled; `/openapi.json` is always available.

A non-loopback `--host` requires authentication: without `--api-key` or `--api-key-file` the server refuses to start. Pass `--allow-unauthenticated` to accept the risk and start anyway, which downgrades the refusal to a warning.

<Callout type="warn">
Do not enable `--docs` on a non-loopback bind unless public introspection is acceptable. The Swagger UI and its assets remain unauthenticated, and `/openapi.json` is public on every bind even when API authentication is enabled.
</Callout>
...
`--docs` adds same-port `localhost`, `127.0.0.1`, and `[::1]` origins to CORS so a locally opened Swagger UI can use "Try it out." When the configured `--host` is itself loopback, its same-port origin is added too. It does not trust a bound non-loopback host, a port-forwarded origin, or arbitrary websites. Add each additional browser origin with `--cors-origin` or `serve.cors.origins`.

Because those origins are derived from the configured port, `--docs` requires a fixed port: `--docs --port 0` fails startup with a `Serve Options Error` rather than binding an ephemeral port whose `http://localhost:0` origins no browser can match. Pick a port, or drop `--docs` and read the always-available `/openapi.json`.
