---
title: Profiler
description: Measure and export timing metrics across model loading, inference, and P2P delegation.
---

## Overview

Use the profiler to measure and analyze how long SDK operations take in your application. You can enable profiling in two ways:

- **Global:** call `profiler.enable()` to profile all subsequent operations until `profiler.disable()` is called.
- **Per call:** pass `{ profiling: { enabled: true } }` in the options of an individual function call.

You can also enable profiling globally and use a per-call override to opt out of specific calls by passing `{ profiling: { enabled: false } }`. Data collected through both modes is stored in and exported from the same `profiler` object.

The following operations support profiling: `loadModel()` | `completion()` | `embed()` | `transcribe()` | `translate()` | `ragSearch()` | `invokePlugin()` | `downloadAsset()`.

<Callout type="info">
`profiler` is a process-wide singleton — its state persists until the process exits.
</Callout>

## Functions

1. `profiler.enable(options?)` — enable profiling globally
2. `profiler.isEnabled()` — check if profiler is active
3. `profiler.onRecord(callback)` — subscribe to profiling events in real time
4. Export collected data using any of:
   - `profiler.exportSummary()` — high-level summary string
   - `profiler.exportTable()` — detailed aggregated metrics table
   - `profiler.exportJSON(options?)` — full export as structured JSON
5. `profiler.disable()` — disable profiling
6. `profiler.clear()` — reset collected data

For the full metrics catalog of each export function and detailed usage, see [SDK — API reference](/sdk/api/).

## Precedence

Profiling behavior is determined in the following order:

1. **Per-call override:** `{ profiling: { enabled: true/false } }` on an individual call always wins.
2. **Global enablement:** `profiler.enable()` applies to all calls that don't carry a per-call override.
3. **Default:** profiling is disabled.

This means you can enable profiling globally and still opt out specific calls, or keep the profiler disabled and opt in only the calls you care about.

## Enabling

### Global

Enable profiling globally by calling `profiler.enable()`:

```ts
profiler.enable({
  mode: "verbose",                 // "summary" (default) | "verbose"
  includeServerBreakdown: true,    // include server-side timing in responses
  operationFilters: ["completion"], // only profile these operations (empty = all)
});
```

### Per call

To profile a single call, pass the `profiling` option option when invoking a function:

```ts
await embed(
  { modelId, text: "hello" },
  {
    profiling: {
      enabled: true,
      includeServerBreakdown: true,
      mode: "verbose",
    },
  },
);
```

<Callout type="info">
Per-call profiling is supported on the following function calls: [`loadModel()`](/sdk/api/loadModel) | [`completion()`](/sdk/api/completion) | [`embed()`](/sdk/api/embed) | [`transcribe()`](/sdk/api/transcribe) | [`translate()`](/sdk/api/translate) | [`ragSearch()`](/sdk/api/ragSearch) | [`invokePlugin()`](/sdk/api/invokePlugin) | [`downloadAsset()`](/sdk/api/downloadAsset).
</Callout>

### Opt-out

If you enabled profiling globally, you can pass the `profiling` option to opt out of a specific call:

```ts
await embed(
  { modelId, text: "hello" },
  { profiling: { enabled: false } },
);
```

## Examples

### Global

The following script enables profiling globally, loads a model, runs a completion, and exports timing data in all available formats:

<Tabs>
<Tab value="js" label="JavaScript" default>
<WrapCode>

```js file=<rootDir>/packages/sdk/dist/examples/profiling/basic.js title="profiling-basic.js" lineNumbers
```
</WrapCode>
</Tab>

<Tab value="ts" label="TypeScript">
<WrapCode>

```ts file=<rootDir>/packages/sdk/examples/profiling/basic.ts title="profiling-basic.ts" lineNumbers
```
</WrapCode>
</Tab>
</Tabs>

### Per call

The following script keeps the profiler disabled globally and selectively profiles individual `embed()` calls using the per-call option:

<Tabs>
<Tab value="js" label="JavaScript" default>
<WrapCode>

```js file=<rootDir>/packages/sdk/dist/examples/profiling/per-call.js title="profiling-per-call.js" lineNumbers
```
</WrapCode>
</Tab>

<Tab value="ts" label="TypeScript">
<WrapCode>

```ts file=<rootDir>/packages/sdk/examples/profiling/per-call.ts title="profiling-per-call.ts" lineNumbers
```
</WrapCode>
</Tab>
</Tabs>

<Callout type="success">
**Tip:** all examples throughout this documentation are self-contained and runnable. For instructions on how to run them, see [SDK quickstart](/sdk/getting-started/quickstart).
</Callout>
