...
Optional delegation configuration for remote (P2P) inference. Consumers connect directly to the provider via the DHT using `providerPublicKey` — no topic or discovery step is required.

| Field | Type | Required? | Default | Description |
| --- | --- | :---: | --- | --- |
| providerPublicKey | `string` | ✓ | — | Provider's public key (from `startQVACProvider`) |
| timeout | `number` | ✗ | — | Timeout in milliseconds (min 100). On a cold DHT the first connect can take 15–45s — use a generous value (e.g. `60_000`). Subsequent connections reuse the open socket and are sub-second. |
