...
The SDK is cross-platform, pluggable, and provides:
- A type-safe JS API.
- An HTTP server that exposes an OpenAI-compatible API.
- A CLI for tooling and for running the HTTP server.

Both the JS and the HTTP APIs expose all QVAC capabilities through a unified interface.
...
- **OpenAI-compatible API**: integrate with systems that implement the OpenAI API.
