...
Install the TypeScript or Python clients in your project. Then, load models and use them to perform AI inference locally, or delegate inference to peers using the built-in P2P capability.
...
<Cards>
  <Card href="/js-ts-sdk" title="TypeScript SDK">
    TypeScript client — `@qvac/sdk` on npm. Runs on Node.js, Bare, and Expo.
  </Card>
...
- **Type-safe:** typed APIs in both TypeScript and Python (Pydantic models generated from the same contract).
...
For more on querying the model registry, see [`modelRegistryList()`](/reference/api#modelregistrylist), [`modelRegistrySearch()`](/reference/api#modelregistrysearch), and [`modelRegistryGetModel()`](/reference/api#modelregistrygetmodel) (JS/TS), or `model_registry_list` / `_search` / `_get_model` (Python).
...
<Cards>
  ...
  <Card href="/python-sdk#the-public-api" title="Python API surface">
