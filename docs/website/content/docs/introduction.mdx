...
Install the JS/TS, Python, or Kotlin client in your project. Then, load models and use them to perform AI inference locally.

The SDK has two layers:

- **Worker** — the runtime that actually runs the models. Today the worker ships as the `@qvac/sdk` npm package.
- **Clients** — per-language APIs that drive the worker. JS/TS (`@qvac/sdk`), Python (`tetherto-qvac-sdk`), and Kotlin (`io.tether:qvac-sdk-kotlin`) use the same generated contract. Android can embed the worker through capability-specific AARs.
...
<Cards>
  ...
  <Card href="/kotlin-sdk" title="Kotlin SDK">
    Kotlin Multiplatform client plus self-contained Android AARs. Coroutine-native and Android service-ready.
  </Card>
</Cards>
...
- **Cross-platform:** portable code across Linux, macOS, and Windows (JS/TS on Node.js / [Bare runtime](https://bare.pears.com), Python on CPython, Kotlin on JVM); Android via Expo or a native Kotlin AAR, and iOS via Expo. Kotlin/Native targets expose the client and accept an application-provided transport.
- **Pluggable**: build lean apps by including only what you need, and extend the SDK with custom plugins.
- **Type-safe:** typed APIs in JavaScript, Python (Pydantic), and Kotlin (serialization models and typed builders generated from the same contract).
...
The concrete API names differ by client. JS/TS uses `loadModel()` / `completion()` / `unloadModel()` / `close()`, Python uses `load_model` / `completion` / `unload_model`, and Kotlin uses `client.models.load(...)` / `client.completion.run(...)` / `client.models.unload(...)` / `client.close()`.
...
<Cards>
  ...
  <Card href="/kotlin-sdk#api-surface" title="Kotlin API surface">
    Coroutine-native high-level wrappers plus the complete generated worker contract.
  </Card>
</Cards>
