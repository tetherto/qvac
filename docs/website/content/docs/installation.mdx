...
| Linux | Ubuntu 22+ | arm64, x64 | Vulkan $\geq$ 1.4 | Vulkan is required for GPU inference. If not available, it falls back to CPU. |
| Android | 12+ | arm64 | Vulkan, OpenCL (Adreno 700+) | Requires Expo |
| Windows | 10+ | x64 | Vulkan $\geq$ 1.4 | Vulkan is required even for CPU-only inference |
...
- Vulkan runtime $\geq$ 1.4: Vulkan loader + a GPU driver with Vulkan support.
