...
## CLI bundling error: plugin export mismatch
...
This usually means there is version skew between `@qvac/cli` and `@qvac/sdk`. An older CLI version may generate a bundle using outdated built-in plugin metadata, while a newer SDK version exports the renamed plugin only through its current named export shape.

Starting with `@qvac/cli` `0.6.0`, `qvac bundle sdk` delegates to `@qvac/sdk/commands`, which keeps the bundling logic aligned with the SDK.

### Solution

Upgrade `@qvac/cli` to version `0.6.0` or later, then rebuild the SDK bundle:
