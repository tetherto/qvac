...
sampling. Resource gauges are local diagnostics; they are not transmitted
telemetry and do not affect model admission. Enabling them adds one CPU query
and one query per GPU to each profiled operation's response path. If the worker
resource collector is not initialized, the event omits the resource block.
