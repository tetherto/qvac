...
becoming zero. Profiling-disabled operations and calls that do not opt in
perform no resource sampling. Resource gauges are local diagnostics; they are
not transmitted telemetry and do not affect model admission.
