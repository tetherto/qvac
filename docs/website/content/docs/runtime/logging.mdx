...
<Callout type="info">
**Python:** `getLogger` is JS/TS-only and intentionally not ported — use the standard library [`logging`](https://docs.python.org/3/library/logging.html) module (a `logging.Handler` is the transport equivalent). The log-stream APIs `logging_stream` and `subscribe_server_logs` (plus `SDK_LOG_ID` / `SDK_ALL_LOG_ID`) **are** available in the Python SDK.
</Callout>
...
```python file=<rootDir>/packages/sdk-python/examples/logging_streams.py title="logging_streams.py" lineNumbers
```
</WrapCode>
</Tab>
</Tabs>
