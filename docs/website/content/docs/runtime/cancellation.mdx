...
**Targeted cancel by `requestId` — only the cancelled call stops**: `completion()`, `batchCompletion()`, LLM `translate()`, `loadModel()`, `downloadAsset()`, and `rag*()` (`ragIngest`, `ragSaveEmbeddings`, `ragReindex`).

**Accepted by `requestId` but also aborts other in-flight calls on the same model**: `embed()`, `transcribe()`, `audioGen()`.
...
**Watch out** for `embed()`, `transcribe()` and `audioGen()`: a targeted cancel by `requestId` **also stops every other in-flight call on the same model**. If several requests share a model, expect them all to be aborted together.
...
```python file=<rootDir>/packages/sdk-python/examples/cancel.py title="cancel.py" lineNumbers
```
</WrapCode>
</Tab>
</Tabs>
...
- `RequestRejectedByPolicyError` (code `52420`) — the request was rejected before it started. On an LLM model, `completion`, `batchCompletion`, LLM `translate`, and `finetune` share a per-model queue: up to [`modelConfig.parallel`](/ai-capabilities/text-generation#concurrent-completions) of them run concurrently, extras wait first-come-first-serve, and the queue caps at 64 waiters per model — the 65th request in line is rejected up-front with this error. A running `finetune` holds the model exclusively, so during training every other call on that model queues behind it. If you need to serve bursts larger than this, raise `parallel` at load (and raise [`ctx_size`](/ai-capabilities/text-generation#concurrent-completions) proportionally), back-pressure at the application level, or catch this error and retry. Carries `requestId`, `kind`, `modelId`, and a human-readable `reason`.
