...
<Callout type="info">
`@qvac/llm-llamacpp` uses addon-specific transaction checkpoints. Explicit prefill or generation cancellation restores the affected sequence to its pre-request model-memory state. Partial output may already have streamed to the caller, but it is not retained in the addon's KV/recurrent memory. This checkpoint behavior is specific to `llm-llamacpp`; other addons may use different cancellation mechanics.
</Callout>
