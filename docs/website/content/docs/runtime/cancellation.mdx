...
Only the targeted call is affected — every other in-flight call on the same `modelId` keeps running. That includes concurrent `completion()` / `batchCompletion()` peers when the model was loaded with `modelConfig.parallel >= 2`, so this is the right form when you want a per-request stop button that does not disturb the rest of the workload. To cancel `translate`, `textToSpeech`, `ocr`, `diffusion`, or `upscale` — or to sweep every in-flight call on a model in one shot — use the broad-cancel form below.
...
- `RequestRejectedByPolicyError` (code `52420`) — the request was rejected before it started. On an LLM model, `completion`, `batchCompletion`, LLM `translate`, and `finetune` share a per-model queue: up to `modelConfig.parallel` of them run concurrently, extras wait first-come-first-serve, and the queue caps at 64 waiters per model — the 65th request in line is rejected up-front with this error. A running `finetune` holds the model exclusively, so during training every other call on that model queues behind it. If you need to serve bursts larger than this, raise `parallel` at load, back-pressure at the application level, or catch this error and retry. Carries `requestId`, `kind`, `modelId`, and a human-readable `reason`.
