...
1. [`completion()`](/reference/api#completion) and [`batchCompletion()`](/ai-capabilities/batch-processing) — return a run object that exposes a synchronous `requestId` field.
...
<Callout type="info">
`@qvac/llm-llamacpp` uses addon-specific transaction checkpoints only for requests with `cacheKey` and `saveCacheToDisk: true`. When the active same-key session has newer RAM state than disk, the addon first writes that RAM state to the canonical cache file and pins it as the rollback artifact. A non-empty persistent request fails before mutation if there is no usable committed artifact or committable live state, or if pinned bytes and metadata cannot be validated. Explicit cancellation clears the target sequence, then restores that artifact's state and cache metadata together. Without persistent saving, cancellation clears unsaved live model state instead. Partial output may already have streamed to the caller, but it is not retained in the addon's KV/recurrent memory. This behavior is specific to `llm-llamacpp`; other addons may use different cancellation mechanics.
</Callout>
...
A cancelled assistant turn delivered to the application is **partial** — the model stopped mid-decode, so its content cuts off in the middle of a thought. Drop it (or mark it as partial) before appending the next user turn to application-managed `history`. Otherwise that explicit history makes a later request see the truncated assistant message as if it were complete:
...
<Callout type="info">
The same partial-turn rule applies if you abort the `events` iterator early (e.g., `break` out of the `for await` loop) without calling `cancel()`. Output already delivered to the application remains partial; whether an addon retains model-memory state depends on that addon's cancellation implementation.
</Callout>
