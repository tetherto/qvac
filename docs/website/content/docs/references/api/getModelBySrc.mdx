...
[`RegistryItem`](/references/api/getModelByName#registryitem) ` | undefined` — The matching model constant, or `undefined` if not found. See [`getModelByName()`](/references/api/getModelByName#registryitem) for the full `RegistryItem` shape.
