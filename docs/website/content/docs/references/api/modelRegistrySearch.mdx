...
`Promise<`[`ModelRegistryEntry[]`](/references/api/modelRegistryGetModel#modelregistryentry)`>` — Matching model entries. See [`modelRegistryGetModel()`](/references/api/modelRegistryGetModel#modelregistryentry) for the `ModelRegistryEntry` shape.
