...
| handlers | `Record<string,` [`PluginHandlerDefinition`](/references/api/defineHandler#pluginhandlerdefinition)`>` | ✓ | Map of handler names to handler definitions |
...
See the [Write a custom plugin](/how-tos/write-custom-plugin) guide for a full walkthrough.
