...
Host-level toolchain and driver setup that the QVAC worker relies on. Client install commands (`npm i @qvac/sdk`, `pip install tetherto-qvac-sdk`, Expo plugin wiring, …) live on each client page — see [JS/TS SDK](/sdk/js-ts-sdk) and [Python SDK](/sdk/python-sdk).
...
In a container build running as root, omit `sudo`. A missing library can prevent the worker from starting before any model loads; see [missing libatomic at startup](/resources/troubleshooting#startup-failure-missing-libatomicso1).
...
Mobile is supported by the JS/TS SDK via Expo. See [JS/TS SDK — Expo setup](/sdk/js-ts-sdk#expo) for the full flow.
