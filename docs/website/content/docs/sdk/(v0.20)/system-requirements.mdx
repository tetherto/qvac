...
In a container build running as root, omit `sudo`. A missing library can prevent the worker from starting before any model loads; see [missing libatomic at startup](/sdk/troubleshooting#startup-failure-missing-libatomicso1).
