...
Now that the app works in dev env, let's build a distributable using the `@qvac/sdk/electron-forge` plugin, which:
...
<Step>
_Optionally_ Add a `qvac.config.json` to declare which QVAC plugins your app uses:
...
- **macOS universal builds are blocked.** Native addon prebuilds are arch-specific (`darwin-arm64.bare` vs `darwin-x64.bare`); a universal Electron app would need to ship both prebuild sets, which conflicts with the plugin's single-arch pruning. Build the two architectures separately:
