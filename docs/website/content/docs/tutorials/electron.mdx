...
Now that the app works in development, let's build a distributable using the `@qvac/sdk/electron-forge` plugin, which:
...
Run `npm run make` to also produce `.zip` / `.dmg` / `.exe` distributables from the makers in `forge.config.cjs`.
</Step>
...
- **`asar: false` is forced.** The Bare worker can't load addons from inside an ASAR archive — paths inside `app.asar` aren't real files on disk. The plugin overrides any `packagerConfig.asar` setting and warns if you've set it to anything else.
