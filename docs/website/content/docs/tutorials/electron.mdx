...
Now that the app works in development, let's build a distributable. We'll switch from the scaffold's default electron-builder to **electron-forge** so we can use the [`@qvac/sdk/electron-forge`](https://www.npmjs.com/package/@qvac/sdk) plugin, which:

- Bundles the QVAC worker for the target platform/arch.
- Tree-shakes unused `@qvac/*` addons.
- Prunes per-arch native prebuilds so a darwin-arm64 build doesn't ship win32/linux binaries (typical reclamation: ~700 MB).
- Hard-blocks configurations that produce broken apps (universal macOS, ASAR-enabled).

<Steps>

<Step>
Replace electron-builder with electron-forge:

```bash
npm uninstall electron-builder
rm electron-builder.yml
npm pkg delete scripts.postinstall
npm pkg delete scripts.build:unpack
npm pkg delete scripts.build:win
npm pkg delete scripts.build:mac
npm pkg delete scripts.build:linux
npm i -D @electron-forge/cli @electron-forge/plugin-base @electron-forge/maker-zip
```

`@electron-forge/plugin-base` is the QVAC plugin's only peer dependency. `@electron-forge/maker-zip` produces a zip distributable; add other makers (`@electron-forge/maker-dmg`, `@electron-forge/maker-squirrel`, etc.) as needed.
</Step>

<Step>
Resolve the `out/` directory collision. electron-forge writes packaged output to `out/`, and electron-vite's default output dir is also `out/` — they conflict. Move electron-vite's output to `dist/`:
...
Update `package.json`'s `main` field to point at the new location:
...
<Step>
Add a `forge.config.cjs` at the project root:
...
Optionally, add a `qvac.config.json` to declare which QVAC plugins your app uses:
...
With no `qvac.config.json` the plugin auto-discovers and bundles all built-in QVAC plugins. Listing only what you use produces a smaller worker bundle.
</Step>
...
Run `npm run make` instead to also produce `.zip` / `.dmg` / `.exe` distributables from the makers in `forge.config.cjs`.
</Step>
...
- **`asar: false` is forced.** The Bare worker can't load addons from inside an ASAR archive — paths inside `app.asar` aren't real files on disk. The plugin overrides any `packagerConfig.asar` setting and warns when it's been set to anything truthy.

- **macOS universal builds are blocked.** Native addon prebuilds are arch-specific (`darwin-arm64.bare` vs `darwin-x64.bare`); a universal Electron app would need to ship both prebuild sets, which conflicts with the plugin's single-arch pruning. Build the two arches separately:
...
- **Cross-platform packaging is supported.** The plugin reads `--platform` and `--arch` from CLI args (and from `packagerConfig` in the config file) to drive both `bundleSdk` and `verifyBundle`. From a darwin-arm64 host you can package for darwin-x64 as long as each addon ships a `darwin-x64.bare` prebuild:
