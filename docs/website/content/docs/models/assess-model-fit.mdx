...
`native-fit` is the strongest: it is the answer the loader itself would give on this machine, so it outranks `calibration` where both exist. It carries no `estimate`, because the fitter returns a plan rather than a byte range, so branch on the verdict rather than on bounds. It is reachable only for a single llama.cpp candidate whose registry record publishes a description; a set of candidates, a model with no description, and an offline caller keep the calibrated estimate.
...
`native-fit` is independent of calibration. Wherever the engine's fitter runs — every platform with a llama.cpp addon, mobile included — a single llama.cpp candidate with a published description gets the fitter's verdict, including `likely-fits`, before any coefficients are consulted.
