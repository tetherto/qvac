...
1. [`getLogger()`](/references/api/getLogger) — create a logger
2. [`loadModel()`](/references/api/loadModel) — pass logger via `logger` option
3. [`loggingStream()`](/references/api/loggingStream) — stream real-time logs from models or SDK server
