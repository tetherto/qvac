...
Downloads in QVAC are _resumable by default_. When you download an asset via [`downloadAsset()`](/references/api/downloadAsset) or [`loadModel()`](/references/api/loadModel)), the SDK writes partial files to disk so the next run can continue from where it left off. The progress callback provides a `downloadKey`. Persist it if you want to programmatically pause/cancel the download.

## Functions

1. [`downloadAsset()`](/references/api/downloadAsset) or [`loadModel()`](/references/api/loadModel) — with `onProgress` for progress tracking
2. [`cancel()`](/references/api/cancel) — pause/cancel a download using the `downloadKey`

For how to use each function, see [SDK — API reference](/references/api/).

## Flow

- Pause: call [`cancel()`](/references/api/cancel) with `operation: "downloadAsset"` and the `downloadKey` from progress events.
