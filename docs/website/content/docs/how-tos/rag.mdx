...
RAG (retrieval-augmented generation) uses [text embeddings](/how-tos/text-embeddings): you embed (vectorize) documents, store them in a vector DB, and later retrieve the most relevant chunks for a query using similarity search.
...
- You _typically_ pass the retrieved text to [`completion()`](/references/api/completion) as context to ground the model's answer (this retrieval step is what makes it RAG).

Note that you should bring your own vector DB (e.g., HyperDB, LanceDB, ChromaDB, SQLite-Vector, etc.).

## Functions

1. [`loadModel()`](/references/api/loadModel) (with `modelType: "embeddings"`)
2. [`embed()`](/references/api/embed) — generate vectors
3. Use any combination of the RAG functions below as needed:
   - [`ragChunk()`](/references/api/ragChunk) — chunk documents
   - [`ragIngest()`](/references/api/ragIngest) — embed and store (requires model)
   - [`ragSaveEmbeddings()`](/references/api/ragSaveEmbeddings) — save pre-computed vectors
   - [`ragSearch()`](/references/api/ragSearch) — query similar documents (requires model)
   - [`ragReindex()`](/references/api/ragReindex) — optimize search index
   - [`ragDeleteEmbeddings()`](/references/api/ragDeleteEmbeddings) — remove documents
   - [`ragListWorkspaces()`](/references/api/ragListWorkspaces) — list workspaces
   - [`ragCloseWorkspace()`](/references/api/ragCloseWorkspace) — release resources
   - [`ragDeleteWorkspace()`](/references/api/ragDeleteWorkspace) — delete workspace and data
4. [`unloadModel()`](/references/api/unloadModel)

For how to use each function, see [SDK — API reference](/references/api/).

## Pipeline

Create your RAG pipeline using [text embeddings](/how-tos/text-embeddings) and [completion](/how-tos/completion).
...
**Tip:** all examples throughout this documentation are self-contained and runnable. For instructions on how to run them, see [SDK quickstart](/getting-started/quickstart).
</Callout>
