...
Delegation is configured at model-load time by passing a `delegate` object to [`loadModel()`](/references/api/loadModel). The provider is started separately using [`startQVACProvider()`](/references/api/startQVACProvider).

## Functions

**Provider:**
1. [`startQVACProvider()`](/references/api/startQVACProvider) — join a topic and serve requests
2. [`stopQVACProvider()`](/references/api/stopQVACProvider) — leave the topic

**Consumer:**
1. [`loadModel()`](/references/api/loadModel) — with `delegate` option
2. [`completion()`](/references/api/completion) / [`transcribe()`](/references/api/transcribe) / [`translate()`](/references/api/translate) / etc. — same as local
...
- To stop a running provider and leave the topic, call [`stopQVACProvider()`](/references/api/stopQVACProvider).
