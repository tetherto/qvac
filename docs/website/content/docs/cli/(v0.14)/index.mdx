...
This is what makes QVAC usable as a local model provider: any tool that already speaks that API can point at it and run against models on your own machine, with no code change.

<Cards>
  <Card href="/cli/http-server" title="HTTP server">
    Run a local HTTP server that exposes an OpenAI-compatible API.
  </Card>
  <Card href="/cli/http-server/connection" title="Connect AI tools to QVAC">
    Use the HTTP server as a local model provider for AI tools that support OpenAI-compatible API.
  </Card>
  <Card href="/cli/http-server/integration" title="Integrate with the OpenAI-compatible server">
