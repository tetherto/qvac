...
`qvac configure` searches the SDK's model catalog, previews the exact `serve.models` entry it will write, and merges into an existing `qvac.config.json` — see [`qvac configure`](/cli#qvac-configure) for the full reference.
...
Served by the `--openai` extension. All endpoints follow the [OpenAI API](https://ecosystem.openai.com/docs/api-reference) request and response format. Base path: `/v1`.
