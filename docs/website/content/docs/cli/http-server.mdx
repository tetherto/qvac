...
* [Completion](/ai-capabilities/text-generation) — via `/v1/chat/completions`, `/v1/responses` (modern), or `/v1/completions` (legacy).
* [Text embeddings](/ai-capabilities/text-embeddings) — via `/v1/embeddings`.
* [Transcription](/ai-capabilities/transcription) — via `/v1/audio/transcriptions`.
* [Translation](/ai-capabilities/translation) (audio-to-English only) — via `/v1/audio/translations` (Whisper translate task).
* [Text-to-speech](/ai-capabilities/text-to-speech) — via `/v1/audio/speech`.
* [Image generation](/ai-capabilities/image-generation) — via `/v1/images/generations` and `/v1/images/edits`.
* [RAG](/ai-capabilities/rag) — via the `/v1/files` and `/v1/vector_stores` endpoints.
...
| Method | Path | Purpose |
| --- | --- | --- |
| `POST` | `/v1/files` | Upload bytes (multipart). |
| `GET` | `/v1/files` | List files currently held in memory. |
| `GET` | `/v1/files/:id` | File metadata. |
| `GET` | `/v1/files/:id/content` | Raw bytes with the stored `Content-Type` (used by image `response_format=url`). |
...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/v1/vector_stores` | List all stores (merged with on-disk RAG workspaces). |
| `POST` | `/v1/vector_stores` | Create a new store. |
| `GET` | `/v1/vector_stores/:id` | Retrieve store metadata. |
| `POST` | `/v1/vector_stores/:id` | Update `name`, `expires_after`, or `metadata`. |
| `DELETE` | `/v1/vector_stores/:id` | Delete the store and the underlying RAG workspace. |
| `POST` | `/v1/vector_stores/:id/search` | Embed `query` and run top-K similarity search. |
| `POST` | `/v1/vector_stores/:id/files` | Attach a previously-uploaded `/v1/files` entry (UTF-8 text content). |
