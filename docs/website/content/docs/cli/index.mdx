...
| `--cors` | Compatibility switch that enables CORS only when at least one origin is supplied by `--cors-origin` or `serve.cors.origins` |
| `--docs` | Mount the Swagger UI at `/docs`. Off by default; adds only same-port loopback origins to CORS automatically. |
...
Scripts that previously relied on wildcard CORS must name every trusted browser origin:

```bash
# Before: allowed every browser origin
qvac serve openai --cors

# After: repeat once per trusted origin
qvac serve openai \
