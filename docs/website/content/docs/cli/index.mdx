---
title: CLI
description: QVAC CLI provides tools and an HTTP server exposing the QVAC API, with an optional OpenAI-compatible extension.
ogImage: /og-cli.png
...
For mobile hosts (`android-arm64`, `ios-*`), `qvac bundle sdk` adds any addon platform packages the bundle needs (for example `@qvac/tts-ggml-android-arm64`) to `package.json` with your project's package manager, pinned to the addon's exact version, then bundles again. If it cannot install them, it prints the dependencies to add and bundles without them.
