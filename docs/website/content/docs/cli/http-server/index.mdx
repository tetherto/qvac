...
qvac serve openai [options]
  ...
  --cors-origin <origin>       Trusted CORS origin (repeatable; no wildcard)
  --cors                       Require an explicit configured CORS origin
  --docs                       Mount Swagger UI at /docs (adds loopback origins)
...
CORS is disabled unless trusted origins are supplied through repeatable `--cors-origin` flags or `serve.cors.origins` in `qvac.config.*`. Origins must be exact HTTP(S) origins without credentials, paths, queries, or fragments. Wildcard (`*`) access is not supported.
...
The legacy `--cors` flag is retained as a compatibility switch, but it no longer enables wildcard CORS. It now requires at least one origin from `--cors-origin` or `serve.cors.origins`; otherwise startup fails. Existing scripts can migrate as follows:
...
`--docs` adds only same-port loopback origins (`localhost`, `127.0.0.1`, and `[::1]`) to CORS so a locally opened Swagger UI can use "Try it out." It does not trust the bound non-loopback host, a port-forwarded origin, or arbitrary websites. Add each additional browser origin with `--cors-origin` or `serve.cors.origins`.
