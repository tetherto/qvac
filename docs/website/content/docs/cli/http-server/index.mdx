...
Use `--api-key` whenever `--host` is not a loopback address. If the server binds to a non-loopback address without a key, the CLI logs a security warning that the unauthenticated API is exposed to the network. The warning does not prevent startup, so automation should provide a key explicitly rather than treating the warning as enforcement.
