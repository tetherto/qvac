...
_At the moment, its main advantage is providing introspection of the models supported by QVAC for each API operation._ In addition, it provides branded exports, automatic configuration, and a discoverable handle for the [models.dev](https://models.dev/providers/qvac/) catalog, allowing QVAC to appear in `/connect` for [OpenCode](https://opencode.ai) and other catalog consumers.
...
const qvac = createQvac({
  ...
  apiKey: process.env.QVAC_API_KEY      // must match qvac serve --api-key when enabled
})
...
A managed server is started with a generated API key, so requests you make yourself must be authenticated. `provider.apiKey` returns the key of the server currently in use:
...
<Callout type="warn">
`provider.apiKey` is secret material. Read it fresh per request — crash recovery respawns the server with a new key — and never log it or pass it to a process you do not trust. The property is non-enumerable so spreads and object dumps of the provider do not carry it.
</Callout>

Managed integrations authenticate automatically. The OpenCode plugin gives OpenCode a per-session proxy token while keeping the managed serve key inside its host process. The OpenClaw plugin instead requires `openclaw onboard --auth-choice provider-plugin:qvac`; onboarding creates its private key file and configures both OpenClaw and the local serve to use it. Installing or enabling the OpenClaw plugin without onboarding is not sufficient.
