...
The HTTP server's primary use case is integrating local AI with coding agents (OpenCode, Cline, Aider, Continue, and Roo). Although the API is OpenAI-compatible, _the following behaviors require explicit configuration for this use case._
...
To update model metadata against our live model registry, run:
