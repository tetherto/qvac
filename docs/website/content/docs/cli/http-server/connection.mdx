...
- **API key** — if the server was started with `--api-key`, configure that exact value so the client sends it as a Bearer token. Otherwise requests are rejected with `401`. Authentication is strongly recommended for non-loopback binds.
