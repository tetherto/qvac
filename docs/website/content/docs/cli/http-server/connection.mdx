...
<Callout type="info" title="Which LLMs can I use?">
For a comparison of the LLMs supported by QVAC when used as a local model provider, see [QVAC on models.dev](https://models.dev/providers/qvac/).
</Callout>
...
- **API key** — when the server was started with `--api-key`, configure that exact value; a missing, placeholder, or incorrect token receives `401`. When server authentication is disabled, omit this field if the tool allows it. If the tool requires a non-empty placeholder, the unauthenticated server ignores its value. Authentication is strongly recommended for non-loopback binds.
  ...
  "provider": {
    "qvac": {
      ...
      "options": {
        ...
        "apiKey": "the-value-you-passed-to-qvac-serve"
      },
      "models": {
        "qwen3-8b-chat": {
          "name": "Qwen3 8B (chat)",
...
Drop `options.apiKey` only if you started the server without authentication. `opencode.json` is a plaintext file that is commonly committed, so prefer starting the server with `--api-key-file` and keeping the key somewhere you are willing to have on disk in the clear.
