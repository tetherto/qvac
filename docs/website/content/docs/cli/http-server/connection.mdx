...
#### Recommended: managed OpenCode plugin

For OpenCode, the easiest path is the published [`@qvac/opencode-plugin`](https://www.npmjs.com/package/@qvac/opencode-plugin). Add it to `opencode.json` and OpenCode starts a local managed `qvac serve`, registers the `qvac` provider, selects the default model, and tears the serve down on exit:
...
The plugin uses `qvac/qwen3.5-9b` by default. You can choose a smaller model through the plugin tuple or `QVAC_MODEL`:

```json title="opencode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": [["@qvac/opencode-plugin", { "model": "qwen3.5-4b" }]]
}
```

| Plugin model id | Use when | Notes |
| --- | --- | --- |
| `qwen3.5-9b` | You want the best local OpenCode experience and your machine can keep it warm. | Default. Best choice for tool use, repository edits, and multi-step coding tasks. |
| `qwen3.5-4b` | You need a smaller/faster model for lighter code questions or modest hardware. | Less reliable for tool-heavy workflows than 9B. |
...
All plugin model ids are friendly, models.dev-style aliases for QVAC constants; for example `qwen3.5-9b` loads `QWEN3_5_9B_MULTIMODAL_Q4_K_M` and appears in OpenCode as `qvac/qwen3.5-9b`.
...
OpenCode routes some tool-heavy work to `small_model`, not just lightweight title generation, so a 4k secondary model overflows quickly. If you configure a separate utility model, give it enough context for summaries and compaction rather than a tiny title-only budget.
