...
Each key under `models` must match an alias declared in `serve.models`; `limit.context` should match that alias's `ctx_size` so OpenCode tracks remaining context correctly; `limit.output` is **required** by current OpenCode for every custom model entry; and `modalities` declares the input/output types the model accepts. Note that text is currently the only supported modality.

The `apiKey: "qvac"` value in `options` is sent as the `Authorization` header. The server does not validate it, but some OpenAI-shaped clients refuse to send a request without a non-empty key, so setting any value avoids that edge case.
