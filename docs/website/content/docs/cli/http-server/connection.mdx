...
QVAC supports more SDK model constants than the friendly plugin catalog lists, including GPT-OSS and Gemma4 models. Use the manual HTTP-server path below when you want to run one of those constants directly.
...
Common raw SDK constants for larger local agent work include:

| SDK constant | Suggested alias | Notes |
| --- | --- | --- |
| `GPT_OSS_20B_INST_Q4_K_M` | `gpt-oss-20b` | Larger local text/code model with Harmony tool-call support. |
| `GEMMA4_31B_MULTIMODAL_Q4_K_M` | `gemma4-31b` | Larger Gemma4 model; needs a machine with enough memory. |
| `GEMMA4_2B_MULTIMODAL_Q4_K_M` | `gemma4-2b` | Smaller Gemma4 model; useful for testing Gemma4 behavior. |

For those models, the model format changes by layer:

| Layer | Format | Example |
| --- | --- | --- |
| QVAC serve config value | SDK constant | `GPT_OSS_20B_INST_Q4_K_M` |
| QVAC serve alias | Your chosen alias | `gpt-oss-20b` |
| OpenClaw model name | `qvac/<alias>` | `qvac/gpt-oss-20b` |
...
Start `qvac serve openai` with that config, register an OpenClaw custom provider whose model id is `gpt-oss-20b`, and select `qvac/gpt-oss-20b` in OpenClaw. The raw SDK constant belongs in `qvac.config.json`; the OpenClaw-facing model name should use the alias.
