...
`clip_on_cpu`, `vae_on_cpu` and `control_net_cpu` were removed and now fail the load. To keep the previous parameter placement, use `params_backend: 'te=cpu'` in place of `clip_on_cpu` and `params_backend: 'vae=cpu'` in place of `vae_on_cpu`. To move graph execution instead, use `backend` with the same module name. `control_net_cpu` has no parameter-residency equivalent — use `backend: 'controlnet=cpu'`.
