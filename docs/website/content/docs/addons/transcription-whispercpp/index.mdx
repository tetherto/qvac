...
[Bare module](https://bare.pears.com) that adds support for transcription in QVAC using [`qvac-fabric-speech.cpp`](https://github.com/tetherto/qvac-fabric-speech.cpp) as the inference engine.
...
If you don't want to stage files manually, use [`@qvac/registry-client`](https://www.npmjs.com/package/@qvac/registry-client) to download them to disk first, then pass the resulting paths to the constructor:
