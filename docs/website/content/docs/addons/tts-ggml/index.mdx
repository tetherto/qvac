...
Three engine families are wrapped — **Chatterbox**, **Supertonic**, and
**Parler-TTS** — each with its own GGUF layout under `models/`:
...
Point the addon at a custom location via `files.modelDir` (engine auto-detected from the GGUF filenames present), or pass explicit `files.t3Model` + `files.s3genModel` (Chatterbox), `files.supertonicModel` (Supertonic), or `files.parlerModel` (Parler-TTS).
