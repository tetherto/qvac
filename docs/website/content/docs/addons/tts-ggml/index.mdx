...
The published Linux x64 prebuild ships the Vulkan backend as a runtime-loaded
module, so `useGPU: true` uses Vulkan there. CUDA was dropped from that tarball
in 0.8.1 to stay under the npm registry size limit; it remains available as an
opt-in source build (`ENABLE_CUDA=ON`), where the engine prefers CUDA when the
NVIDIA driver and the CUDA 13 runtime libraries (`cudart`, `cuBLAS`) resolve at
load time and otherwise falls back to Vulkan or CPU. Where more than one
backend is usable, `TTS_CPP_GPU_BACKEND` (`cuda` | `vulkan` | `metal` |
`opencl`) pins the choice — the SDK worker inherits the host process
environment, so exporting it before starting the SDK is enough.
...
| `referenceAudio`                     | string  | —       | Mono WAV for voice cloning (Chatterbox: ≥ 5 s; Audio8: also needs `referenceText`) |
