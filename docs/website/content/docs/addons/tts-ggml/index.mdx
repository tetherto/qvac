...
| CosyVoice3              | `cosyvoice3/` model directory                         | Instruct-conditioned Chinese dialect and multilingual synthesis             |
...
Point the addon at a custom location via `files.modelDir` (engine auto-detected from the GGUF filenames present), or pass explicit engine file options. Use `npm run download-models:registry` to acquire registry-published Chatterbox, Supertonic, and Parler models. CosyVoice3 and Audio8 must currently be staged from local converted artifacts.
...
- Works across all five engine families. Native chunk enhancement is available
  where the selected engine supports native chunk streaming.
