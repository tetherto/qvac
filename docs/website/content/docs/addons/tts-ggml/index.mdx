...
modules. With `useGPU: true` the engine prefers CUDA when the NVIDIA driver and
the CUDA 13 runtime libraries (`cudart`, `cuBLAS`) resolve at load time;
otherwise the CUDA module is skipped and it falls back to Vulkan or CPU. Where
more than one backend is usable, `TTS_CPP_GPU_BACKEND` (`cuda` | `vulkan` |
`metal` | `opencl`) pins the choice — the SDK worker inherits the host process
