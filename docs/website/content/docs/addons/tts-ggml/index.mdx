...
The native prebuild for each host installs through a version-locked,
...
Do not depend on the platform packages directly. Supported installers are
npm 7+, pnpm, bun, and Yarn Berry; Yarn v1 and `--omit=optional` installs skip
the platform package and fail at require time with an error naming it. A
