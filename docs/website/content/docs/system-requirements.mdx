...
| GPU acceleration (Metal on macOS, Vulkan on Linux/Windows) | QVAC uses Metal on macOS (always present) and Vulkan `>=1.4` on Linux/Windows. On Windows, Vulkan is required even for CPU-only inference. |
