...
Minimum host requirements for running the QVAC SDK and `@qvac/cli`. These are worker-level — all SDK clients (JS/TS and Python) drive the same worker, so the requirements below apply regardless of which client you use.
