...
The JS/TS `getLogger` and `profiler` facades are intentionally not ported — Python uses the standard library [`logging`](https://docs.python.org/3/library/logging.html) module and the per-call `profiled_call` (`tetherto.qvac_sdk.profiling`) instead. See the [Logging](/runtime/logging) and [Profiler](/runtime/profiler) pages.
