...
For the alternative client, see the [JS/TS SDK](/js-ts-sdk). Both drive the
same worker over the same generated contract, so capabilities and behavior are
identical.
...
The `bare-rpc` extra is required — it is the wire transport the client uses to reach the worker. Optional extras: `vla`, `notebook`.
</Step>
