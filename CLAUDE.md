...
This copies shared config (conduct rules, knowledge docs, agents, skills) from `.agent/` into `.claude/` and `.cursor/`. Re-run anytime after pulling changes to `.agent/`.
...
Domain-specific reference docs in `.agent/knowledge/` (copied to `.claude/knowledge/` by `/setup`).
...
|-------|-------------|------|
| CI / GitHub Actions | CI failures, workflow triggers, validation, publishing | `.agent/knowledge/ci-validation.md` |
...
| vcpkg / native builds | vcpkg deps, triplets, registries, CMake integration, build failures | `.agent/knowledge/vcpkg-management.md` |
| llama.cpp Android | Cross-compiling llama.cpp, ADB deployment, Vulkan GPU, Android inference | `.agent/knowledge/llama-cpp-android.md` |
| Model registry | Adding/updating models, registry format, vcpkg port config | `.agent/knowledge/registry-models.md` |
