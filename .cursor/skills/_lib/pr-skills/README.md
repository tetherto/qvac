...
- [`pr-test-discover.mjs`](pr-test-discover.mjs) — discovery CLI for `/pr-test`. Reads cached PR metadata plus the committed-state patch from `worktree-prepare.mjs`, classifies touched packages, reads package.json scripts, identifies added/modified examples/tests by patch headers, and emits a JSON manifest with recommended tier information. It never uses `git diff`/`git status` against the worktree for classification.
