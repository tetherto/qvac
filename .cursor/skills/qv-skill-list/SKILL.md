...
- [`setup`](../setup/SKILL.md) — repo-wide bootstrap (`/setup`); copies shared config from `.agent/` into `.claude/` and `.cursor/`.
- `.agent/skills/` — framework skills (`orchestrate`, `release`, `ci-validate`, `commit-trace`, `review`); installed by `/setup`, not `qv-*` prefixed.
